...
/**
 * Base TestCase para SalesOrder. Mismo patron que CustomerTestCase:
 * seed parent rows (languages/locales/regions/countries/plans/tenants/
 * subscriptions) + companies/warehouses minimos (por cada tenant) +
 * roles/permissions con namespace sales_orders.*
 *
 * Plan enterprise activo en ambos tenants para que tests de bulk/export
 * no choquen contra plan_feature middlewares.
 */
abstract class SalesOrderTestCase extends TestCase
{
    use RefreshDatabase;
    ...
    protected function seedParentRows(): void
    {
        ...
        // Companies — una por tenant. SalesOrder.company_id es required.
        DB::table('companies')->insertOrIgnore([
            ['id' => 1, 'slug' => Str::random(22), 'name' => 'Cliente Default T1', 'is_active' => true, 'tenant_id' => 1, 'created_at' => now(), 'updated_at' => now()],
            ['id' => 2, 'slug' => Str::random(22), 'name' => 'Cliente Default T2', 'is_active' => true, 'tenant_id' => 2, 'created_at' => now(), 'updated_at' => now()],
        ]);

        // Warehouses — uno por tenant. SalesOrder.warehouse_id es required.
    ...
    protected function seedRolesAndPermissions(): void
    {
        ...
        foreach (['sales_orders.view', 'sales_orders.create', 'sales_orders.edit', 'sales_orders.delete', 'sales_orders.show'] as $perm) {
    ...
    /**
     * Devuelve el payload minimal del header + 1 item para crear via POST.
     * El test puede sobreescribir cualquier campo via $overrides.
     */
    protected function validOrderPayload(array $overrides = []): array
    {
        return array_merge([
            'reference'       => 'OV-TEST-' . Str::random(6),
            'company_id'      => 1,
            'warehouse_id'    => 1,
            'status'          => 'pending',
