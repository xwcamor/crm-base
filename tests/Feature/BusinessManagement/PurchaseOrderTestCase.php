...
/**
 * Base TestCase para PurchaseOrder. Mismo patron que SalesOrderTestCase:
 * seed parent rows (languages/locales/regions/countries/plans/tenants/
 * subscriptions) + companies (suppliers) + warehouses + products minimos
 * (por cada tenant) + roles/permissions con namespace purchase_orders.*
 *
 * Plan enterprise activo en ambos tenants para que tests de bulk/export
 * no choquen contra plan_feature middlewares.
 */
abstract class PurchaseOrderTestCase extends TestCase
{
    use RefreshDatabase;
    ...
    protected function seedParentRows(): void
    {
        ...
        // Companies (proveedores) — uno por tenant. PurchaseOrder.supplier_company_id es required.
        DB::table('companies')->insertOrIgnore([
            ['id' => 1, 'slug' => Str::random(22), 'name' => 'Proveedor T1', 'company_type' => 'supplier', 'is_active' => true, 'tenant_id' => 1, 'created_at' => now(), 'updated_at' => now()],
            ['id' => 2, 'slug' => Str::random(22), 'name' => 'Proveedor T2', 'company_type' => 'supplier', 'is_active' => true, 'tenant_id' => 2, 'created_at' => now(), 'updated_at' => now()],
        ]);

        // Warehouses — uno por tenant. PurchaseOrder.warehouse_id es required.
        ...
        // Products — uno por tenant. PurchaseOrder.items requiere product_id (nullable, pero
        // habitualmente referenciable).
    ...
    protected function seedRolesAndPermissions(): void
    {
        ...
        foreach (['purchase_orders.view', 'purchase_orders.create', 'purchase_orders.edit', 'purchase_orders.delete', 'purchase_orders.show'] as $perm) {
    ...
    /**
     * Payload minimal de PurchaseOrder + 1 item para POST store.
     * El test override cualquier campo via $overrides.
     */
    protected function validOrderPayload(array $overrides = []): array
    {
        $supplierId  = $overrides['supplier_company_id'] ?? 1;
        $warehouseId = $overrides['warehouse_id'] ?? 1;
        $productId   = $overrides['product_id'] ?? 1;

        return array_merge([
            'reference'           => 'PO-TEST-' . Str::random(6),
        ...
        ], array_diff_key($overrides, ['product_id' => true]));
    }
}
