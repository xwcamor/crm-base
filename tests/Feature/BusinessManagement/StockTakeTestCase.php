...
/**
 * Base TestCase para StockTake. Mismo patron que SalesOrderTestCase:
 * seed parent rows + warehouses por cada tenant + roles/permissions con
 * namespace stock_takes.*
 *
 * Plan enterprise activo en ambos tenants para que tests de bulk/export
 * no choquen contra plan_feature middlewares.
 */
abstract class StockTakeTestCase extends TestCase
{
    use RefreshDatabase;
    ...
    protected function seedParentRows(): void
    {
        ...
        // Warehouses — uno por tenant. StockTake.warehouse_id es required.
    ...
    protected function seedRolesAndPermissions(): void
    {
        ...
        foreach (['stock_takes.view', 'stock_takes.create', 'stock_takes.edit', 'stock_takes.delete', 'stock_takes.show'] as $perm) {
    ...
    /**
     * Payload minimal de StockTake para POST store. El service auto-genera
     * lineas desde StockLevel del warehouse (en estos tests, vacio).
     */
    protected function validStockTakePayload(array $overrides = []): array
    {
        return array_merge([
            'reference'    => 'COUNT-TEST-' . Str::random(6),
            'warehouse_id' => 1,
            'status'       => 'draft',
            'note'         => null,
        ], $overrides);
