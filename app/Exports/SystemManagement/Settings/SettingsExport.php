...
class SettingsExport implements FromCollection, WithEvents, WithTitle
{
    ...
    public function __construct($settings, array $options = [])
    {
        $this->settings = $settings instanceof Collection ? $settings : collect($settings);
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $headerRow  = 4;
                $dataStart  = 5;
                $dataEnd    = $dataStart + $this->settings->count() - 1;
