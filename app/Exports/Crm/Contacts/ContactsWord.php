...
namespace App\Exports\Crm\Contacts;
...
/**
 * Generates a styled .docx report of Contacts using PhpWord directly
...
class ContactsWord
{
    /** Map column key → ['heading' => string, 'value' => fn($contact) => mixed] */
    ...
    public function generate(
        ...
        $this->columnDefs = [
            'id'         => ['heading' => __('contacts.id'),        'value' => fn($c) => (string) $c->id],
            'name'       => ['heading' => __('contacts.name'),      'value' => fn($c) => (string) $c->name],
'is_active'  => ['heading' => __('contacts.is_active'), 'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('contacts.export_title');
        ...
        $brandCell->addText(
            __('global.generated_at') . ': ' . now()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT) . ' · ' . trans_choice('global.records_in_report', count($contacts), ['count' => count($contacts)]),
        ...
        if (count($contacts) === 0) {
            ...
            $phpWord->addTableStyle('ContactsTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('ContactsTable');
            ...
            foreach ($contacts as $i => $contact) {
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($contact);

                    if ($col === 'is_active') {
                        $color = $contact->is_active ? '1D7044' : 'C8281D';
