 ...
 *
 */
class ContactsExport implements FromCollection, WithEvents, WithTitle
{
    protected $contacts;
    ...
    public function __construct($contacts, array $options = [])
    {
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('contacts.id'),        'value' => fn($c, $i) => $c->id],
            'name'       => ['heading' => __('contacts.name'),      'value' => fn($c, $i) => $c->name],
'is_active'  => ['heading' => __('contacts.is_active'), 'value' => fn($c, $i) => $c->state_text],
    ...
    {
        $title    = $this->options['title'] ?? __('contacts.export_title');
        ...
        foreach ($this->contacts as $contact) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($contact, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('contacts.export_title'), 0, 31);
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $headerRow  = 4;
                $dataStart  = 5;
                $dataEnd    = $dataStart + $this->contacts->count() - 1;
