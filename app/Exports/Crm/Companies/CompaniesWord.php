...
namespace App\Exports\Crm\Companies;
...
/**
 * Generates a styled .docx report of Companies using PhpWord directly
...
class CompaniesWord
{
    /** Map column key → ['heading' => string, 'value' => fn($company) => mixed] */
...
'is_active'  => ['heading' => __('companies.is_active'), 'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('companies.export_title');
        ...
        $brandCell->addText(
            __('global.generated_at') . ': ' . now()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT) . ' · ' . trans_choice('global.records_in_report', count($companies), ['count' => count($companies)]),
        ...
        if (count($companies) === 0) {
            ...
            $phpWord->addTableStyle('CompaniesTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('CompaniesTable');
            ...
            foreach ($companies as $i => $company) {
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($company);

                    if ($col === 'is_active') {
                        $color = $company->is_active ? '1D7044' : 'C8281D';
