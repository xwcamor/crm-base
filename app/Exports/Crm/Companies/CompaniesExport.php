 ...
 *
 */
class CompaniesExport implements FromCollection, WithEvents, WithTitle
{
    protected $companies;
    ...
    public function __construct($companies, array $options = [])
    {
...
'is_active'  => ['heading' => __('companies.is_active'), 'value' => fn($c, $i) => $c->state_text],
    ...
    public function collection()
    {
        ...
        foreach ($this->companies as $company) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($company, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('companies.export_title'), 0, 31);
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $headerRow  = 4;
                $dataStart  = 5;
                $dataEnd    = $dataStart + $this->companies->count() - 1;
