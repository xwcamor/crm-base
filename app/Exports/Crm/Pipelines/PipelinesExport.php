 ...
 *
 */
class PipelinesExport implements FromCollection, WithEvents, WithTitle
{
    protected $pipelines;
    ...
    public function __construct($pipelines, array $options = [])
    {
        $this->pipelines = $pipelines instanceof Collection ? $pipelines : collect($pipelines);
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('pipelines.id'),        'value' => fn($c, $i) => $c->id],
            'name'       => ['heading' => __('pipelines.name'),      'value' => fn($c, $i) => $c->name],
'is_active'  => ['heading' => __('pipelines.is_active'), 'value' => fn($c, $i) => $c->state_text],
    ...
    {
        $title    = $this->options['title'] ?? __('pipelines.export_title');
        $count    = $this->pipelines->count();
        $subtitle = sprintf(
            '%s · %s · %s',
        ...
        foreach ($this->pipelines as $pipeline) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($pipeline, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('pipelines.export_title'), 0, 31);
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $headerRow  = 4;
                $dataStart  = 5;
                $dataEnd    = $dataStart + $this->pipelines->count() - 1;
