...
namespace App\Exports\Crm\Pipelines;
...
/**
 * Generates a styled .docx report of Pipelines using PhpWord directly
...
class PipelinesWord
{
    /** Map column key → ['heading' => string, 'value' => fn($pipeline) => mixed] */
    ...
    public function generate(
        ...
        $this->columnDefs = [
            'id'         => ['heading' => __('pipelines.id'),        'value' => fn($c) => (string) $c->id],
            'name'       => ['heading' => __('pipelines.name'),      'value' => fn($c) => (string) $c->name],
'is_active'  => ['heading' => __('pipelines.is_active'), 'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('pipelines.export_title');
        ...
        $brandCell->addText(
            __('global.generated_at') . ': ' . now()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT) . ' · ' . trans_choice('global.records_in_report', count($pipelines), ['count' => count($pipelines)]),
        ...
        if (count($pipelines) === 0) {
            ...
            $phpWord->addTableStyle('PipelinesTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('PipelinesTable');
            ...
            foreach ($pipelines as $i => $pipeline) {
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($pipeline);

                    if ($col === 'is_active') {
                        $color = $pipeline->is_active ? '1D7044' : 'C8281D';
