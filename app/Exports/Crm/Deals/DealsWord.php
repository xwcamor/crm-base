...
/**
 * Generates a styled .docx report of Deals using PhpWord directly
...
class DealsWord
{
    /** Map column key → ['heading' => string, 'value' => fn($deal) => mixed] */
    ...
    public function generate(
        ...
        $this->columnDefs = [
            'id'         => ['heading' => __('deals.id'),        'value' => fn($c) => (string) $c->id],
            'name'       => ['heading' => __('deals.name'),      'value' => fn($c) => (string) $c->name],
'is_active'  => ['heading' => __('deals.is_active'), 'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('deals.export_title');
        ...
        $brandCell->addText(
            __('global.generated_at') . ': ' . now()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT) . ' · ' . trans_choice('global.records_in_report', count($deals), ['count' => count($deals)]),
        ...
        if (count($deals) === 0) {
            ...
            $phpWord->addTableStyle('DealsTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('DealsTable');
            ...
            foreach ($deals as $i => $deal) {
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($deal);

                    if ($col === 'is_active') {
                        $color = $deal->is_active ? '1D7044' : 'C8281D';
