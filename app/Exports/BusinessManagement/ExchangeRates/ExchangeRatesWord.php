...
/**
 * Generates a styled .docx report of ExchangeRates using PhpWord directly.
 * SAP Fiori Quartz Light layout (mismo patron que DiscountsWord).
 */
class ExchangeRatesWord
{
    ...
    public function generate(
        ...
        $this->columnDefs = [
            'id'         => ['heading' => __('exchange_rates.id'),         'value' => fn($r) => (string) $r->id],
            'base_code'  => ['heading' => __('exchange_rates.base_code'),  'value' => fn($r) => (string) $r->base_code],
            'quote_code' => ['heading' => __('exchange_rates.quote_code'), 'value' => fn($r) => (string) $r->quote_code],
            'rate'       => ['heading' => __('exchange_rates.rate'),       'value' => fn($r) => (string) $r->rate],
            ...
            'source'     => ['heading' => __('exchange_rates.source'),     'value' => fn($r) => (string) ($r->source ?? '')],
        ...
        $title         = $options['title']         ?? __('exchange_rates.export_title');
        ...
        $brandCell->addText(
            __('global.generated_at') . ': ' . now()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT) . ' · ' . trans_choice('global.records_in_report', count($rates), ['count' => count($rates)]),
        ...
        if (count($rates) === 0) {
            ...
            $phpWord->addTableStyle('ExchangeRatesTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('ExchangeRatesTable');
            ...
            foreach ($rates as $i => $rate) {
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($rate);

                    if ($col === 'is_active') {
                        $color = $rate->is_active ? '1D7044' : 'C8281D';
