...
namespace App\Exports\BusinessManagement\Payments;
...
/**
 * Generates a styled .docx report of Payments using PhpWord directly
...
class PaymentsWord
{
    /** Map column key → ['heading' => string, 'value' => fn($payment) => mixed] */
    ...
    public function generate(
        ...
        $this->columnDefs = [
            'id'         => ['heading' => __('payments.id'),        'value' => fn($c) => (string) $c->id],
            'name'       => ['heading' => __('payments.name'),      'value' => fn($c) => (string) $c->name],
'is_active'  => ['heading' => __('payments.is_active'), 'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('payments.export_title');
        ...
        $brandCell->addText(
            __('global.generated_at') . ': ' . now()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT) . ' · ' . trans_choice('global.records_in_report', count($payments), ['count' => count($payments)]),
        ...
        if (count($payments) === 0) {
            ...
            $phpWord->addTableStyle('PaymentsTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('PaymentsTable');
            ...
            foreach ($payments as $i => $payment) {
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($payment);

                    if ($col === 'is_active') {
                        $color = $payment->is_active ? '1D7044' : 'C8281D';
