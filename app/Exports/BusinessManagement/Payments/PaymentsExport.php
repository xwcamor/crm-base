 ...
 *
 */
class PaymentsExport implements FromCollection, WithEvents, WithTitle
{
    protected $payments;
    ...
    public function __construct($payments, array $options = [])
    {
        $this->payments = $payments instanceof Collection ? $payments : collect($payments);
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('payments.id'),        'value' => fn($c, $i) => $c->id],
            'name'       => ['heading' => __('payments.name'),      'value' => fn($c, $i) => $c->name],
'is_active'  => ['heading' => __('payments.is_active'), 'value' => fn($c, $i) => $c->state_text],
    ...
    {
        $title    = $this->options['title'] ?? __('payments.export_title');
        ...
        foreach ($this->payments as $payment) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($payment, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('payments.export_title'), 0, 31);
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $headerRow  = 4;
                $dataStart  = 5;
                $dataEnd    = $dataStart + $this->payments->count() - 1;
