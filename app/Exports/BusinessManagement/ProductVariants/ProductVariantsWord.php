...
/**
 * Reporte .docx de Product Variants. SAP Fiori Quartz Light layout.
 */
class ProductVariantsWord
{
    ...
    public function generate(
        ...
        $this->columnDefs = [
            'id'         => ['heading' => __('product_variants.id'),         'value' => fn($v) => (string) $v->id],
            'name'       => ['heading' => __('product_variants.name'),       'value' => fn($v) => (string) $v->name],
            'sku'        => ['heading' => __('product_variants.sku'),        'value' => fn($v) => (string) $v->sku],
            'barcode'    => ['heading' => __('product_variants.barcode'),    'value' => fn($v) => (string) ($v->barcode ?? '')],
            'product'    => ['heading' => __('product_variants.product'),    'value' => fn($v) => (string) ($v->product?->name ?? '')],
            'attributes' => ['heading' => __('product_variants.attributes'), 'value' => fn($v) => is_array($v->attributes) ? json_encode($v->attributes, JSON_UNESCAPED_UNICODE) : ''],
            'cost'       => ['heading' => __('product_variants.cost'),       'value' => fn($v) => (string) $v->cost],
            'price'      => ['heading' => __('product_variants.price'),      'value' => fn($v) => (string) $v->price],
            'sort_order' => ['heading' => __('product_variants.sort_order'), 'value' => fn($v) => (string) $v->sort_order],
            'is_active'  => ['heading' => __('product_variants.is_active'),  'value' => fn($v) => $v->state_text],
            'slug'       => ['heading' => 'Slug',                             'value' => fn($v) => (string) $v->slug],
            'created_at' => ['heading' => __('global.created_at'),            'value' => fn($v) => $v->created_at?->copy()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT) ?? ''],
            'updated_at' => ['heading' => __('global.updated_at'),            'value' => fn($v) => $v->updated_at?->copy()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT) ?? ''],
            'creator'    => ['heading' => __('global.created_by'),            'value' => fn($v) => $v->creator->name ?? '—'],
        ];

        $title         = $options['title']         ?? __('product_variants.export_title');
        ...
        $brandCell->addText(
            __('global.generated_at') . ': ' . now()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT) . ' · ' . trans_choice('global.records_in_report', count($variants), ['count' => count($variants)]),
        ...
        if (count($variants) === 0) {
            ...
            $phpWord->addTableStyle('ProductVariantsTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('ProductVariantsTable');
            ...
            foreach ($variants as $i => $variant) {
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($variant);

                    if ($col === 'is_active') {
                        $color = $variant->is_active ? '1D7044' : 'C8281D';
