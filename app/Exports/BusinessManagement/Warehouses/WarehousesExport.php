 ...
 *
 */
class WarehousesExport implements FromCollection, WithEvents, WithTitle
{
    ...
    public function __construct($warehouses, array $options = [])
    {
        $this->warehouses = $warehouses instanceof Collection ? $warehouses : collect($warehouses);
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('warehouses.id'),        'value' => fn($c, $i) => $c->id],
            'name'       => ['heading' => __('warehouses.name'),      'value' => fn($c, $i) => $c->name],
'is_active'  => ['heading' => __('warehouses.is_active'), 'value' => fn($c, $i) => $c->state_text],
    ...
    {
        $title    = $this->options['title'] ?? __('warehouses.export_title');
        ...
        foreach ($this->warehouses as $warehouse) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($warehouse, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('warehouses.export_title'), 0, 31);
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $headerRow  = 4;
                $dataStart  = 5;
                $dataEnd    = $dataStart + $this->warehouses->count() - 1;
