...
/**
 * Generates a styled .docx report of Warehouses using PhpWord directly
 ...
 */
class WarehousesWord
{
    /** Map column key → ['heading' => string, 'value' => fn($warehouse) => mixed] */
    ...
    public function generate(
        ...
        $this->columnDefs = [
            'id'         => ['heading' => __('warehouses.id'),        'value' => fn($c) => (string) $c->id],
            'name'       => ['heading' => __('warehouses.name'),      'value' => fn($c) => (string) $c->name],
'is_active'  => ['heading' => __('warehouses.is_active'), 'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('warehouses.export_title');
        ...
        $brandCell->addText(
            __('global.generated_at') . ': ' . now()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT) . ' · ' . trans_choice('global.records_in_report', count($warehouses), ['count' => count($warehouses)]),
        ...
        if (count($warehouses) === 0) {
            ...
            $phpWord->addTableStyle('WarehousesTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('WarehousesTable');
            ...
            foreach ($warehouses as $i => $warehouse) {
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($warehouse);

                    if ($col === 'is_active') {
                        $color = $warehouse->is_active ? '1D7044' : 'C8281D';
