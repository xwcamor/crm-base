 ...
 *
 */
class InvoicesExport implements FromCollection, WithEvents, WithTitle
{
    protected $invoices;
    ...
    public function __construct($invoices, array $options = [])
    {
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('invoices.id'),        'value' => fn($c, $i) => $c->id],
            'name'       => ['heading' => __('invoices.name'),      'value' => fn($c, $i) => $c->name],
'is_active'  => ['heading' => __('invoices.is_active'), 'value' => fn($c, $i) => $c->state_text],
    ...
    {
        $title    = $this->options['title'] ?? __('invoices.export_title');
        ...
        foreach ($this->invoices as $invoice) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($invoice, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('invoices.export_title'), 0, 31);
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $headerRow  = 4;
                $dataStart  = 5;
                $dataEnd    = $dataStart + $this->invoices->count() - 1;
