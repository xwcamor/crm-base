...
namespace App\Exports\BusinessManagement\Invoices;
...
/**
 * Generates a styled .docx report of Invoices using PhpWord directly
...
class InvoicesWord
{
    /** Map column key → ['heading' => string, 'value' => fn($invoice) => mixed] */
    ...
    public function generate(
        ...
        $this->columnDefs = [
            'id'         => ['heading' => __('invoices.id'),        'value' => fn($c) => (string) $c->id],
            'name'       => ['heading' => __('invoices.name'),      'value' => fn($c) => (string) $c->name],
'is_active'  => ['heading' => __('invoices.is_active'), 'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('invoices.export_title');
        ...
        $brandCell->addText(
            __('global.generated_at') . ': ' . now()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT) . ' · ' . trans_choice('global.records_in_report', count($invoices), ['count' => count($invoices)]),
        ...
        if (count($invoices) === 0) {
            ...
            $phpWord->addTableStyle('InvoicesTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('InvoicesTable');
            ...
            foreach ($invoices as $i => $invoice) {
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($invoice);

                    if ($col === 'is_active') {
                        $color = $invoice->is_active ? '1D7044' : 'C8281D';
