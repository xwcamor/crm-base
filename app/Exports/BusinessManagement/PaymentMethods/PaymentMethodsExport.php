 ...
 */
class PaymentMethodsExport implements FromCollection, WithEvents, WithTitle
{
    protected $methods;
    ...
    public function __construct($methods, array $options = [])
    {
        $this->methods = $methods instanceof Collection ? $methods : collect($methods);
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'                   => ['heading' => __('payment_methods.id'),                   'value' => fn($m, $i) => $m->id],
            'name'                 => ['heading' => __('payment_methods.name'),                 'value' => fn($m, $i) => $m->name],
            'code'                 => ['heading' => __('payment_methods.code'),                 'value' => fn($m, $i) => $m->code ?? ''],
            'description'          => ['heading' => __('payment_methods.description'),          'value' => fn($m, $i) => $m->description ?? ''],
            'integration_provider' => ['heading' => __('payment_methods.integration_provider'), 'value' => fn($m, $i) => $m->integration_provider ?? ''],
            'requires_reference'   => ['heading' => __('payment_methods.requires_reference'),   'value' => fn($m, $i) => $m->requires_reference ? __('global.yes') : __('global.no')],
            'sort_order'           => ['heading' => __('payment_methods.sort_order'),           'value' => fn($m, $i) => $m->sort_order],
            'is_active'            => ['heading' => __('payment_methods.is_active'),            'value' => fn($m, $i) => $m->state_text],
            'slug'                 => ['heading' => 'Slug',                                      'value' => fn($m, $i) => $m->slug],
            'created_at'           => ['heading' => __('global.created_at'),                     'value' => fn($m, $i) => $m->created_at?->copy()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT)],
            'updated_at'           => ['heading' => __('global.updated_at'),                     'value' => fn($m, $i) => $m->updated_at?->copy()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT)],
            'creator'              => ['heading' => __('global.created_by'),                     'value' => fn($m, $i) => $m->creator->name ?? '—'],
        ];
    ...
    public function collection()
    {
        ...
        foreach ($this->methods as $method) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($method, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('payment_methods.export_title'), 0, 31);
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $headerRow  = 4;
                $dataStart  = 5;
                $dataEnd    = $dataStart + $this->methods->count() - 1;
