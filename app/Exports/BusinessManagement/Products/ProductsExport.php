 ...
 *
 */
class ProductsExport implements FromCollection, WithEvents, WithTitle
{
    protected $products;
    ...
    public function __construct($products, array $options = [])
    {
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('products.id'),        'value' => fn($c, $i) => $c->id],
            'name'       => ['heading' => __('products.name'),      'value' => fn($c, $i) => $c->name],
'is_active'  => ['heading' => __('products.is_active'), 'value' => fn($c, $i) => $c->state_text],
    ...
    public function collection()
    {
        ...
        foreach ($this->products as $product) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($product, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('products.export_title'), 0, 31);
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $headerRow  = 4;
                $dataStart  = 5;
                $dataEnd    = $dataStart + $this->products->count() - 1;
