...
namespace App\Exports\BusinessManagement\Products;
...
/**
 * Generates a styled .docx report of Products using PhpWord directly
...
class ProductsWord
{
    /** Map column key → ['heading' => string, 'value' => fn($product) => mixed] */
    ...
    public function generate(
        ...
        $this->columnDefs = [
            'id'         => ['heading' => __('products.id'),        'value' => fn($c) => (string) $c->id],
            'name'       => ['heading' => __('products.name'),      'value' => fn($c) => (string) $c->name],
'is_active'  => ['heading' => __('products.is_active'), 'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('products.export_title');
        ...
        $brandCell->addText(
            __('global.generated_at') . ': ' . now()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT) . ' · ' . trans_choice('global.records_in_report', count($products), ['count' => count($products)]),
        ...
        if (count($products) === 0) {
            ...
            $phpWord->addTableStyle('ProductsTable', [
                'borderSize'      => 4,
            ...
            foreach ($products as $i => $product) {
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($product);

                    if ($col === 'is_active') {
                        $color = $product->is_active ? '1D7044' : 'C8281D';
