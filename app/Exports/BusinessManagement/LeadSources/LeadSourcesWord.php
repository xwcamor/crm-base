...
/**
 * Reporte .docx de Lead Sources. SAP Fiori Quartz Light layout.
 */
class LeadSourcesWord
{
    ...
    public function generate(
        ...
        $this->columnDefs = [
            'id'          => ['heading' => __('lead_sources.id'),          'value' => fn($c) => (string) $c->id],
            'name'        => ['heading' => __('lead_sources.name'),        'value' => fn($c) => (string) $c->name],
            'description' => ['heading' => __('lead_sources.description'), 'value' => fn($c) => (string) ($c->description ?? '')],
            'category'    => ['heading' => __('lead_sources.category'),    'value' => fn($c) => (string) ($c->category ?? '')],
            'sort_order'  => ['heading' => __('lead_sources.sort_order'),  'value' => fn($c) => (string) $c->sort_order],
            'is_active'   => ['heading' => __('lead_sources.is_active'),   'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('lead_sources.export_title');
        ...
        $brandCell->addText(
            __('global.generated_at') . ': ' . now()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT) . ' · ' . trans_choice('global.records_in_report', count($leadSources), ['count' => count($leadSources)]),
        ...
        if (count($leadSources) === 0) {
            ...
            $phpWord->addTableStyle('LeadSourcesTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('LeadSourcesTable');
            ...
            foreach ($leadSources as $i => $leadSource) {
                $table->addRow(360);
                $isEven = $i % 2 === 1;
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($leadSource);

                    if ($col === 'is_active') {
                        $color = $leadSource->is_active ? '1D7044' : 'C8281D';
