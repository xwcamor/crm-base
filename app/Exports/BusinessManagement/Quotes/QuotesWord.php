...
namespace App\Exports\BusinessManagement\Quotes;
...
/**
 * Generates a styled .docx report of Quotes using PhpWord directly
...
class QuotesWord
{
    /** Map column key → ['heading' => string, 'value' => fn($quote) => mixed] */
    ...
    public function generate(
        ...
        $this->columnDefs = [
            'id'         => ['heading' => __('quotes.id'),        'value' => fn($c) => (string) $c->id],
            'name'       => ['heading' => __('quotes.name'),      'value' => fn($c) => (string) $c->name],
'is_active'  => ['heading' => __('quotes.is_active'), 'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('quotes.export_title');
        ...
        $brandCell->addText(
            __('global.generated_at') . ': ' . now()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT) . ' · ' . trans_choice('global.records_in_report', count($quotes), ['count' => count($quotes)]),
        ...
        if (count($quotes) === 0) {
            ...
            $phpWord->addTableStyle('QuotesTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('QuotesTable');
            ...
            foreach ($quotes as $i => $quote) {
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($quote);

                    if ($col === 'is_active') {
                        $color = $quote->is_active ? '1D7044' : 'C8281D';
