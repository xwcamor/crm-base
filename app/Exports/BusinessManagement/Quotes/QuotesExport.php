 ...
 *
 */
class QuotesExport implements FromCollection, WithEvents, WithTitle
{
    protected $quotes;
    ...
    public function __construct($quotes, array $options = [])
    {
        $this->quotes = $quotes instanceof Collection ? $quotes : collect($quotes);
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('quotes.id'),        'value' => fn($c, $i) => $c->id],
            'name'       => ['heading' => __('quotes.name'),      'value' => fn($c, $i) => $c->name],
'is_active'  => ['heading' => __('quotes.is_active'), 'value' => fn($c, $i) => $c->state_text],
    ...
    {
        $title    = $this->options['title'] ?? __('quotes.export_title');
        ...
        foreach ($this->quotes as $quote) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($quote, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('quotes.export_title'), 0, 31);
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $headerRow  = 4;
                $dataStart  = 5;
                $dataEnd    = $dataStart + $this->quotes->count() - 1;
