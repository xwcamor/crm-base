...
/**
 * Generates a styled .docx report of PriceLists using PhpWord directly.
 * SAP Fiori Quartz Light layout (mismo patron que DiscountsWord).
 */
class PriceListsWord
{
    ...
    public function generate(
        ...
        $this->columnDefs = [
            'id'                  => ['heading' => __('price_lists.id'),                  'value' => fn($c) => (string) $c->id],
            'name'                => ['heading' => __('price_lists.name'),                'value' => fn($c) => (string) $c->name],
            'description'         => ['heading' => __('price_lists.description'),         'value' => fn($c) => (string) ($c->description ?? '')],
            'currency_code'       => ['heading' => __('price_lists.currency'),            'value' => fn($c) => (string) ($c->currency_code ?? '')],
            'global_discount_pct' => ['heading' => __('price_lists.global_discount_pct'), 'value' => fn($c) => (string) $c->global_discount_pct],
            'priority'            => ['heading' => __('price_lists.priority'),            'value' => fn($c) => (string) $c->priority],
            'valid_from'          => ['heading' => __('price_lists.valid_from'),          'value' => fn($c) => $c->valid_from?->copy()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT) ?? ''],
            'valid_until'         => ['heading' => __('price_lists.valid_until'),         'value' => fn($c) => $c->valid_until?->copy()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT) ?? ''],
            'is_default'          => ['heading' => __('price_lists.is_default'),          'value' => fn($c) => $c->is_default ? __('global.yes') : __('global.no')],
            'is_active'           => ['heading' => __('price_lists.is_active'),           'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('price_lists.export_title');
        ...
        $brandCell->addText(
            __('global.generated_at') . ': ' . now()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT) . ' · ' . trans_choice('global.records_in_report', count($priceLists), ['count' => count($priceLists)]),
        ...
        if (count($priceLists) === 0) {
            ...
            $phpWord->addTableStyle('PriceListsTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('PriceListsTable');
            ...
            foreach ($priceLists as $i => $priceList) {
                $table->addRow(360);
                $isEven = $i % 2 === 1;
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($priceList);

                    if ($col === 'is_active') {
                        $color = $priceList->is_active ? '1D7044' : 'C8281D';
