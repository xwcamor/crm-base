 ...
 */
class PriceListsExport implements FromCollection, WithEvents, WithTitle
{
    ...
    public function __construct($priceLists, array $options = [])
    {
        $this->priceLists = $priceLists instanceof Collection ? $priceLists : collect($priceLists);
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'                  => ['heading' => __('price_lists.id'),                  'value' => fn($c, $i) => $c->id],
            'name'                => ['heading' => __('price_lists.name'),                'value' => fn($c, $i) => $c->name],
            'description'         => ['heading' => __('price_lists.description'),         'value' => fn($c, $i) => $c->description ?? ''],
            'currency_code'       => ['heading' => __('price_lists.currency'),            'value' => fn($c, $i) => $c->currency_code ?? ''],
            'global_discount_pct' => ['heading' => __('price_lists.global_discount_pct'), 'value' => fn($c, $i) => $c->global_discount_pct],
            'priority'            => ['heading' => __('price_lists.priority'),            'value' => fn($c, $i) => $c->priority],
            'valid_from'          => ['heading' => __('price_lists.valid_from'),          'value' => fn($c, $i) => $c->valid_from?->copy()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT) ?? ''],
            'valid_until'         => ['heading' => __('price_lists.valid_until'),         'value' => fn($c, $i) => $c->valid_until?->copy()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT) ?? ''],
            'is_default'          => ['heading' => __('price_lists.is_default'),          'value' => fn($c, $i) => $c->is_default ? __('global.yes') : __('global.no')],
            'is_active'           => ['heading' => __('price_lists.is_active'),           'value' => fn($c, $i) => $c->state_text],
    ...
    {
        $title    = $this->options['title'] ?? __('price_lists.export_title');
        $count    = $this->priceLists->count();
        $subtitle = sprintf(
            '%s · %s · %s',
        ...
        foreach ($this->priceLists as $priceList) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($priceList, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('price_lists.export_title'), 0, 31);
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $headerRow  = 4;
                $dataStart  = 5;
                $dataEnd    = $dataStart + $this->priceLists->count() - 1;
