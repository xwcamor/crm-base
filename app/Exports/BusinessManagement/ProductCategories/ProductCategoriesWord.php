...
/**
 * Reporte .docx de Product Categories. SAP Fiori Quartz Light layout.
 */
class ProductCategoriesWord
{
    ...
    public function generate(
        ...
        $this->columnDefs = [
            'id'          => ['heading' => __('product_categories.id'),          'value' => fn($c) => (string) $c->id],
            'name'        => ['heading' => __('product_categories.name'),        'value' => fn($c) => (string) $c->name],
            'description' => ['heading' => __('product_categories.description'), 'value' => fn($c) => (string) ($c->description ?? '')],
            'parent'      => ['heading' => __('product_categories.parent'),      'value' => fn($c) => (string) ($c->parent?->name ?? '')],
            'sort_order'  => ['heading' => __('product_categories.sort_order'),  'value' => fn($c) => (string) $c->sort_order],
            'is_active'   => ['heading' => __('product_categories.is_active'),   'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('product_categories.export_title');
        ...
        $brandCell->addText(
            __('global.generated_at') . ': ' . now()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT) . ' · ' . trans_choice('global.records_in_report', count($categories), ['count' => count($categories)]),
        ...
        if (count($categories) === 0) {
            ...
            $phpWord->addTableStyle('ProductCategoriesTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('ProductCategoriesTable');
            ...
            foreach ($categories as $i => $category) {
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($category);

                    if ($col === 'is_active') {
                        $color = $category->is_active ? '1D7044' : 'C8281D';
