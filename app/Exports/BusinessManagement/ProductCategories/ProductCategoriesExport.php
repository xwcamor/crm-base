 ...
 */
class ProductCategoriesExport implements FromCollection, WithEvents, WithTitle
{
    ...
    public function __construct($categories, array $options = [])
    {
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'          => ['heading' => __('product_categories.id'),          'value' => fn($c, $i) => $c->id],
            'name'        => ['heading' => __('product_categories.name'),        'value' => fn($c, $i) => $c->name],
            'description' => ['heading' => __('product_categories.description'), 'value' => fn($c, $i) => $c->description ?? ''],
            'parent'      => ['heading' => __('product_categories.parent'),      'value' => fn($c, $i) => $c->parent?->name ?? ''],
            'sort_order'  => ['heading' => __('product_categories.sort_order'),  'value' => fn($c, $i) => $c->sort_order],
            'is_active'   => ['heading' => __('product_categories.is_active'),   'value' => fn($c, $i) => $c->state_text],
    ...
    public function collection()
    {
        ...
        foreach ($this->categories as $category) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($category, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('product_categories.export_title'), 0, 31);
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $headerRow  = 4;
                $dataStart  = 5;
                $dataEnd    = $dataStart + $this->categories->count() - 1;
