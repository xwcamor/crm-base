...
/**
 * Generates a styled .docx report of Discounts using PhpWord directly.
 * SAP Fiori Quartz Light layout (mismo patron que WarehousesWord).
 */
class DiscountsWord
{
    ...
    public function generate(
        ...
        $this->columnDefs = [
            'id'                  => ['heading' => __('discounts.id'),                  'value' => fn($c) => (string) $c->id],
            'code'                => ['heading' => __('discounts.code'),                'value' => fn($c) => (string) $c->code],
            'name'                => ['heading' => __('discounts.name'),                'value' => fn($c) => (string) $c->name],
            'description'         => ['heading' => __('discounts.description'),         'value' => fn($c) => (string) ($c->description ?? '')],
            'type'                => ['heading' => __('discounts.type'),                'value' => fn($c) => $c->type_text],
            'value'               => ['heading' => __('discounts.value'),               'value' => fn($c) => (string) $c->value],
            'currency_code'       => ['heading' => __('discounts.currency'),            'value' => fn($c) => (string) ($c->currency_code ?? '')],
            'min_purchase_amount' => ['heading' => __('discounts.min_purchase_amount'), 'value' => fn($c) => (string) ($c->min_purchase_amount ?? '')],
            'usage_limit'         => ['heading' => __('discounts.usage_limit'),         'value' => fn($c) => (string) ($c->usage_limit ?? '')],
            'usage_per_customer'  => ['heading' => __('discounts.usage_per_customer'),  'value' => fn($c) => (string) ($c->usage_per_customer ?? '')],
            'usage_count'         => ['heading' => __('discounts.usage_count'),         'value' => fn($c) => (string) $c->usage_count],
            'valid_from'          => ['heading' => __('discounts.valid_from'),          'value' => fn($c) => $c->valid_from?->copy()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT) ?? ''],
            'valid_until'         => ['heading' => __('discounts.valid_until'),         'value' => fn($c) => $c->valid_until?->copy()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT) ?? ''],
            'is_active'           => ['heading' => __('discounts.is_active'),           'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('discounts.export_title');
        ...
        $brandCell->addText(
            __('global.generated_at') . ': ' . now()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT) . ' · ' . trans_choice('global.records_in_report', count($discounts), ['count' => count($discounts)]),
        ...
        if (count($discounts) === 0) {
            ...
            $phpWord->addTableStyle('DiscountsTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('DiscountsTable');
            ...
            foreach ($discounts as $i => $discount) {
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($discount);

                    if ($col === 'is_active') {
                        $color = $discount->is_active ? '1D7044' : 'C8281D';
