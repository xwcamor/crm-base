 ...
 *
 */
class TaxClassesExport implements FromCollection, WithEvents, WithTitle
{
    ...
    public function __construct($tax_classes, array $options = [])
    {
        $this->tax_classes = $tax_classes instanceof Collection ? $tax_classes : collect($tax_classes);
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('tax_classes.id'),        'value' => fn($c, $i) => $c->id],
            'name'       => ['heading' => __('tax_classes.name'),      'value' => fn($c, $i) => $c->name],
'is_active'  => ['heading' => __('tax_classes.is_active'), 'value' => fn($c, $i) => $c->state_text],
    ...
    {
        $title    = $this->options['title'] ?? __('tax_classes.export_title');
        $count    = $this->tax_classes->count();
        $subtitle = sprintf(
            '%s · %s · %s',
        ...
        // Row 5+ — data
        $i = 0;
        foreach ($this->tax_classes as $taxClass) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($taxClass, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('tax_classes.export_title'), 0, 31);
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $headerRow  = 4;
                $dataStart  = 5;
                $dataEnd    = $dataStart + $this->tax_classes->count() - 1;
