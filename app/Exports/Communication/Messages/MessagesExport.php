 ...
 *
 * Clon de DiscountsExport adaptado a Messages.
 */
class MessagesExport implements FromCollection, WithEvents, WithTitle
{
    protected $messages;
    ...
    public function __construct($messages, array $options = [])
    {
        $this->messages = $messages instanceof Collection ? $messages : collect($messages);
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'             => ['heading' => __('messages.id'),             'value' => fn($m) => $m->id],
            'subject'        => ['heading' => __('messages.subject'),        'value' => fn($m) => $m->subject],
            'audience_type'  => ['heading' => __('messages.audience_type'),  'value' => fn($m) => $m->audience_type_text],
            'audience_id'    => ['heading' => __('messages.audience_target'),'value' => fn($m) => $m->audience_id ?? ''],
            ...
            'slug'           => ['heading' => 'Slug',                        'value' => fn($m) => $m->slug],
    ...
    public function collection()
    {
        ...
        foreach ($this->messages as $message) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($message),
                $this->activeColumns
            ));
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('messages.export_title'), 0, 31);
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $headerRow  = 4;
                $dataStart  = 5;
                $dataEnd    = $dataStart + $this->messages->count() - 1;
