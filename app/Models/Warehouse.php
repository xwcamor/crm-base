...
/**
 * Warehouse — master template del scaffold `php artisan make:module`.
 *
...
class Warehouse extends Model
{
    ...
    protected string $auditModule = 'warehouses';

    protected $fillable = [
        'slug', 'code', 'name', 'description',
        'address_line', 'city', 'country_id',
        'type', 'is_default', 'is_active',
        'manager_user_id',
    ...
    public const TYPES = ['main', 'branch', 'dropship', 'virtual', 'consignment'];
...
public function scopeFilter($query, $request)
    {
        ...
        // Columnas con prefijo `warehouses.*` para evitar ambiguedad cuando
        ...
        $query->when($request->filled('city'), fn ($q) => $q->where("{$tbl}.city", 'like', '%' . $request->city . '%'));
        $query->when($request->has('is_default') && $request->is_default !== '',
            fn ($q) => $q->where("{$tbl}.is_default", filter_var($request->is_default, FILTER_VALIDATE_BOOLEAN)));
...
$query->when($request->has('is_active') && $request->is_active !== '', function ($q) use ($request, $tbl) {
     ...
     *   - key       : columna real de la BD (prefijada con `warehouses.`)
    ...
    {
        return [
            ['key' => 'name',       'label' => __('warehouses.name'),       'type' => 'string',  'operators' => ['=', '!=', 'contains']],
            ['key' => 'code',       'label' => __('warehouses.code'),       'type' => 'string',  'operators' => ['=', '!=', 'contains']],
            ['key' => 'city',       'label' => __('warehouses.city'),       'type' => 'string',  'operators' => ['=', '!=', 'contains']],
            ['key' => 'type',       'label' => __('warehouses.type'),       'type' => 'enum',    'operators' => ['=', '!='],
                'options' => collect(self::TYPES)->map(fn ($t) => ['value' => $t, 'label' => __('warehouses.type_options.' . $t)])->all()],
            ['key' => 'is_active',  'label' => __('warehouses.is_active'),  'type' => 'boolean', 'operators' => ['=']],
            ['key' => 'is_default', 'label' => __('warehouses.is_default'), 'type' => 'boolean', 'operators' => ['=']],
