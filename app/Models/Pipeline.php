...
/**
 * Pipeline — contenedor de stages (columnas del kanban) para Deals.
 *
 * Cada tenant puede tener múltiples pipelines (Sales / Renewal / Onboarding).
 * Un pipeline `is_default = true` por tenant — los deals nuevos lo usan
 * automáticamente salvo override explícito.
 */
class Pipeline extends Model
{
    use HasFactory, SoftDeletes, Auditable, BelongsToTenant, HasFavorites;

    protected string $auditModule = 'pipelines';

    protected $fillable = [
        'slug', 'name', 'description',
        'color', 'is_default', 'sort_order',
        'is_active', 'tenant_id',
...
public function scopeFilter($query, $request)
    {
        ...
        // Columnas con prefijo `pipelines.*` para evitar ambiguedad cuando
...
$query->when($request->has('is_active') && $request->is_active !== '', function ($q) use ($request, $tbl) {
     ...
     *   - key       : columna real de la BD (prefijada con `pipelines.`)
    ...
    {
        return [
            ['key' => 'name',       'label' => __('pipelines.name'),       'type' => 'string',  'operators' => ['=', '!=', 'contains']],
['key' => 'is_active',  'label' => __('pipelines.is_active'),  'type' => 'boolean', 'operators' => ['=']],
