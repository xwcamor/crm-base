...
/**
 * LeadSource — fuentes de leads per-tenant.
 *
...
class LeadSource extends Model
{
    ...
    protected string $auditModule = 'lead_sources';
    ...
    public function scopeFilter($query, $request)
    {
        ...
        $query->when($request->filled('category'), fn ($q) => $q->where("{$tbl}.category", $request->category));
    ...
    {
        return [
            ['key' => 'name',          'label' => __('lead_sources.name'),       'type' => 'string',  'operators' => ['=', '!=', 'contains']],
            ['key' => 'category',      'label' => __('lead_sources.category'),   'type' => 'string',  'operators' => ['=', '!=', 'contains']],
            ['key' => 'sort_order',    'label' => __('lead_sources.sort_order'), 'type' => 'number',  'operators' => ['=', '!=', '>', '<', '>=', '<=']],
            ['key' => 'is_active',     'label' => __('lead_sources.is_active'),  'type' => 'boolean', 'operators' => ['=']],
