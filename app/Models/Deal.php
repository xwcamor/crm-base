...
/**
 * Deal — oportunidad de venta. El core del CRM.
 *
 * Cada deal vive en un pipeline + stage. value en una currency. Cuando
 * cambia de stage se registra en stage_history (audit). Cuando se "wins"
 * se puede convertir en Quote → Order → Invoice.
 */
class Deal extends Model
{
    use HasFactory, SoftDeletes, Auditable, BelongsToTenant, HasFavorites;

    protected string $auditModule = 'deals';

    protected $fillable = [
        'slug', 'name', 'description', 'prefix', 'reference',
        'pipeline_id', 'stage_id', 'status',
        'value', 'currency_code', 'weighted_value',
        'expected_close_date', 'won_at', 'lost_at',
        'lost_reason_source_id', 'lost_reason_note',
        'company_id', 'contact_id', 'owner_id', 'lead_source_id',
        'probability_pct', 'external_id',
        'is_active', 'tenant_id',
    ...
    public const STATUSES = ['open', 'won', 'lost', 'dormant'];
...
protected static function booted(): void
    ...
    /** Tags polimorficos — usados por Deal Show + filtros. */
...
public function scopeFilter($query, $request)
    {
        ...
        // Columnas con prefijo `deals.*` para evitar ambiguedad cuando
...
$query->when($request->has('is_active') && $request->is_active !== '', function ($q) use ($request, $tbl) {
     ...
     *   - key       : columna real de la BD (prefijada con `deals.`)
    ...
    {
        return [
            ['key' => 'name',       'label' => __('deals.name'),       'type' => 'string',  'operators' => ['=', '!=', 'contains']],
['key' => 'is_active',  'label' => __('deals.is_active'),  'type' => 'boolean', 'operators' => ['=']],
