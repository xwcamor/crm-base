...
namespace App\Imports\BusinessManagement\LeadSources;
...
class LeadSourcesImport implements ToCollection, WithHeadingRow
{
    ...
    public function __construct(
        ...
        $this->currentCount = $this->tenantId !== null
            ? LeadSource::withoutGlobalScopes()->where('tenant_id', $this->tenantId)->count()
            : LeadSource::withoutGlobalScopes()->count();
    }
    ...
    public function collection(Collection $rows): void
    {
        ...
        try {
            ...
            foreach ($rows as $i => $row) {
                ...
                $description = $this->normalizeText($row['description'] ?? null, 255);
                $category    = $this->normalizeText($row['category']    ?? null, 60);
                ...
                if ($existing) {
                    ...
                    LeadSource::create(array_merge($payload, [
                        'name'       => $name,
                        'created_by' => Auth::id(),
                    ]));
    ...
    protected function findExistingByName(string $name): ?LeadSource
    {
        $query = LeadSource::query()->withoutGlobalScopes();

        if ($this->tenantId !== null) {
            $query->where('lead_sources.tenant_id', $this->tenantId);
        }
