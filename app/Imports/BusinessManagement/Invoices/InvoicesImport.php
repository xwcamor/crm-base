...
namespace App\Imports\BusinessManagement\Invoices;
...
/** tenant_id del usuario autenticado, capturado al construir. */
    ...
    /** Count actual de invoices del tenant (pre-import). */
    ...
    public function __construct(
        ...
        $this->currentCount = $this->tenantId !== null
            ? Invoice::withoutGlobalScopes()->where('tenant_id', $this->tenantId)->count()
            : Invoice::withoutGlobalScopes()->count();
    ...
    public function collection(Collection $rows): void
    {
        ...
        try {
            ...
            $seenInFileByName = [];
$newRecordsCount = 0; // contador de filas que crearian un nuevo invoice
...
/** Lowercase + strip accents (iconv) — mismo pattern que el DB-level layer 2. */
     ...
     */
    protected function findExistingByNameInsensitive(string $name): ?Invoice
    {
        $isPgsql = DB::getDriverName() === 'pgsql';
        $query   = Invoice::query()->withoutGlobalScopes();

        if ($this->tenantId !== null) {
            $query->where('invoices.tenant_id', $this->tenantId);
        }

        if ($isPgsql) {
            $query->whereRaw('unaccent(LOWER(invoices.name)) = unaccent(LOWER(?))', [$name]);
        } else {
            $query->whereRaw('LOWER(invoices.name) = LOWER(?)', [$name]);
        }
