...
namespace App\Imports\BusinessManagement\Products;
...
/** tenant_id del usuario autenticado, capturado al construir. */
    ...
    /** Count actual de products del tenant (pre-import). */
    ...
    public function __construct(
        ...
        $this->currentCount = $this->tenantId !== null
            ? Product::withoutGlobalScopes()->where('tenant_id', $this->tenantId)->count()
    ...
    public function collection(Collection $rows): void
    {
        ...
        try {
            ...
            $seenInFileByName = [];
$newRecordsCount = 0; // contador de filas que crearian un nuevo product
...
/** Lowercase + strip accents (iconv) — mismo pattern que el DB-level layer 2. */
     ...
     */
    protected function findExistingByNameInsensitive(string $name): ?Product
    {
        ...
        if ($isPgsql) {
            $query->whereRaw('unaccent(LOWER(products.name)) = unaccent(LOWER(?))', [$name]);
        } else {
            $query->whereRaw('LOWER(products.name) = LOWER(?)', [$name]);
        }
