<?php

namespace App\Imports\BusinessManagement\StockTakes;

use App\Models\StockLevel;
use App\Models\StockTake;
use App\Models\StockTakeLine;
use App\Models\Warehouse;
use Illuminate\Support\Collection;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\DB;
use Maatwebsite\Excel\Concerns\ToCollection;
use Maatwebsite\Excel\Concerns\WithHeadingRow;

/**
 * Imports Stock Takes desde .xlsx/.csv.
 *
 * Limitaciones de scope:
 *   - El import crea conteos con lineas auto-generadas desde el StockLevel
 *     actual del warehouse. qty_counted queda en null hasta que el usuario
 *     edite el conteo y registre los conteos fisicos.
 *
 * 2 layers de dedup:
 *   1. In-file: por reference normalizado
 *   2. DB: lookup case+accent insensitive contra el tenant
 *
 * Modes: 'create_only' | 'update_or_create'
 */
class StockTakesImport implements ToCollection, WithHeadingRow
{
    public int $created = 0;
    public int $updated = 0;
    public int $skipped = 0;

    /** @var array<int, array{row:int, message:string, value?:string}> */
    public array $errors = [];

    /** @var array<int, array> */
    public array $preview = [];

    protected ?int $tenantId;
    protected int $maxRecords;
    protected int $currentCount;

    /** Cache warehouseCode → warehouseId */
    protected array $warehouseCodeCache = [];

    public function __construct(
        protected string $mode = 'update_or_create',
        protected bool $dryRun = false,
    ) {
        $user           = Auth::user();
        $this->tenantId = $user?->tenant_id;

        if ($user && $user->tenant) {
            $this->maxRecords = $user->tenant->maxRecordsPerModule();
        } else {
            $this->maxRecords = PHP_INT_MAX;
        }

        $this->currentCount = $this->tenantId !== null
            ? StockTake::withoutGlobalScopes()->where('tenant_id', $this->tenantId)->count()
            : StockTake::withoutGlobalScopes()->count();

        // Precarga warehouses del tenant.
        $whQuery = Warehouse::query();
        if ($this->tenantId !== null) $whQuery->where('tenant_id', $this->tenantId);
        $whQuery->get(['id', 'code'])->each(function ($w) {
            if ($w->code) {
                $this->warehouseCodeCache[mb_strtoupper(trim($w->code))] = $w->id;
            }
        });
    }

    public function collection(Collection $rows): void
    {
        DB::beginTransaction();

        try {
            $seenInFile = [];
            $newRecords = 0;

            $validStatuses = StockTake::STATUSES;

            foreach ($rows as $i => $row) {
                $absoluteRow = $i + 2;

                $reference     = $this->normalizeString($row['reference'] ?? null);
                $warehouseCode = $this->normalizeIso($row['warehouse_code'] ?? null);
                $status        = $this->normalizeString($row['status'] ?? null) ?? 'draft';
                $note          = $this->normalizeString($row['note'] ?? null);

                if ($warehouseCode === null) {
                    $this->errors[] = ['row' => $absoluteRow, 'message' => __('stock_takes.import_warehouse_required'), 'value' => '—'];
                    continue;
                }

                $warehouseId = $this->warehouseCodeCache[$warehouseCode] ?? null;
                if (!$warehouseId) {
                    $this->errors[] = ['row' => $absoluteRow, 'message' => __('stock_takes.import_warehouse_not_found'), 'value' => $warehouseCode];
                    continue;
                }

                if (!in_array($status, $validStatuses, true)) {
                    $this->errors[] = ['row' => $absoluteRow, 'message' => __('stock_takes.import_invalid_status'), 'value' => $status];
                    continue;
                }

                if ($reference !== null) {
                    $refKey = $this->normalizeKey($reference);
                    if (isset($seenInFile[$refKey])) {
                        $this->errors[] = [
                            'row'     => $absoluteRow,
                            'message' => __('imports.err_duplicate_in_file', ['row' => $seenInFile[$refKey]]),
                            'value'   => $reference,
                        ];
                        continue;
                    }
                    $seenInFile[$refKey] = $absoluteRow;
                }

                $existing = $reference !== null ? $this->findExistingByReference($reference) : null;

                if ($existing) {
                    if ($this->mode === 'create_only') {
                        $this->skipped++;
                        $this->preview[] = $this->previewRow($absoluteRow, $reference, $warehouseCode, $status, 'skipped');
                        continue;
                    }

                    $patch = [];
                    if ((string) $existing->status !== $status)         $patch['status'] = $status;
                    if ($existing->warehouse_id !== $warehouseId)       $patch['warehouse_id'] = $warehouseId;
                    if ($note !== null && $existing->note !== $note)    $patch['note'] = $note;

                    if (!empty($patch)) $existing->fill($patch)->save();

                    $this->updated++;
                    $this->preview[] = $this->previewRow($absoluteRow, $reference, $warehouseCode, $status, 'updated');
                } else {
                    if ($this->maxRecords > 0 && $this->maxRecords !== PHP_INT_MAX) {
                        if (($this->currentCount + $newRecords) >= $this->maxRecords) {
                            $this->errors[] = [
                                'row'     => $absoluteRow,
                                'message' => __('plans.limit_records_reached', ['max' => $this->maxRecords]),
                                'value'   => $reference ?? $warehouseCode,
                            ];
                            continue;
                        }
                    }

                    $autoRef = $reference ?? $this->nextReference();

                    $take = StockTake::create([
                        'reference'    => $autoRef,
                        'warehouse_id' => $warehouseId,
                        'status'       => $status,
                        'started_at'   => $status !== 'draft' ? now() : null,
                        'note'         => $note,
                        'created_by'   => Auth::id(),
                    ]);

                    // Auto-genera lineas desde el StockLevel del warehouse.
                    $levels = StockLevel::where('warehouse_id', $warehouseId)->get();
                    foreach ($levels as $sl) {
                        StockTakeLine::create([
                            'stock_take_id' => $take->id,
                            'product_id'    => $sl->product_id,
                            'variant_id'    => $sl->variant_id ?? null,
                            'qty_system'    => $sl->qty_on_hand,
                            'qty_counted'   => null,
                            'variance'      => 0,
                        ]);
                    }

                    $newRecords++;
                    $this->created++;
                    $this->preview[] = $this->previewRow($absoluteRow, $autoRef, $warehouseCode, $status, 'created');
                }
            }

            if ($this->dryRun) {
                DB::rollBack();
            } else {
                DB::commit();
            }
        } catch (\Throwable $e) {
            DB::rollBack();
            throw $e;
        }
    }

    public function summary(): array
    {
        return [
            'created'      => $this->created,
            'updated'      => $this->updated,
            'skipped'      => $this->skipped,
            'error_count'  => count($this->errors),
            'total_rows'   => $this->created + $this->updated + $this->skipped + count($this->errors),
            'errors'       => array_slice($this->errors, 0, 50),
            'preview'      => array_slice($this->preview, 0, 100),
            'dry_run'      => $this->dryRun,
        ];
    }

    protected function previewRow(int $row, ?string $ref, string $warehouse, string $status, string $action): array
    {
        return [
            'row'       => $row,
            'reference' => $ref,
            'warehouse' => $warehouse,
            'status'    => $status,
            'action'    => $action,
        ];
    }

    protected function normalizeString(mixed $value): ?string
    {
        if ($value === null) return null;
        $v = trim((string) $value);
        return $v === '' ? null : $v;
    }

    protected function normalizeIso(mixed $value): ?string
    {
        if ($value === null) return null;
        $v = mb_strtoupper(trim((string) $value));
        return $v === '' ? null : $v;
    }

    protected function normalizeKey(string $name): string
    {
        $lower    = mb_strtolower(trim($name));
        $stripped = @iconv('UTF-8', 'ASCII//TRANSLIT//IGNORE', $lower);
        return $stripped !== false ? $stripped : $lower;
    }

    protected function findExistingByReference(string $reference): ?StockTake
    {
        $isPgsql = DB::getDriverName() === 'pgsql';
        $query   = StockTake::query()->withoutGlobalScopes();

        if ($this->tenantId !== null) {
            $query->where('stock_takes.tenant_id', $this->tenantId);
        }

        if ($isPgsql) {
            $query->whereRaw('unaccent(LOWER(stock_takes.reference)) = unaccent(LOWER(?))', [$reference]);
        } else {
            $query->whereRaw('LOWER(stock_takes.reference) = LOWER(?)', [$reference]);
        }

        return $query->first();
    }

    protected function nextReference(): string
    {
        $year  = now()->year;
        $count = StockTake::withoutGlobalScopes()
            ->where('tenant_id', $this->tenantId)
            ->whereYear('created_at', $year)
            ->count() + 1;
        return sprintf('IN-%d-%04d', $year, $count);
    }
}
