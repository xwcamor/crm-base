...
namespace App\Imports\BusinessManagement\TaxClasses;
...
class TaxClassesImport implements ToCollection, WithHeadingRow
{
...
/** tenant_id del usuario autenticado, capturado al construir. */
    ...
    /** Count actual de tax_classes del tenant (pre-import). */
    ...
    public function __construct(
        ...
        $this->currentCount = $this->tenantId !== null
            ? TaxClass::withoutGlobalScopes()->where('tenant_id', $this->tenantId)->count()
            : TaxClass::withoutGlobalScopes()->count();
    ...
    public function collection(Collection $rows): void
    {
        ...
        try {
            ...
            $seenInFileByName = [];
$newRecordsCount = 0; // contador de filas que crearian un nuevo taxClass
...
/** Lowercase + strip accents (iconv) — mismo pattern que el DB-level layer 2. */
     ...
     */
    protected function findExistingByNameInsensitive(string $name): ?TaxClass
    {
        $isPgsql = DB::getDriverName() === 'pgsql';
        $query   = TaxClass::query()->withoutGlobalScopes();

        if ($this->tenantId !== null) {
            $query->where('tax_classes.tenant_id', $this->tenantId);
        }

        if ($isPgsql) {
            $query->whereRaw('unaccent(LOWER(tax_classes.name)) = unaccent(LOWER(?))', [$name]);
        } else {
            $query->whereRaw('LOWER(tax_classes.name) = LOWER(?)', [$name]);
        }
