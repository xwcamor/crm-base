...
namespace App\Imports\BusinessManagement\Quotes;
...
/** tenant_id del usuario autenticado, capturado al construir. */
    ...
    /** Count actual de quotes del tenant (pre-import). */
    ...
    public function __construct(
        ...
        $this->currentCount = $this->tenantId !== null
            ? Quote::withoutGlobalScopes()->where('tenant_id', $this->tenantId)->count()
            : Quote::withoutGlobalScopes()->count();
    ...
    public function collection(Collection $rows): void
    {
        ...
        try {
            ...
            $seenInFileByName = [];
$newRecordsCount = 0; // contador de filas que crearian un nuevo quote
...
/** Lowercase + strip accents (iconv) — mismo pattern que el DB-level layer 2. */
     ...
     */
    protected function findExistingByNameInsensitive(string $name): ?Quote
    {
        $isPgsql = DB::getDriverName() === 'pgsql';
        $query   = Quote::query()->withoutGlobalScopes();

        if ($this->tenantId !== null) {
            $query->where('quotes.tenant_id', $this->tenantId);
        }

        if ($isPgsql) {
            $query->whereRaw('unaccent(LOWER(quotes.name)) = unaccent(LOWER(?))', [$name]);
        } else {
            $query->whereRaw('LOWER(quotes.name) = LOWER(?)', [$name]);
        }
