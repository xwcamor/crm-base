...
namespace App\Imports\BusinessManagement\PaymentMethods;
...
class PaymentMethodsImport implements ToCollection, WithHeadingRow
{
    ...
    public function __construct(
        ...
        $this->currentCount = $this->tenantId !== null
            ? PaymentMethod::withoutGlobalScopes()->where('tenant_id', $this->tenantId)->count()
            : PaymentMethod::withoutGlobalScopes()->count();
    }
    ...
    public function collection(Collection $rows): void
    {
        ...
        try {
            ...
            foreach ($rows as $i => $row) {
                ...
                $code                = $this->normalizeText($row['code'] ?? null, 30);
                $description         = $this->normalizeText($row['description'] ?? null, 500);
                $integrationProvider = $this->normalizeText($row['integration_provider'] ?? null, 60);
                $requiresReference   = $this->normalizeBool($row['requires_reference'] ?? null, default: false);
                ...
                if ($existing) {
                    ...
                    PaymentMethod::create(array_merge($payload, [
                        'name'       => $name,
                        'created_by' => Auth::id(),
                    ]));
    ...
    protected function findExistingByName(string $name): ?PaymentMethod
    {
        $query = PaymentMethod::query()->withoutGlobalScopes();

        if ($this->tenantId !== null) {
            $query->where('payment_methods.tenant_id', $this->tenantId);
        }
