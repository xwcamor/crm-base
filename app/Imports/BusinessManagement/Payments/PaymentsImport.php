...
namespace App\Imports\BusinessManagement\Payments;
...
/** tenant_id del usuario autenticado, capturado al construir. */
    ...
    /** Count actual de payments del tenant (pre-import). */
    ...
    public function __construct(
        ...
        $this->currentCount = $this->tenantId !== null
            ? Payment::withoutGlobalScopes()->where('tenant_id', $this->tenantId)->count()
    ...
    public function collection(Collection $rows): void
    {
        ...
        try {
            ...
            $seenInFileByName = [];
$newRecordsCount = 0; // contador de filas que crearian un nuevo payment
...
/** Lowercase + strip accents (iconv) — mismo pattern que el DB-level layer 2. */
     ...
     */
    protected function findExistingByNameInsensitive(string $name): ?Payment
    {
        $isPgsql = DB::getDriverName() === 'pgsql';
        $query   = Payment::query()->withoutGlobalScopes();

        if ($this->tenantId !== null) {
            $query->where('payments.tenant_id', $this->tenantId);
        }

        if ($isPgsql) {
            $query->whereRaw('unaccent(LOWER(payments.name)) = unaccent(LOWER(?))', [$name]);
        } else {
            $query->whereRaw('LOWER(payments.name) = LOWER(?)', [$name]);
        }
