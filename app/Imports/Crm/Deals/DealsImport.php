...
namespace App\Imports\Crm\Deals;
...
/** tenant_id del usuario autenticado, capturado al construir. */
    ...
    /** Count actual de deals del tenant (pre-import). */
    ...
    public function __construct(
        ...
        $this->currentCount = $this->tenantId !== null
            ? Deal::withoutGlobalScopes()->where('tenant_id', $this->tenantId)->count()
            : Deal::withoutGlobalScopes()->count();
    ...
    public function collection(Collection $rows): void
    {
        ...
        try {
            ...
            $seenInFileByName = [];
$newRecordsCount = 0; // contador de filas que crearian un nuevo deal
...
/** Lowercase + strip accents (iconv) — mismo pattern que el DB-level layer 2. */
     ...
     */
    protected function findExistingByNameInsensitive(string $name): ?Deal
    {
        $isPgsql = DB::getDriverName() === 'pgsql';
        $query   = Deal::query()->withoutGlobalScopes();

        if ($this->tenantId !== null) {
            $query->where('deals.tenant_id', $this->tenantId);
        }

        if ($isPgsql) {
            $query->whereRaw('unaccent(LOWER(deals.name)) = unaccent(LOWER(?))', [$name]);
        } else {
            $query->whereRaw('LOWER(deals.name) = LOWER(?)', [$name]);
        }
