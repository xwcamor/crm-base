...
namespace App\Imports\Crm\Companies;
...
/** tenant_id del usuario autenticado, capturado al construir. */
    ...
    /** Count actual de companies del tenant (pre-import). */
    ...
    public function __construct(
        ...
        $this->currentCount = $this->tenantId !== null
            ? Company::withoutGlobalScopes()->where('tenant_id', $this->tenantId)->count()
...
/** Lowercase + strip accents (iconv) — mismo pattern que el DB-level layer 2. */
    ...
    protected function findExistingByNameInsensitive(string $name): ?Company
    {
        ...
        $query   = Company::query()->withoutGlobalScopes();

        if ($this->tenantId !== null) {
            $query->where('companies.tenant_id', $this->tenantId);
        }
