...
namespace App\Imports\Crm\Contacts;
...
/** tenant_id del usuario autenticado, capturado al construir. */
    ...
    /** Count actual de contacts del tenant (pre-import). */
    ...
    public function __construct(
        ...
        $this->currentCount = $this->tenantId !== null
            ? Contact::withoutGlobalScopes()->where('tenant_id', $this->tenantId)->count()
    ...
    public function collection(Collection $rows): void
    {
        ...
        try {
            ...
            $seenInFileByName = [];
$newRecordsCount = 0; // contador de filas que crearian un nuevo contact
...
/** Lowercase + strip accents (iconv) — mismo pattern que el DB-level layer 2. */
     ...
     */
    protected function findExistingByNameInsensitive(string $name): ?Contact
    {
        $isPgsql = DB::getDriverName() === 'pgsql';
        $query   = Contact::query()->withoutGlobalScopes();

        if ($this->tenantId !== null) {
            $query->where('contacts.tenant_id', $this->tenantId);
        }

        if ($isPgsql) {
            $query->whereRaw('unaccent(LOWER(contacts.name)) = unaccent(LOWER(?))', [$name]);
        } else {
            $query->whereRaw('LOWER(contacts.name) = LOWER(?)', [$name]);
        }
