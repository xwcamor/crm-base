...
namespace App\Imports\Crm\Pipelines;
...
class PipelinesImport implements ToCollection, WithHeadingRow
{
...
/** tenant_id del usuario autenticado, capturado al construir. */
    ...
    /** Count actual de pipelines del tenant (pre-import). */
    ...
    public function __construct(
        ...
        $this->currentCount = $this->tenantId !== null
            ? Pipeline::withoutGlobalScopes()->where('tenant_id', $this->tenantId)->count()
            : Pipeline::withoutGlobalScopes()->count();
    ...
    public function collection(Collection $rows): void
    {
        ...
        try {
            ...
            $seenInFileByName = [];
$newRecordsCount = 0; // contador de filas que crearian un nuevo pipeline
...
/** Lowercase + strip accents (iconv) — mismo pattern que el DB-level layer 2. */
     ...
     */
    protected function findExistingByNameInsensitive(string $name): ?Pipeline
    {
        $isPgsql = DB::getDriverName() === 'pgsql';
        $query   = Pipeline::query()->withoutGlobalScopes();

        if ($this->tenantId !== null) {
            $query->where('pipelines.tenant_id', $this->tenantId);
        }

        if ($isPgsql) {
            $query->whereRaw('unaccent(LOWER(pipelines.name)) = unaccent(LOWER(?))', [$name]);
        } else {
            $query->whereRaw('LOWER(pipelines.name) = LOWER(?)', [$name]);
        }
