<?php

namespace App\Http\Controllers\Crm;

use App\Http\Controllers\Controller;
use App\Http\Requests\Crm\Company\BulkDeleteCompanyRequest;
use App\Http\Requests\Crm\Company\BulkRestoreCompanyRequest;
use App\Http\Requests\Crm\Company\BulkSetActiveCompanyRequest;
use App\Http\Requests\Crm\Company\DeleteCompanyRequest;
use App\Http\Requests\Crm\Company\EditAllUpdateCompanyRequest;
use App\Http\Requests\Crm\Company\ForceDeleteCompanyRequest;
use App\Http\Requests\Crm\Company\ImportCompanyRequest;
use App\Http\Requests\Crm\Company\StoreCompanyRequest;
use App\Http\Requests\Crm\Company\UpdateCompanyRequest;
use App\Http\Resources\AuditLogResource;
use App\Jobs\Crm\Companies\GenerateCompaniesCsvJob;
use App\Jobs\Crm\Companies\GenerateCompaniesExcelJob;
use App\Jobs\Crm\Companies\GenerateCompaniesPdfJob;
use App\Jobs\Crm\Companies\GenerateCompaniesWordJob;
use App\Models\AuditLog;
use App\Models\Company;
use App\Services\Crm\CompanyService;
use Illuminate\Http\RedirectResponse;
use Illuminate\Http\Request;

class CompanyController extends Controller
{
    public function index(Request $request)
    {
        $perPage = (int) $request->get('per_page', 25);
        $perPage = in_array($perPage, [10, 25, 50, 100, 200]) ? $perPage : 25;

        if (!$request->filled('sort')) {
            $request->merge(['sort' => 'id', 'direction' => 'desc']);
        }

        $userId  = $request->user()?->id;
        $isSuper = $request->user()?->hasRole('super') ?? false;

        // Solo super necesita el tenant eager-loaded — admins ven solo los suyos
        // y la columna workspace queda oculta en el frontend.
        $with = [
            'creator:id,name,email',
            // Relaciones para enriquecer el listado: industria + pais + owner.
            // Limitado a campos minimos por relacion para no inflar el JSON.
            'industry:id,name',
            'country:id,name,iso2',
            'owner:id,name,email',
        ];
        if ($isSuper) {
            $with[] = 'tenant:id,name';
        }

        $companies = Company::query()
            ->select('companies.*')
            ->withCount(['deals' => fn ($q) => $q->where('status', 'open')])
            ->with($with)
            ->orderByFavoriteFirst($userId)
            ->filter($request)
            ->paginate($perPage)
            ->withQueryString();

        $totalUnfiltered = Company::count();

        $names = $request->get('name', []);
        if (is_string($names)) $names = $names === '' ? [] : [$names];

        return inertia('Companies/Index', [
            'companies' => array_merge($companies->toArray(), [
                'total_unfiltered' => $totalUnfiltered,
            ]),
            // Limites de export por formato — el frontend deshabilita formatos
            // que exceden su limite. CSV con 0 = sin limite (streaming).
            'exportLimits' => \App\Models\Setting::getExportLimits('companies'),
            'filters' => [
                'name'         => array_values($names),
'is_active'    => $request->has('is_active') && $request->is_active !== ''
                    ? filter_var($request->is_active, FILTER_VALIDATE_BOOLEAN)
                    : null,
                'created_from' => $request->get('created_from', ''),
                'created_to'   => $request->get('created_to', ''),
                'only_favorites' => $request->boolean('only_favorites'),
                'sort'         => $request->get('sort', 'id'),
                'direction'    => $request->get('direction', 'desc'),
                'per_page'     => $perPage,
                // Filtros avanzados: array de clausulas {field, op, value}
                // que el drawer construye. Lo persisto para que al recargar
                // la pagina (paginate, sort) el filtro siga aplicado.
                'advanced_where' => $this->parseAdvancedWhere($request),
            ],
            'isSuper'        => $isSuper,
            // Schema de campos filtrables — alimenta el drawer "Filtros
            // avanzados" del frontend (selects de field/op + control tipado
            // del valor). Cada modulo declara el suyo en su modelo.
            'filterSchema'   => Company::filterSchema(),
        ]);
    }

    /**
     * Normaliza `advanced_where` del request: viene como JSON string o
     * array directo segun como Inertia lo serialice. Filtra clausulas
     * vacias o incompletas antes de pasarlo al frontend.
     */
    protected function parseAdvancedWhere(\Illuminate\Http\Request $request): array
    {
        $raw = $request->input('advanced_where', []);
        if (is_string($raw)) {
            $raw = json_decode($raw, true) ?: [];
        }
        if (!is_array($raw)) return [];

        return array_values(array_filter($raw, fn ($c) =>
            is_array($c) && !empty($c['field']) && !empty($c['op'])
        ));
    }

    public function show(Request $request, Company $company)
    {
        $company->load([
            'creator:id,name,email',
            'deleter:id,name,email',
            'tags:id,name,color',
            // Relaciones para mostrar nombres en Show (en vez de IDs sueltos).
            'country:id,name,iso2',
            'industry:id,name',
            'owner:id,name,email',
            'accountManager:id,name,email',
            'parentCompany:id,name,slug',
            'referrerCompany:id,name,slug',
            'defaultPaymentMethod:id,name',
            'preferredLanguage:id,name,code',
        ]);

        $canSeeAudit = $request->user()?->hasAnyRole(['super', 'admin']) ?? false;
        $activity = $canSeeAudit
            ? AuditLogResource::collection(
                AuditLog::query()
                    ->where('auditable_type', Company::class)
                    ->where('auditable_id', $company->id)
                    ->with('user:id,name,email')
                    ->orderByDesc('created_at')
                    ->limit(20)
                    ->get(['id', 'user_id', 'event', 'old_values', 'new_values', 'created_at'])
            )->resolve()
            : [];

        $activities = $request->user()?->can('activities.view')
            ? \App\Models\Activity::query()
                ->where('activitable_type', Company::class)
                ->where('activitable_id', $company->id)
                ->with('actor:id,name,email')
                ->orderByDesc('created_at')
                ->limit(100)
                ->get()
                ->map(fn ($a) => $a->toPanelArray())
                ->all()
            : [];

        // Tabs de relaciones: contactos, deals, cotizaciones, facturas de esta
        // empresa. Cada uno gated por la permission view del modulo respectivo
        // para que el admin no vea tabs que su rol no puede abrir.
        $contacts = $request->user()?->can('contacts.view')
            ? \App\Models\Contact::query()
                ->where('company_id', $company->id)
                ->orderByDesc('id')
                ->limit(100)
                ->get(['id', 'slug', 'name', 'primary_email', 'primary_phone', 'job_title', 'is_active'])
                ->map(fn ($c) => [
                    'id'       => $c->id,
                    'slug'     => $c->slug,
                    'name'     => $c->name,
                    'email'    => $c->primary_email,
                    'phone'    => $c->primary_phone,
                    'position' => $c->job_title,
                    'is_active' => (bool) $c->is_active,
                ])
                ->all()
            : [];

        $deals = $request->user()?->can('deals.view')
            ? \App\Models\Deal::query()
                ->where('company_id', $company->id)
                ->orderByDesc('id')
                ->limit(100)
                ->get(['id', 'slug', 'name', 'status', 'value', 'currency_code', 'expected_close_date', 'stage_id'])
                ->map(fn ($d) => [
                    'id'       => $d->id,
                    'slug'     => $d->slug,
                    'name'     => $d->name,
                    'status'   => $d->status,
                    'value'    => $d->value,
                    'currency' => $d->currency_code,
                    'expected_close_date' => $d->expected_close_date?->toDateString(),
                ])
                ->all()
            : [];

        $quotes = $request->user()?->can('quotes.view')
            ? \App\Models\Quote::query()
                ->where('company_id', $company->id)
                ->orderByDesc('issue_date')
                ->limit(100)
                ->get(['id', 'slug', 'reference', 'status', 'grand_total', 'currency_code', 'issue_date', 'valid_until'])
                ->map(fn ($q) => [
                    'id'         => $q->id,
                    'slug'       => $q->slug,
                    'reference'  => $q->reference,
                    'status'     => $q->status,
                    'total'      => $q->grand_total,
                    'currency'   => $q->currency_code,
                    'issue_date' => $q->issue_date?->toDateString(),
                    'valid_until'=> $q->valid_until?->toDateString(),
                ])
                ->all()
            : [];

        $invoices = $request->user()?->can('invoices.view')
            ? \App\Models\Invoice::query()
                ->where('company_id', $company->id)
                ->orderByDesc('issue_date')
                ->limit(100)
                ->get(['id', 'slug', 'reference', 'status', 'grand_total', 'currency_code', 'issue_date', 'due_date'])
                ->map(fn ($i) => [
                    'id'         => $i->id,
                    'slug'       => $i->slug,
                    'reference'  => $i->reference,
                    'status'     => $i->status,
                    'total'      => $i->grand_total,
                    'currency'   => $i->currency_code,
                    'issue_date' => $i->issue_date?->toDateString(),
                    'due_date'   => $i->due_date?->toDateString(),
                ])
                ->all()
            : [];

        return inertia('Companies/Show', [
            'company'    => $this->payload($company, withAudit: true),
            'activity'   => $activity,
            'activities' => $activities,
            'contacts'   => $contacts,
            'deals'      => $deals,
            'quotes'     => $quotes,
            'invoices'   => $invoices,
            'canManageActivities' => $request->user()?->can('activities.create') ?? false,
        ]);
    }

    public function create()
    {
        return inertia('Companies/Form', array_merge(
            ['company' => null],
            $this->formSelectOptions()
        ));
    }

    /** Opciones para los Select del Form. */
    protected function formSelectOptions(): array
    {
        return [
            'countryOptions' => \App\Models\Country::query()
                ->where('is_active', true)->orderBy('name')
                ->get(['id', 'name', 'iso_code'])
                ->map(fn ($c) => ['value' => $c->id, 'label' => $c->name . ' (' . $c->iso_code . ')'])
                ->all(),

            'industryOptions' => \App\Models\Industry::query()
                ->where('is_active', true)->orderBy('name')
                ->get(['id', 'name'])
                ->map(fn ($i) => ['value' => $i->id, 'label' => $i->name])
                ->all(),

            // Owner visibility por rol:
            //   - super        → todos los users del sistema (cross-tenant)
            //   - admin        → users de su workspace
            //   - user simple  → solo a si mismo
            'ownerOptions' => (function () {
                $u = auth()->user();
                $q = \App\Models\User::query();

                if (!$u || !$u->hasRole('super')) {
                    // No-super: scopeamos al tenant.
                    $q->when($u?->tenant_id, fn ($qq, $tid) => $qq->where('tenant_id', $tid));

                    // No-admin (= user simple): solo a si mismo.
                    if ($u && !$u->hasAnyRole(['super', 'admin'])) {
                        $q->where('id', $u->id);
                    }
                }

                return $q->orderBy('name')->get(['id', 'name', 'email'])
                    ->map(fn ($x) => ['value' => $x->id, 'label' => $x->name . ' (' . $x->email . ')'])
                    ->all();
            })(),

            'currencyOptions' => \App\Models\Currency::query()
                ->where('is_active', true)->orderBy('code')
                ->get(['code', 'name', 'symbol'])
                ->map(fn ($c) => ['value' => $c->code, 'label' => $c->code . ' — ' . $c->symbol . ' ' . $c->name])
                ->all(),

            'languageOptions' => \App\Models\Language::query()
                ->where('is_active', true)->orderBy('name')
                ->get(['id', 'name', 'iso_code'])
                ->map(fn ($l) => ['value' => $l->id, 'label' => $l->name . ' (' . $l->iso_code . ')'])
                ->all(),

            'typeOptions' => collect(\App\Models\Company::COMPANY_TYPES)
                ->map(fn ($t) => ['value' => $t, 'label' => __('companies.company_type_options.' . $t)])
                ->all(),

            'stageOptions' => collect(\App\Models\Company::LIFECYCLE_STAGES)
                ->map(fn ($s) => ['value' => $s, 'label' => __('companies.lifecycle_stage_options.' . $s)])
                ->all(),

            'ratingOptions' => collect(\App\Models\Company::RATINGS)
                ->map(fn ($r) => ['value' => $r, 'label' => __('companies.rating_options.' . $r)])
                ->all(),

            'taxStatusOptions' => collect(\App\Models\Company::TAX_STATUSES)
                ->map(fn ($s) => ['value' => $s, 'label' => __('companies.tax_status_options.' . $s)])
                ->all(),

            'priorityOptions' => collect(\App\Models\Company::PRIORITIES)
                ->map(fn ($p) => ['value' => $p, 'label' => __('companies.priority_options.' . $p)])
                ->all(),

            'accountStatusOptions' => collect(\App\Models\Company::ACCOUNT_STATUSES)
                ->map(fn ($s) => ['value' => $s, 'label' => __('companies.account_status_options.' . $s)])
                ->all(),

            'churnRiskOptions' => collect(\App\Models\Company::CHURN_RISKS)
                ->map(fn ($r) => ['value' => $r, 'label' => __('companies.churn_risk_options.' . $r)])
                ->all(),

            'legalEntityOptions' => collect(\App\Models\Company::LEGAL_ENTITY_TYPES)
                ->map(fn ($t) => ['value' => $t, 'label' => $t])
                ->all(),

            'paymentMethodOptions' => \App\Models\PaymentMethod::query()
                ->when(auth()->user()?->tenant_id, fn ($q, $tid) => $q->where('tenant_id', $tid))
                ->where('is_active', true)->orderBy('sort_order')->orderBy('name')
                ->get(['id', 'name', 'code'])
                ->map(fn ($p) => ['value' => $p->id, 'label' => $p->name])
                ->all(),

            'referrerOptions' => \App\Models\Company::query()
                ->orderBy('name')->limit(500)
                ->get(['id', 'name'])
                ->map(fn ($c) => ['value' => $c->id, 'label' => $c->name])
                ->all(),

            // Default currency del tenant (para pre-seleccionar al crear).
            'defaultCurrencyCode' => \App\Support\CurrencyResolver::forCurrentUser(),
        ];
    }

    public function store(StoreCompanyRequest $request, CompanyService $service): RedirectResponse
    {
        // Limite de registros por modulo segun el plan del tenant.
        // super no tiene tenant → no aplica. -1 = ilimitado.
        $tenant = $request->user()?->tenant;
        if ($tenant) {
            $max = $tenant->maxRecordsPerModule();
            if ($max > 0 && Company::count() >= $max) {
                return back()->with('error', __('plans.limit_records_reached', ['max' => $max]));
            }
        }

        $service->create($request->validated());

        return redirect()
            ->route('crm.companies.index')
            ->with('success', __('companies.created'));
    }

    public function edit(Company $company)
    {
        return inertia('Companies/Form', array_merge(
            ['company' => $this->payload($company)],
            $this->formSelectOptions()
        ));
    }

public function update(UpdateCompanyRequest $request, Company $company, CompanyService $service): RedirectResponse
    {
        $service->update($company, $request->validated());

        return redirect()
            ->route('crm.companies.index')
            ->with('success', __('companies.saved'));
    }

    public function delete(Company $company)
    {
        return inertia('Companies/Delete', [
            'company' => $this->payload($company),
        ]);
    }

    public function deleteSave(DeleteCompanyRequest $request, Company $company, CompanyService $service): RedirectResponse
    {
        $service->delete($company, $request->validated()['deleted_description']);

        $this->storeUndoableDelete([$company->id]);

        return redirect()
            ->route('crm.companies.index')
            ->with('success', __('global.deleted_success'))
            ->with('recentDelete', $this->buildRecentDeletePayload([$company->id]));
    }

    /** Persiste el claim en sesion por el window de undo (60s). */
    protected function storeUndoableDelete(array $ids): void
    {
        session(['companies.recent_delete' => [
            'ids'        => array_values($ids),
            'expires_at' => now()->addSeconds(60)->toIso8601String(),
        ]]);
    }

    /** Payload que va al frontend via flash para disparar el toast. */
    protected function buildRecentDeletePayload(array $ids): array
    {
        return [
            'count'   => count($ids),
            'seconds' => 60,
        ];
    }

    public function trash(Request $request)
    {
        abort_unless($request->user()?->hasRole('super'), 403);

        $name    = $request->get('name', '');
        $perPage = (int) $request->get('per_page', 25);
        $perPage = in_array($perPage, [10, 25, 50, 100]) ? $perPage : 25;

        $companies = Company::onlyTrashed()
            ->with('deleter:id,name,email')
            ->when($name !== '', fn ($q) => $q->where('name', 'like', "%{$name}%"))
            ->orderByDesc('deleted_at')
            ->paginate($perPage)
            ->withQueryString();

        return inertia('Companies/Trash', [
            'companies' => $companies,
            'filters'   => [
                'name'     => $name,
                'per_page' => $perPage,
            ],
        ]);
    }

    public function restore(Request $request, $slug, CompanyService $service): RedirectResponse
    {
        abort_unless($request->user()?->hasRole('super'), 403);
        $model = Company::onlyTrashed()->where('slug', $slug)->firstOrFail();
        $service->restore($model);

        return redirect()
            ->route('crm.companies.trash')
            ->with('success', __('global.restored_success'));
    }

    /**
     * Edit All — pagina con tabla editable in-line de name + is_active.
     * El submit hace batch update en transaccion (editAllUpdate).
     */
    public function editAll(Request $request)
    {
        $perPage = (int) $request->get('per_page', 25);
        $perPage = in_array($perPage, [10, 25, 50, 100]) ? $perPage : 25;

        if (!$request->filled('sort')) {
            $request->merge(['sort' => 'id', 'direction' => 'asc']);
        }

        $companies = Company::query()
            ->filter($request)
            ->select('companies.id', 'companies.slug', 'companies.name',
                'companies.is_active')
            ->paginate($perPage)
            ->withQueryString();

        return inertia('Companies/EditAll', [
            'companies' => $companies,
            'filters'   => [
                'name'      => $request->get('name', ''),
                'is_active' => $request->has('is_active') && $request->is_active !== ''
                    ? filter_var($request->is_active, FILTER_VALIDATE_BOOLEAN)
                    : null,
                'sort'      => $request->get('sort', 'id'),
                'direction' => $request->get('direction', 'asc'),
                'per_page'  => $perPage,
            ],
        ]);
    }

    public function editAllUpdate(EditAllUpdateCompanyRequest $request, CompanyService $service): RedirectResponse
    {
        $touched = $service->editAllUpdate($request->validated()['changes']);

        return redirect()
            ->route('crm.companies.edit_all')
            ->with('success', __('global.updated_success') . " ({$touched})");
    }

    /**
     * Clona el company. Sufijo "(copia)" con sanity guard de 100 intentos.
     */
    public function duplicate(Request $request, Company $company, CompanyService $service): RedirectResponse
    {
        $clone = $service->duplicate($company);

        if (!$clone) {
            return back()->with('error', __('global.duplicate_failed'));
        }

        return redirect()
            ->route('crm.companies.index')
            ->with('success', __('global.duplicated_success'));
    }

    public function bulkRestore(BulkRestoreCompanyRequest $request, CompanyService $service): RedirectResponse
    {
        $result = $service->bulkRestore($request->validated()['ids']);

        if (!empty($result['queued'])) {
            return redirect()
                ->route('crm.companies.trash')
                ->with('success', __('global.bulk_in_queue', ['count' => $result['count']]));
        }

        return redirect()
            ->route('crm.companies.trash')
            ->with('success', __('global.restored_success') . " ({$result['restored']})");
    }

    public function forceDelete(ForceDeleteCompanyRequest $request, $slug, CompanyService $service): RedirectResponse
    {
        $model = Company::onlyTrashed()->where('slug', $slug)->firstOrFail();
        $data  = $request->validated();

        if (trim($data['name_confirmation']) !== $model->name) {
            return back()->withErrors(['name_confirmation' => __('global.force_delete_name_mismatch')]);
        }

        $service->forceDelete($model, $data['reason']);

        return redirect()
            ->route('crm.companies.trash')
            ->with('success', __('global.force_deleted_success'));
    }

    protected function payload(Company $m, bool $withAudit = false): array
    {
        $base = [
            'id'                => $m->id,
            'slug'              => $m->slug,
            'tags'              => $m->relationLoaded('tags')
                ? $m->tags->map(fn ($t) => ['id' => $t->id, 'name' => $t->name, 'color' => $t->color])->all()
                : [],
            'name'              => $m->name,
            'legal_name'        => $m->legal_name,
            'tax_id'            => $m->tax_id,
            'description'       => $m->description,
            'company_type'      => $m->company_type,
            'lifecycle_stage'   => $m->lifecycle_stage,
            'country_id'        => $m->country_id,
            'industry_id'       => $m->industry_id,
            'owner_id'          => $m->owner_id,
            'parent_company_id' => $m->parent_company_id,
            'website'           => $m->website,
            'annual_revenue'    => $m->annual_revenue,
            'employee_count'    => $m->employee_count,
            'founded_year'      => $m->founded_year,
            'external_id'       => $m->external_id,
            'reference'         => $m->reference,
            'tax_status'        => $m->tax_status,

            // Priorización
            'domain'              => $m->domain,
            'is_vip'              => (bool) $m->is_vip,
            'priority'            => $m->priority,
            'customer_since'      => $m->customer_since,
            'account_manager_id'  => $m->account_manager_id,

            // Pro fiscal + health
            'tax_exempt'                => (bool) $m->tax_exempt,
            'tax_exempt_reason'         => $m->tax_exempt_reason,
            'legal_entity_type'         => $m->legal_entity_type,
            'bank_account_info'         => $m->bank_account_info,
            'discount_default_pct'      => $m->discount_default_pct,
            'default_payment_method_id' => $m->default_payment_method_id,
            'account_status'            => $m->account_status,
            'health_score'              => $m->health_score,
            'churn_risk'                => $m->churn_risk,
            'referrer_company_id'       => $m->referrer_company_id,

            // Monetario
            'preferred_currency_code' => $m->preferred_currency_code,
            'payment_terms_days'      => $m->payment_terms_days,
            'credit_limit'            => $m->credit_limit,

            // Comunicación
            'preferred_language_id' => $m->preferred_language_id,
            'billing_email'         => $m->billing_email,

            // Scoring
            'rating' => $m->rating,
            'score'  => $m->score,

            // Social
            'logo_url'       => $m->logo_url,
            'linkedin_url'   => $m->linkedin_url,
            'facebook_url'   => $m->facebook_url,
            'twitter_handle' => $m->twitter_handle,
            'instagram_url'  => $m->instagram_url,

            'is_active'         => $m->is_active,
            'is_favorite'       => (bool) ($m->is_favorite ?? false),
            'created_at'        => $m->created_at,
            'updated_at'        => $m->updated_at,
            'deleted_at'        => $m->deleted_at,
        ];

        // Nombres de relaciones para mostrar en Show (solo si vienen pre-cargadas).
        $rel = fn ($name, array $cols = ['id', 'name']) => $m->relationLoaded($name) && $m->$name
            ? collect($cols)->mapWithKeys(fn ($c) => [$c => $m->$name->$c])->all()
            : null;
        $base['country']               = $rel('country', ['id', 'name', 'iso2']);
        $base['industry']              = $rel('industry');
        $base['owner']                 = $rel('owner', ['id', 'name', 'email']);
        $base['account_manager']       = $rel('accountManager', ['id', 'name', 'email']);
        $base['parent_company']        = $rel('parentCompany', ['id', 'name', 'slug']);
        $base['referrer_company']      = $rel('referrerCompany', ['id', 'name', 'slug']);
        $base['default_payment_method'] = $rel('defaultPaymentMethod');
        $base['preferred_language']    = $rel('preferredLanguage', ['id', 'name', 'code']);

        if ($withAudit) {
            $base['deleted_description'] = $m->deleted_description;
            $base['creator'] = $m->creator ? ['id' => $m->creator->id, 'name' => $m->creator->name, 'email' => $m->creator->email] : null;
            $base['deleter'] = $m->deleter ? ['id' => $m->deleter->id, 'name' => $m->deleter->name, 'email' => $m->deleter->email] : null;
        }
        return $base;
    }

    // ── EXPORTS ─────────────────────────────────────────────────────────
    // Los 4 formatos van a queue como jobs async (mismo patron que Regions).
    // El job se encarga de la query con scope + render + Download record.

    public function exportCsv(Request $request)
    {
        return $this->dispatchExport($request, 'csv', GenerateCompaniesCsvJob::class);
    }

    public function exportExcel(Request $request)
    {
        return $this->dispatchExport($request, 'excel', GenerateCompaniesExcelJob::class);
    }

    public function exportPdf(Request $request)
    {
        return $this->dispatchExport($request, 'pdf', GenerateCompaniesPdfJob::class);
    }

    public function exportWord(Request $request)
    {
        return $this->dispatchExport($request, 'word', GenerateCompaniesWordJob::class);
    }

    /**
     * Helper comun de los 4 export endpoints: parse options → limit check →
     * audit → dispatch. Mismo patron que Region.
     */
    protected function dispatchExport(Request $request, string $format, string $jobClass): RedirectResponse
    {
        $options = $this->buildExportOptions($request, $format);
        $this->assertExportLimit($format, $options);
        $this->recordExportAudit($format, $options);
        $jobClass::dispatch(auth()->id(), $options);

        return back()->with('success', __('global.download_in_queue'));
    }

    /**
     * Valida que el dataset no exceda el limite del formato. Usuarios con
     * plan premium (feature flag `export_unlimited_rows`) saltean el limite.
     */
    protected function assertExportLimit(string $format, array $options): void
    {
        if (\App\Support\FeatureGate::allows('export_unlimited_rows', auth()->user())
            && config('features.features.export_unlimited_rows') !== null) {
            return;
        }

        $limit = \App\Models\Setting::getExportLimit('companies', $format);
        if ($limit === 0) return; // CSV streaming, sin limite

        $count = $this->countForExport($options);
        if ($count > $limit) {
            abort(422, __('companies.export_limit_exceeded', [
                'count'  => number_format($count),
                'limit'  => number_format($limit),
                'format' => strtoupper($format),
            ]));
        }
    }

    /** Cuenta filas a exportar segun scope+filters. */
    protected function countForExport(array $options): int
    {
        $scope = $options['scope'] ?? 'filtered';

        if ($scope === 'selected') {
            return count($options['selected_ids'] ?? []);
        }
        if ($scope === 'all') {
            return Company::query()->count();
        }
        // Filters como Request para reusar scopeFilter.
        $fakeReq = new Request($options['filters'] ?? []);
        return Company::query()->filter($fakeReq)->count();
    }

    // ── IMPORTS (two-phase: dry_run preview + commit) ────────────────────
    // El frontend sube 2 veces: primero dry_run=true (preview con summary),
    // despues dry_run=false (commit).

    public function importTemplate()
    {
        return \Maatwebsite\Excel\Facades\Excel::download(
            new \App\Exports\Crm\Companies\CompaniesImportTemplate(),
            __('companies.import_template_filename')
        );
    }

    public function import(ImportCompanyRequest $request)
    {
        $data    = $request->validated();
        $mode    = $data['mode'] ?? 'update_or_create';
        $dryRun  = filter_var($data['dry_run'] ?? false, FILTER_VALIDATE_BOOLEAN);

        $importer = new \App\Imports\Crm\Companies\CompaniesImport(
            mode:   $mode,
            dryRun: $dryRun,
        );

        try {
            \Maatwebsite\Excel\Facades\Excel::import($importer, $data['file']);
        } catch (\Throwable $e) {
            \Log::error('CompaniesImport failed', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);

            return response()->json([
                'ok'      => false,
                'dry_run' => $dryRun,
                'message' => $this->humanizeImportError($e),
            ], 422);
        }

        return response()->json([
            'ok'      => true,
            'dry_run' => $dryRun,
            'summary' => $importer->summary(),
        ], 200);
    }

    /**
     * Convierte una excepcion de import en mensaje legible para el usuario.
     * El detalle tecnico queda en el log, no llega al cliente.
     */
    protected function humanizeImportError(\Throwable $e): string
    {
        $msg = $e->getMessage();

        if ($e instanceof \Illuminate\Database\QueryException) {
            if (str_contains($msg, 'unique') || str_contains($msg, 'duplicate')) {
                return __('imports.err_unique_violation');
            }
            if (str_contains($msg, 'NOT NULL') || str_contains($msg, 'null value')) {
                return __('imports.err_not_null_violation');
            }
            if (str_contains($msg, 'foreign key') || str_contains($msg, 'violates foreign')) {
                return __('imports.err_foreign_key_violation');
            }
        }

        return __('imports.process_failed');
    }

    // ── BULK OPERATIONS ─────────────────────────────────────────────────
    public function bulkDelete(BulkDeleteCompanyRequest $request, CompanyService $service): RedirectResponse
    {
        $data   = $request->validated();
        $result = $service->bulkDelete($data['ids'], $data['deleted_description']);

        if (!empty($result['queued'])) {
            // Async: el delete real ocurre despues del redirect; el undo
            // window de 60s no calza con un job que tarda minutos.
            return back()
                ->with('success', __('global.bulk_in_queue', ['count' => $result['count']]));
        }

        $deletedIds = $result['deleted'];
        $this->storeUndoableDelete($deletedIds);

        return back()
            ->with('success', __('global.deleted_success') . ' (' . count($deletedIds) . ')')
            ->with('recentDelete', $this->buildRecentDeletePayload($deletedIds));
    }

    /**
     * Undo dentro del window de 60s. Validamos contra session claim:
     * quien borro puede deshacer su propio error sin permisos extra.
     * Defense in depth: el service solo restaura las filas con
     * deleted_by = current user.
     */
    public function undoLastDelete(Request $request, CompanyService $service): RedirectResponse
    {
        $claim = session('companies.recent_delete');
        if (!$claim || !is_array($claim) || empty($claim['ids']) || empty($claim['expires_at'])) {
            return back()->with('error', __('global.undo_failed'));
        }
        if (now()->isAfter($claim['expires_at'])) {
            session()->forget('companies.recent_delete');
            return back()->with('error', __('global.undo_failed'));
        }

        $restored = $service->undoLastDelete($claim['ids'], (int) auth()->id());

        if (empty($restored)) {
            session()->forget('companies.recent_delete');
            return back()->with('error', __('global.undo_failed'));
        }

        session()->forget('companies.recent_delete');

        return back()->with('success', __('global.undo_done'));
    }

    public function bulkSetActive(BulkSetActiveCompanyRequest $request, CompanyService $service): RedirectResponse
    {
        $data   = $request->validated();
        $result = $service->bulkSetActive($data['ids'], (bool) $data['is_active']);

        if (!empty($result['queued'])) {
            return back()->with('success', __('global.bulk_in_queue', ['count' => $result['count']]));
        }

        return back()->with('success', __('global.updated_success') . " ({$result['changed']})");
    }

    // ── Export helpers ──────────────────────────────────────────────────

    /**
     * Opciones normalizadas que reciben todos los jobs de export. Allowlist
     * de columnas previene inyeccion de campos sensibles.
     */
    protected function buildExportOptions(Request $request, string $format): array
    {
        $allowedColumns = ['id', 'name',
            'is_active', 'slug', 'created_at', 'updated_at', 'creator'];

        $rules = [
            'scope'                   => 'nullable|in:filtered,selected,all',
            'selected_ids'            => 'array',
            'selected_ids.*'          => 'integer',
            'columns'                 => 'array|min:1',
            'columns.*'               => 'in:' . implode(',', $allowedColumns),
            'title'                   => 'nullable|string|max:120',
            'include_filters_summary' => 'boolean',
            'filters'                 => 'array',
        ];
        if ($format === 'pdf') {
            $rules['orientation'] = 'nullable|in:portrait,landscape';
            $rules['paper_size']  = 'nullable|in:a4,letter';
        }
        if ($format === 'excel') {
            $rules['autofilter']    = 'boolean';
            $rules['freeze_header'] = 'boolean';
        }

        $data = $request->validate($rules);

        return [
            'scope'                   => $data['scope']                   ?? 'filtered',
            'selected_ids'            => $data['selected_ids']            ?? [],
            'columns'                 => $data['columns']                 ?? $allowedColumns,
            'title'                   => $data['title']                   ?? __('companies.export_title'),
            'include_filters_summary' => $data['include_filters_summary'] ?? true,
            'filters'                 => $data['filters']                 ?? [],
            'orientation'             => $data['orientation']             ?? 'portrait',
            'paper_size'              => $data['paper_size']              ?? 'a4',
            'autofilter'              => $data['autofilter']              ?? true,
            'freeze_header'           => $data['freeze_header']           ?? true,
        ];
    }

    /**
     * Escribe audit log manual del export. Event = 'export_queued' registra
     * la INTENCION del usuario; el estado final (ready/failed) vive en `downloads`.
     */
    protected function recordExportAudit(string $format, array $options): void
    {
        AuditLog::create([
            'user_id'        => auth()->id(),
            'event'          => 'export_queued',
            'auditable_type' => Company::class,
            'auditable_id'   => null,
            'module'         => 'companies',
            'old_values'     => null,
            'new_values'     => [
                'format'                  => $format,
                'scope'                   => $options['scope']        ?? 'filtered',
                'columns'                 => $options['columns']      ?? [],
                'title'                   => $options['title']        ?? null,
                'orientation'             => $format === 'pdf'   ? ($options['orientation']    ?? null) : null,
                'paper_size'              => $format === 'pdf'   ? ($options['paper_size']     ?? null) : null,
                'autofilter'              => $format === 'excel' ? ($options['autofilter']     ?? null) : null,
                'freeze_header'           => $format === 'excel' ? ($options['freeze_header']  ?? null) : null,
                'include_filters_summary' => $options['include_filters_summary'] ?? false,
                'filters'                 => $options['filters']      ?? [],
                'selected_ids_count'      => count($options['selected_ids'] ?? []),
            ],
            'url'        => route('crm.companies.index'),
            'ip_address' => request()?->ip(),
            'user_agent' => substr((string) request()?->userAgent(), 0, 500),
            'created_at' => now(),
        ]);
    }
}
