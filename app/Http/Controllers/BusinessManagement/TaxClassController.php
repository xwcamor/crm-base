...
use App\Http\Requests\BusinessManagement\TaxClass\BulkDeleteTaxClassRequest;
use App\Http\Requests\BusinessManagement\TaxClass\BulkRestoreTaxClassRequest;
use App\Http\Requests\BusinessManagement\TaxClass\BulkSetActiveTaxClassRequest;
use App\Http\Requests\BusinessManagement\TaxClass\DeleteTaxClassRequest;
use App\Http\Requests\BusinessManagement\TaxClass\EditAllUpdateTaxClassRequest;
use App\Http\Requests\BusinessManagement\TaxClass\ForceDeleteTaxClassRequest;
use App\Http\Requests\BusinessManagement\TaxClass\ImportTaxClassRequest;
use App\Http\Requests\BusinessManagement\TaxClass\StoreTaxClassRequest;
use App\Http\Requests\BusinessManagement\TaxClass\UpdateTaxClassRequest;
use App\Http\Resources\AuditLogResource;
use App\Jobs\BusinessManagement\TaxClasses\GenerateTaxClassesCsvJob;
use App\Jobs\BusinessManagement\TaxClasses\GenerateTaxClassesExcelJob;
use App\Jobs\BusinessManagement\TaxClasses\GenerateTaxClassesPdfJob;
use App\Jobs\BusinessManagement\TaxClasses\GenerateTaxClassesWordJob;
...
class TaxClassController extends Controller
{
    public function index(Request $request)
    {
        ...
        $totalUnfiltered = TaxClass::count();

        $names = $request->get('name', []);
        if (is_string($names)) $names = $names === '' ? [] : [$names];

        return inertia('TaxClasses/Index', [
            'tax_classes' => array_merge($tax_classes->toArray(), [
                'total_unfiltered' => $totalUnfiltered,
            ]),
            // Limites de export por formato — el frontend deshabilita formatos
            // que exceden su limite. CSV con 0 = sin limite (streaming).
            'exportLimits' => \App\Models\Setting::getExportLimits('tax_classes'),
            'filters' => [
...
'is_active'    => $request->has('is_active') && $request->is_active !== ''
            ...
            'filterSchema'   => TaxClass::filterSchema(),
        ]);
    }
    ...
    public function show(Request $request, TaxClass $taxClass)
    {
        $taxClass->load(['creator:id,name,email', 'deleter:id,name,email']);

        $canSeeAudit = $request->user()?->hasAnyRole(['super', 'admin']) ?? false;
        $activity = $canSeeAudit
            ? AuditLogResource::collection(
                AuditLog::query()
                    ->where('auditable_type', TaxClass::class)
                    ->where('auditable_id', $taxClass->id)
        ...
        return inertia('TaxClasses/Show', [
            'taxClass' => $this->payload($taxClass, withAudit: true),
            'activity'     => $activity,
        ]);
    }
    ...
    {
        return inertia('TaxClasses/Form', [
            'taxClass' => null,
        ]);
    }

    public function store(StoreTaxClassRequest $request, TaxClassService $service): RedirectResponse
    {
        ...
        return redirect()
            ->route('business_management.tax_classes.index')
            ->with('success', __('tax_classes.created'));
    }

    public function edit(TaxClass $taxClass)
    {
        return inertia('TaxClasses/Form', [
            'taxClass'        => $this->payload($taxClass),
        ]);
    }

public function update(UpdateTaxClassRequest $request, TaxClass $taxClass, TaxClassService $service): RedirectResponse
    {
        $service->update($taxClass, $request->validated());

        return redirect()
            ->route('business_management.tax_classes.index')
            ->with('success', __('tax_classes.saved'));
    }

    public function delete(TaxClass $taxClass)
    {
        return inertia('TaxClasses/Delete', [
            'taxClass' => $this->payload($taxClass),
        ]);
    }

    public function deleteSave(DeleteTaxClassRequest $request, TaxClass $taxClass, TaxClassService $service): RedirectResponse
    {
        $service->delete($taxClass, $request->validated()['deleted_description']);

        $this->storeUndoableDelete([$taxClass->id]);

        return redirect()
            ->route('business_management.tax_classes.index')
            ->with('success', __('global.deleted_success'))
            ->with('recentDelete', $this->buildRecentDeletePayload([$taxClass->id]));
    }

    /** Persiste el claim en sesion por el window de undo (60s). */
    protected function storeUndoableDelete(array $ids): void
    {
        session(['tax_classes.recent_delete' => [
    ...
    public function trash(Request $request)
    {
        ...
        return inertia('TaxClasses/Trash', [
    ...
    public function restore(Request $request, $slug, TaxClassService $service): RedirectResponse
    {
        abort_unless($request->user()?->hasRole('super'), 403);
        $model = TaxClass::onlyTrashed()->where('slug', $slug)->firstOrFail();
        $service->restore($model);

        return redirect()
            ->route('business_management.tax_classes.trash')
    ...
    public function editAll(Request $request)
    {
        ...
        $tax_classes = TaxClass::query()
            ->filter($request)
            ->select('tax_classes.id', 'tax_classes.slug', 'tax_classes.name',
                'tax_classes.is_active')
            ->paginate($perPage)
            ->withQueryString();

        return inertia('TaxClasses/EditAll', [
            'tax_classes' => $tax_classes,
            'filters'   => [
                'name'      => $request->get('name', ''),
                'is_active' => $request->has('is_active') && $request->is_active !== ''
    ...
    public function editAllUpdate(EditAllUpdateTaxClassRequest $request, TaxClassService $service): RedirectResponse
    {
    ...
    /**
     * Clona el taxClass. Sufijo "(copia)" con sanity guard de 100 intentos.
     */
    public function duplicate(Request $request, TaxClass $taxClass, TaxClassService $service): RedirectResponse
    {
        $clone = $service->duplicate($taxClass);
    ...
    public function bulkRestore(BulkRestoreTaxClassRequest $request, TaxClassService $service): RedirectResponse
    {
    ...
    public function forceDelete(ForceDeleteTaxClassRequest $request, $slug, TaxClassService $service): RedirectResponse
    {
        $model = TaxClass::onlyTrashed()->where('slug', $slug)->firstOrFail();
    ...
    protected function payload(TaxClass $m, bool $withAudit = false): array
    {
        $base = [
            'id'          => $m->id,
            'slug'        => $m->slug,
    ...
    {
        return $this->dispatchExport($request, 'csv', GenerateTaxClassesCsvJob::class);
    }

    public function exportExcel(Request $request)
    {
        return $this->dispatchExport($request, 'excel', GenerateTaxClassesExcelJob::class);
    }

    public function exportPdf(Request $request)
    {
        return $this->dispatchExport($request, 'pdf', GenerateTaxClassesPdfJob::class);
    }

    public function exportWord(Request $request)
    {
        return $this->dispatchExport($request, 'word', GenerateTaxClassesWordJob::class);
    }
    ...
    protected function assertExportLimit(string $format, array $options): void
    {
        ...
        $limit = \App\Models\Setting::getExportLimit('tax_classes', $format);
        if ($limit === 0) return; // CSV streaming, sin limite

        $count = $this->countForExport($options);
        if ($count > $limit) {
            abort(422, __('tax_classes.export_limit_exceeded', [
    ...
    public function importTemplate()
    {
        return \Maatwebsite\Excel\Facades\Excel::download(
            new \App\Exports\BusinessManagement\TaxClasses\TaxClassesImportTemplate(),
            __('tax_classes.import_template_filename')
        );
    }

    public function import(ImportTaxClassRequest $request)
    {
        $data    = $request->validated();
        $mode    = $data['mode'] ?? 'update_or_create';
        $dryRun  = filter_var($data['dry_run'] ?? false, FILTER_VALIDATE_BOOLEAN);

        $importer = new \App\Imports\BusinessManagement\TaxClasses\TaxClassesImport(
            mode:   $mode,
            dryRun: $dryRun,
        );
        ...
        try {
            ...
            \Log::error('TaxClassesImport failed', [
                'error' => $e->getMessage(),
    ...
    // ── BULK OPERATIONS ─────────────────────────────────────────────────
    public function bulkDelete(BulkDeleteTaxClassRequest $request, TaxClassService $service): RedirectResponse
    {
        $data   = $request->validated();
     ...
     */
    public function undoLastDelete(Request $request, TaxClassService $service): RedirectResponse
    {
        $claim = session('tax_classes.recent_delete');
        if (!$claim || !is_array($claim) || empty($claim['ids']) || empty($claim['expires_at'])) {
            return back()->with('error', __('global.undo_failed'));
        }
        if (now()->isAfter($claim['expires_at'])) {
            session()->forget('tax_classes.recent_delete');
            return back()->with('error', __('global.undo_failed'));
        }
    ...
    public function bulkSetActive(BulkSetActiveTaxClassRequest $request, TaxClassService $service): RedirectResponse
    {
        $data   = $request->validated();
    ...
    protected function buildExportOptions(Request $request, string $format): array
    {
        ...
        return [
            ...
            'title'                   => $data['title']                   ?? __('tax_classes.export_title'),
    ...
    protected function recordExportAudit(string $format, array $options): void
    {
        AuditLog::create([
            ...
            'url'        => route('business_management.tax_classes.index'),
