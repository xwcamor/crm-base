 ...
 */
abstract class BasePaymentMethodExportJob implements ShouldQueue
{
    ...
    {
        ini_set('memory_limit', config('payment_methods.export_job_memory_limit', '512M'));
    ...
    protected function buildQuery()
    {
        ...
        $base = PaymentMethod::query()->withoutGlobalScopes();

        if ($this->tenantId !== null) {
            $base->where('payment_methods.tenant_id', $this->tenantId);
        }
        ...
        if ($scope === 'selected' && !empty($this->options['selected_ids'])) {
            return $base->whereIn('payment_methods.id', $this->options['selected_ids']);
        }
        if ($scope === 'all') {
            return $base;
        }
    ...
    protected function buildFiltersSummary(): array
    {
        ...
        if (!empty($f['name'])) {
            ...
            $out[] = ['label' => __('payment_methods.name'), 'value' => implode(', ', $names)];
        }
        if (!empty($f['code'])) {
            $out[] = ['label' => __('payment_methods.code'), 'value' => (string) $f['code']];
        }
        if (!empty($f['integration_provider'])) {
            $out[] = ['label' => __('payment_methods.integration_provider'), 'value' => (string) $f['integration_provider']];
        }
        if (isset($f['is_active']) && $f['is_active'] !== '' && $f['is_active'] !== null) {
            $bool = filter_var($f['is_active'], FILTER_VALIDATE_BOOLEAN);
            $out[] = ['label' => __('payment_methods.is_active'), 'value' => $bool ? __('global.active') : __('global.inactive')];
        }
        if (isset($f['requires_reference']) && $f['requires_reference'] !== '' && $f['requires_reference'] !== null) {
            $bool = filter_var($f['requires_reference'], FILTER_VALIDATE_BOOLEAN);
            $out[] = ['label' => __('payment_methods.requires_reference'), 'value' => $bool ? __('global.yes') : __('global.no')];
        }
    ...
    {
        $base = Str::slug($this->options['title'] ?? __('payment_methods.export_filename'));
