 ...
 * dispatch viven en WarehouseService.
 */
class BulkWarehousesActionJob implements ShouldQueue
{
    ...
    /**
     * Umbral configurable. Prioridad: Setting global -> config/warehouses.php -> 200.
    ...
    public static function asyncThreshold(): int
    {
        return \App\Models\Setting::getInt(
            ...
            (int) config('warehouses.bulk_async_threshold', 200),
        );
    }
    ...
    public function handle(WarehouseService $service): void
    {
        ...
        if (!$user) {
            \Log::warning('BulkWarehousesActionJob: user not found, aborting', [
                'user_id' => $this->userId,
                'action'  => $this->action,
            ]);
        ...
        foreach (array_chunk($this->ids, 200) as $chunk) {
            $warehouses = match ($this->action) {
                'restore' => Warehouse::onlyTrashed()->whereIn('id', $chunk)->get(),
                default   => Warehouse::whereIn('id', $chunk)->get(),
            };

            foreach ($warehouses as $warehouse) {
                try {
                    match ($this->action) {
                        'delete'     => $service->delete($warehouse, $this->payload['reason'] ?? 'Bulk delete'),
                        'set_active' => $this->setActive($service, $warehouse),
                        'restore'    => $service->restore($warehouse),
                        default      => throw new \InvalidArgumentException("Unknown action: {$this->action}"),
                    };
                    $processed++;
                } catch (\Throwable $e) {
                    $errors++;
                    \Log::warning("BulkWarehousesActionJob: error on warehouse {$warehouse->id}", [
                        'error' => $e->getMessage(),
                    ]);
                }
            }
        ...
        \Log::info("BulkWarehousesActionJob completed", [
            'user_id'   => $this->userId,
            'action'    => $this->action,
            'processed' => $processed,
            'errors'    => $errors,
    ...
    {
        \Log::error("BulkWarehousesActionJob failed", [
            'user_id' => $this->userId,
            'action'  => $this->action,
            'total'   => count($this->ids),
    ...
    protected function notifyUser(string $status, ?string $error = null): void
    {
        try {
            ...
            \Log::warning('BulkWarehousesActionJob: notify failed', ['error' => $e->getMessage()]);
        }
    }

    protected function setActive(WarehouseService $service, Warehouse $warehouse): void
    {
        $target = (bool) ($this->payload['is_active'] ?? true);
        if ((bool) $warehouse->is_active === $target) return;
        $service->update($warehouse, ['is_active' => $target]);
    }
}
