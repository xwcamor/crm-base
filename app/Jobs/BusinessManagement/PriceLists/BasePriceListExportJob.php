 ...
 *
 * Clon de BaseDiscountExportJob adaptado a PriceLists (per-tenant).
 */
abstract class BasePriceListExportJob implements ShouldQueue
{
    ...
    {
        ini_set('memory_limit', config('price_lists.export_job_memory_limit', '512M'));
    ...
    protected function buildQuery()
    {
        ...
        $base = PriceList::query()->withoutGlobalScopes();

        if ($this->tenantId !== null) {
            $base->where('price_lists.tenant_id', $this->tenantId);
        }
        ...
        if ($scope === 'selected' && !empty($this->options['selected_ids'])) {
            return $base->whereIn('price_lists.id', $this->options['selected_ids']);
        }
        if ($scope === 'all') {
            return $base;
        }
    ...
    protected function buildFiltersSummary(): array
    {
        ...
        if (!empty($f['name'])) {
            ...
            $out[] = ['label' => __('price_lists.name'), 'value' => implode(', ', $names)];
        }
        if (!empty($f['currency_code'])) {
            $out[] = ['label' => __('price_lists.currency'), 'value' => (string) $f['currency_code']];
        }
        if (isset($f['is_active']) && $f['is_active'] !== '' && $f['is_active'] !== null) {
            $bool = filter_var($f['is_active'], FILTER_VALIDATE_BOOLEAN);
            $out[] = ['label' => __('price_lists.is_active'), 'value' => $bool ? __('global.active') : __('global.inactive')];
        }
        if (isset($f['is_default']) && $f['is_default'] !== '' && $f['is_default'] !== null) {
            $bool = filter_var($f['is_default'], FILTER_VALIDATE_BOOLEAN);
            $out[] = ['label' => __('price_lists.is_default'), 'value' => $bool ? __('global.yes') : __('global.no')];
        }
    ...
    {
        $base = Str::slug($this->options['title'] ?? __('price_lists.export_filename'));
