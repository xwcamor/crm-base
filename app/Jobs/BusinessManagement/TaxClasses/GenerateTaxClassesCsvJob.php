 ...
 */
class GenerateTaxClassesCsvJob extends BaseTaxClassExportJob
{
    ...
    protected function executeExport(Download $download): void
    {
        ...
        $tempFile = tempnam(sys_get_temp_dir(), 'tax_classes_csv') . '.csv';
        ...
        try {
            ...
            $headings = [
                'id'         => __('tax_classes.id'),
                'name'       => __('tax_classes.name'),
                'is_active'  => __('tax_classes.is_active'),
                'slug'       => 'Slug',
            ...
            $this->buildQuery()->chunkById(1000, function ($tax_classes) use ($handle, $columns, $tz) {
                foreach ($tax_classes as $taxClass) {
                    $row = array_map(fn ($col) => match ($col) {
                        'id'         => $taxClass->id,
                        'name'       => $taxClass->name,
                        'is_active'  => $taxClass->is_active ? '1' : '0',
                        'slug'       => $taxClass->slug,
                        'created_at' => $taxClass->created_at?->copy()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT),
                        'updated_at' => $taxClass->updated_at?->copy()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT),
                        'creator'    => $taxClass->creator?->name ?? '',
                        default      => $taxClass->{$col} ?? '',
                    }, $columns);
                    fputcsv($handle, $row);
                }
            }, 'tax_classes.id', 'id');
