 ...
 *
 * Clon de BaseWarehouseExportJob adaptado a Discounts (per-tenant).
 */
abstract class BaseDiscountExportJob implements ShouldQueue
{
    ...
    {
        ini_set('memory_limit', config('discounts.export_job_memory_limit', '512M'));
    ...
    protected function buildQuery()
    {
        ...
        $base = Discount::query()->withoutGlobalScopes();

        if ($this->tenantId !== null) {
            $base->where('discounts.tenant_id', $this->tenantId);
        }
        ...
        if ($scope === 'selected' && !empty($this->options['selected_ids'])) {
            return $base->whereIn('discounts.id', $this->options['selected_ids']);
        }
        if ($scope === 'all') {
            return $base;
        }
    ...
    protected function buildFiltersSummary(): array
    {
        ...
        if (!empty($f['name'])) {
            ...
            $out[] = ['label' => __('discounts.name'), 'value' => implode(', ', $names)];
        }
        if (!empty($f['code'])) {
            $out[] = ['label' => __('discounts.code'), 'value' => (string) $f['code']];
        }
        if (!empty($f['type'])) {
            $out[] = ['label' => __('discounts.type'), 'value' => __('discounts.type_options.' . $f['type'])];
        }
        if (isset($f['is_active']) && $f['is_active'] !== '' && $f['is_active'] !== null) {
            $bool = filter_var($f['is_active'], FILTER_VALIDATE_BOOLEAN);
            $out[] = ['label' => __('discounts.is_active'), 'value' => $bool ? __('global.active') : __('global.inactive')];
        }
    ...
    {
        $base = Str::slug($this->options['title'] ?? __('discounts.export_filename'));
