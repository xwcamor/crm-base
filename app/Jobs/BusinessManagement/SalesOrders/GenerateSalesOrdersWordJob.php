...
use App\Exports\BusinessManagement\SalesOrders\SalesOrdersWord;
use App\Models\Download;
use Illuminate\Support\Facades\Storage;

class GenerateSalesOrdersWordJob extends BaseSalesOrderExportJob
{
    ...
    protected function executeExport(Download $download): void
    {
        ...
        $tempFile = tempnam(sys_get_temp_dir(), 'sales_orders_export') . '.docx';

        $opts = $this->options + ['timezone' => $this->userTimezone];

        (new SalesOrdersWord())->generate(
            orders:         $orders,
            filename:       $tempFile,
            options:        $opts,
