 ...
 */
class BulkSalesOrdersActionJob implements ShouldQueue
{
    ...
    public static function asyncThreshold(): int
    {
        return \App\Models\Setting::getInt(
            ...
            (int) config('sales_orders.bulk_async_threshold', 200),
        );
    }
    ...
    public function handle(SalesOrderService $service): void
    {
        $user = \App\Models\User::find($this->userId);
        if (!$user) {
            \Log::warning('BulkSalesOrdersActionJob: user not found, aborting', [
                'user_id' => $this->userId,
                'action'  => $this->action,
            ]);
        ...
        foreach (array_chunk($this->ids, 200) as $chunk) {
            $orders = match ($this->action) {
                'restore' => SalesOrder::onlyTrashed()->whereIn('id', $chunk)->get(),
                default   => SalesOrder::whereIn('id', $chunk)->get(),
            };
            ...
            foreach ($orders as $order) {
                try {
                    match ($this->action) {
                        ...
                        'set_status' => $this->setStatus($service, $order),
                    ...
                    $errors++;
                    \Log::warning("BulkSalesOrdersActionJob: error on sales_order {$order->id}", [
                        'error' => $e->getMessage(),
                    ]);
                }
            }
        ...
        \Log::info("BulkSalesOrdersActionJob completed", [
            'user_id'   => $this->userId,
            'action'    => $this->action,
            'processed' => $processed,
            'errors'    => $errors,
    ...
    {
        \Log::error("BulkSalesOrdersActionJob failed", [
            'user_id' => $this->userId,
            'action'  => $this->action,
            'total'   => count($this->ids),
    ...
    protected function notifyUser(string $status, ?string $error = null): void
    {
        try {
            ...
            \Log::warning('BulkSalesOrdersActionJob: notify failed', ['error' => $e->getMessage()]);
        }
    }

    protected function setStatus(SalesOrderService $service, SalesOrder $order): void
    {
        $target = (string) ($this->payload['status'] ?? 'pending');
        if ((string) $order->status === $target) return;
        $service->update($order, ['status' => $target]);
    }
}
