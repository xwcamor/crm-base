 ...
 */
abstract class BaseLeadSourceExportJob implements ShouldQueue
{
    ...
    {
        ini_set('memory_limit', config('lead_sources.export_job_memory_limit', '512M'));
    ...
    protected function buildQuery()
    {
        ...
        $base = LeadSource::query()->withoutGlobalScopes();

        if ($this->tenantId !== null) {
            $base->where('lead_sources.tenant_id', $this->tenantId);
        }
        ...
        if ($scope === 'selected' && !empty($this->options['selected_ids'])) {
            return $base->whereIn('lead_sources.id', $this->options['selected_ids']);
        }
        if ($scope === 'all') {
            return $base;
        }
    ...
    protected function buildFiltersSummary(): array
    {
        ...
        if (!empty($f['name'])) {
            ...
            $out[] = ['label' => __('lead_sources.name'), 'value' => implode(', ', $names)];
        }
        if (!empty($f['category'])) {
            $out[] = ['label' => __('lead_sources.category'), 'value' => $f['category']];
        }
        if (isset($f['is_active']) && $f['is_active'] !== '' && $f['is_active'] !== null) {
            $bool = filter_var($f['is_active'], FILTER_VALIDATE_BOOLEAN);
            $out[] = ['label' => __('lead_sources.is_active'), 'value' => $bool ? __('global.active') : __('global.inactive')];
        }
    ...
    {
        $base = Str::slug($this->options['title'] ?? __('lead_sources.export_filename'));
