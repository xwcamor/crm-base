 ...
 */
abstract class BaseProductVariantExportJob implements ShouldQueue
{
    ...
    {
        ini_set('memory_limit', config('product_variants.export_job_memory_limit', '512M'));
    ...
    protected function buildQuery()
    {
        ...
        $columns = $this->options['columns'] ?? ['creator', 'product'];

        $base = ProductVariant::query()->withoutGlobalScopes();

        if ($this->tenantId !== null) {
            $base->where('product_variants.tenant_id', $this->tenantId);
        }
        ...
        if (in_array('product', $columns)) {
            $base->with('product:id,name,sku');
        }

        if ($scope === 'selected' && !empty($this->options['selected_ids'])) {
            return $base->whereIn('product_variants.id', $this->options['selected_ids']);
        }
        if ($scope === 'all') {
            return $base;
        }
    ...
    protected function buildFiltersSummary(): array
    {
        ...
        if (!empty($f['name'])) {
            ...
            $out[] = ['label' => __('product_variants.name'), 'value' => implode(', ', $names)];
        }
        if (!empty($f['sku'])) {
            $out[] = ['label' => __('product_variants.sku'), 'value' => (string) $f['sku']];
        }
        if (isset($f['is_active']) && $f['is_active'] !== '' && $f['is_active'] !== null) {
            $bool = filter_var($f['is_active'], FILTER_VALIDATE_BOOLEAN);
            $out[] = ['label' => __('product_variants.is_active'), 'value' => $bool ? __('global.active') : __('global.inactive')];
        }
    ...
    {
        $base = Str::slug($this->options['title'] ?? __('product_variants.export_filename'));
