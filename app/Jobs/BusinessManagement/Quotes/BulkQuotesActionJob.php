 ...
 * dispatch viven en QuoteService.
 */
class BulkQuotesActionJob implements ShouldQueue
{
    ...
    /**
     * Umbral configurable. Prioridad: Setting global -> config/quotes.php -> 200.
    ...
    public static function asyncThreshold(): int
    {
        return \App\Models\Setting::getInt(
            ...
            (int) config('quotes.bulk_async_threshold', 200),
        );
    }
    ...
    public function handle(QuoteService $service): void
    {
        ...
        if (!$user) {
            \Log::warning('BulkQuotesActionJob: user not found, aborting', [
                'user_id' => $this->userId,
                'action'  => $this->action,
            ]);
        ...
        foreach (array_chunk($this->ids, 200) as $chunk) {
            $quotes = match ($this->action) {
                'restore' => Quote::onlyTrashed()->whereIn('id', $chunk)->get(),
                default   => Quote::whereIn('id', $chunk)->get(),
            };

            foreach ($quotes as $quote) {
                try {
                    match ($this->action) {
                        'delete'     => $service->delete($quote, $this->payload['reason'] ?? 'Bulk delete'),
                        'set_active' => $this->setActive($service, $quote),
                        'restore'    => $service->restore($quote),
                        default      => throw new \InvalidArgumentException("Unknown action: {$this->action}"),
                    };
                    $processed++;
                } catch (\Throwable $e) {
                    $errors++;
                    \Log::warning("BulkQuotesActionJob: error on quote {$quote->id}", [
                        'error' => $e->getMessage(),
                    ]);
                }
            }
        ...
        \Log::info("BulkQuotesActionJob completed", [
            'user_id'   => $this->userId,
            'action'    => $this->action,
            'processed' => $processed,
            'errors'    => $errors,
    ...
    {
        \Log::error("BulkQuotesActionJob failed", [
            'user_id' => $this->userId,
            'action'  => $this->action,
            'total'   => count($this->ids),
    ...
    protected function notifyUser(string $status, ?string $error = null): void
    {
        try {
            ...
            \Log::warning('BulkQuotesActionJob: notify failed', ['error' => $e->getMessage()]);
        }
    }

    protected function setActive(QuoteService $service, Quote $quote): void
    {
        $target = (bool) ($this->payload['is_active'] ?? true);
        if ((bool) $quote->is_active === $target) return;
        $service->update($quote, ['is_active' => $target]);
    }
}
