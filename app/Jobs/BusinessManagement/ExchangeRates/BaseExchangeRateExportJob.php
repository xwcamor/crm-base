 ...
 * Clon de BaseDiscountExportJob adaptado a ExchangeRates.
 */
abstract class BaseExchangeRateExportJob implements ShouldQueue
{
    ...
    {
        ini_set('memory_limit', config('exchange_rates.export_job_memory_limit', '512M'));
    ...
    protected function buildQuery()
    {
        ...
        $base = ExchangeRate::query()->withoutGlobalScopes();

        if ($this->tenantId !== null) {
            $base->where('exchange_rates.tenant_id', $this->tenantId);
        }
        ...
        if ($scope === 'selected' && !empty($this->options['selected_ids'])) {
            return $base->whereIn('exchange_rates.id', $this->options['selected_ids']);
        }
        if ($scope === 'all') {
            return $base;
        }
    ...
    protected function buildFiltersSummary(): array
    {
        ...
        if (!empty($f['base_code'])) {
            $codes = is_array($f['base_code']) ? $f['base_code'] : [$f['base_code']];
            $out[] = ['label' => __('exchange_rates.base_code'), 'value' => implode(', ', $codes)];
        }
        if (!empty($f['quote_code'])) {
            $codes = is_array($f['quote_code']) ? $f['quote_code'] : [$f['quote_code']];
            $out[] = ['label' => __('exchange_rates.quote_code'), 'value' => implode(', ', $codes)];
        }
        if (!empty($f['source'])) {
            $out[] = ['label' => __('exchange_rates.source'), 'value' => (string) $f['source']];
        }
        if (isset($f['is_active']) && $f['is_active'] !== '' && $f['is_active'] !== null) {
            $bool = filter_var($f['is_active'], FILTER_VALIDATE_BOOLEAN);
            $out[] = ['label' => __('exchange_rates.is_active'), 'value' => $bool ? __('global.active') : __('global.inactive')];
        }
        if (!empty($f['valid_from']) || !empty($f['valid_to'])) {
            $out[] = ['label' => __('exchange_rates.valid_at'), 'value' => ($f['valid_from'] ?? '…') . ' → ' . ($f['valid_to'] ?? '…')];
        }
    ...
    {
        $base = Str::slug($this->options['title'] ?? __('exchange_rates.export_filename'));
