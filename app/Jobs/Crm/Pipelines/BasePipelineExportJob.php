 ...
 *
 * Clon de BaseRegionExportJob adaptado a Pipelines (per-tenant, con relacion
 ...
 */
abstract class BasePipelineExportJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        ini_set('memory_limit', config('pipelines.export_job_memory_limit', '512M'));
     ...
     *
     * Pipelines es per-tenant: aplicamos `withoutGlobalScopes()` y manualmente
    ...
    protected function buildQuery()
    {
        ...
        $base = Pipeline::query()->withoutGlobalScopes();
        ...
        if ($this->tenantId !== null) {
            $base->where('pipelines.tenant_id', $this->tenantId);
        }
...
if ($scope === 'selected' && !empty($this->options['selected_ids'])) {
            return $base->whereIn('pipelines.id', $this->options['selected_ids']);
        }
        if ($scope === 'all') {
            return $base;
        }
    ...
    protected function buildFiltersSummary(): array
    {
        ...
        if (!empty($f['name'])) {
            ...
            $out[] = ['label' => __('pipelines.name'), 'value' => implode(', ', $names)];
        }
if (isset($f['is_active']) && $f['is_active'] !== '' && $f['is_active'] !== null) {
            $bool = filter_var($f['is_active'], FILTER_VALIDATE_BOOLEAN);
            $out[] = ['label' => __('pipelines.is_active'), 'value' => $bool ? __('global.active') : __('global.inactive')];
        }
    ...
    {
        $base = Str::slug($this->options['title'] ?? __('pipelines.export_filename'));
