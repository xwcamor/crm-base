 ...
 *
 * Clon de BaseRegionExportJob adaptado a Contacts (per-tenant, con relacion
 ...
 */
abstract class BaseContactExportJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        ini_set('memory_limit', config('contacts.export_job_memory_limit', '512M'));
     ...
     *
     * Contacts es per-tenant: aplicamos `withoutGlobalScopes()` y manualmente
    ...
    protected function buildQuery()
    {
        ...
        $base = Contact::query()->withoutGlobalScopes();
        ...
        if ($this->tenantId !== null) {
            $base->where('contacts.tenant_id', $this->tenantId);
        }
...
if ($scope === 'selected' && !empty($this->options['selected_ids'])) {
            return $base->whereIn('contacts.id', $this->options['selected_ids']);
        }
        if ($scope === 'all') {
            return $base;
        }
    ...
    protected function buildFiltersSummary(): array
    {
        ...
        if (!empty($f['name'])) {
            ...
            $out[] = ['label' => __('contacts.name'), 'value' => implode(', ', $names)];
        }
if (isset($f['is_active']) && $f['is_active'] !== '' && $f['is_active'] !== null) {
            $bool = filter_var($f['is_active'], FILTER_VALIDATE_BOOLEAN);
            $out[] = ['label' => __('contacts.is_active'), 'value' => $bool ? __('global.active') : __('global.inactive')];
        }
    ...
    {
        $base = Str::slug($this->options['title'] ?? __('contacts.export_filename'));
