 ...
 */
class GenerateContactsCsvJob extends BaseContactExportJob
{
    ...
    protected function executeExport(Download $download): void
    {
        ...
        $tempFile = tempnam(sys_get_temp_dir(), 'contacts_csv') . '.csv';
        ...
        try {
            ...
            $headings = [
                'id'         => __('contacts.id'),
                'name'       => __('contacts.name'),
                'is_active'  => __('contacts.is_active'),
                'slug'       => 'Slug',
            ...
            $this->buildQuery()->chunkById(1000, function ($contacts) use ($handle, $columns, $tz) {
                foreach ($contacts as $contact) {
                    $row = array_map(fn ($col) => match ($col) {
                        ...
                        'slug'       => $contact->slug,
                        'created_at' => $contact->created_at?->copy()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT),
                        'updated_at' => $contact->updated_at?->copy()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT),
