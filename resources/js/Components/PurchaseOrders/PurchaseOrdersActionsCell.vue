...
<template>
    <div v-if="isMobile" class="row-actions-mobile" @click.stop>
        <Tooltip :title="t('global.view')">
            <Link :href="route('business_management.purchase_orders.show', record.slug)">
                <Button type="text" class="row-icon-btn" :aria-label="t('global.view')">
                    <EyeOutlined />
                </Button>
            </Link>
        </Tooltip>
        <Tooltip v-if="canEdit" :title="t('purchase_orders.edit_hint')">
        ...
        <Tooltip v-if="canDelete" :title="t('purchase_orders.delete_hint')">
            <Link :href="route('business_management.purchase_orders.delete', record.slug)">
                <Button
                    type="text" danger class="row-icon-btn"
    ...
    <Space v-else :size="4" class="row-actions-desktop" @click.stop>
        ...
        <Tooltip v-if="canEdit" :title="t('purchase_orders.edit_hint')">
            <Link :href="route('business_management.purchase_orders.edit', record.slug)">
