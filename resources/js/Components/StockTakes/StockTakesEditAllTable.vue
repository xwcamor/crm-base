<script setup>
/**
 * Tabla editable in-line del flujo Edit-All de StockTakes. Permite editar
 * reference + status. NO toca lineas/qty_counted (esos cambios deben ir via
 * Form.vue normal).
 */
...
<template>
    <table v-if="draft.length > 0" class="edit-table">
        <thead>
            <tr>
                ...
                <th class="col-ref">{{ t('stock_takes.reference') }}</th>
                <th class="col-status">{{ t('stock_takes.status') }}</th>
            </tr>
        </thead>
        <tbody>
            <tr
    ...
    <div v-else class="empty">
        {{ t('stock_takes.edit_all_no_results') }}
    </div>
</template>
...
.edit-table .col-ref     { width: 240px; }
