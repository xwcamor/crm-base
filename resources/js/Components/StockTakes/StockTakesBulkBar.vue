<script setup>
/**
 * Bulk bar de StockTakes. A diferencia de Customer (activate/deactivate),
 * StockTakes expone bulk update de status: el padre muestra un Select para
 * elegir el status objetivo via el evento `set-status`.
 */
import { ref } from 'vue';
        ...
        <Space wrap>
            ...
            <template v-if="canEdit">
                ...
                <Button size="small" :loading="bulkActivating" @click="applyStatus">
                    <CheckCircleOutlined /> {{ $t('stock_takes.bulk_set_status') }}
                </Button>
            </template>
            <Button
                v-if="canDelete"
