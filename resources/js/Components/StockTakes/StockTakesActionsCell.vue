...
<template>
    <div v-if="isMobile" class="row-actions-mobile" @click.stop>
        <Tooltip :title="t('global.view')">
            <Link :href="route('business_management.stock_takes.show', record.slug)">
                <Button type="text" class="row-icon-btn" :aria-label="t('global.view')">
                    <EyeOutlined />
                </Button>
            </Link>
        </Tooltip>
        <Tooltip v-if="canEdit" :title="t('stock_takes.edit_hint')">
        ...
        <Tooltip v-if="canDelete" :title="t('stock_takes.delete_hint')">
            <Link :href="route('business_management.stock_takes.delete', record.slug)">
    ...
    <Space v-else :size="4" class="row-actions-desktop" @click.stop>
        ...
        <Tooltip v-if="canEdit" :title="t('stock_takes.edit_hint')">
            <Link :href="route('business_management.stock_takes.edit', record.slug)">
