...
<template>
    <div v-if="isMobile" class="row-actions-mobile" @click.stop>
        <Tooltip :title="t('global.view')">
            <Link :href="route('business_management.price_lists.show', record.slug)">
                <Button type="text" class="row-icon-btn" :aria-label="t('global.view')">
                    <EyeOutlined />
                </Button>
            </Link>
        </Tooltip>
        <Tooltip v-if="canEdit" :title="t('price_lists.edit_hint')">
        ...
        <Tooltip v-if="canDelete" :title="t('price_lists.delete_hint')">
            <Link :href="route('business_management.price_lists.delete', record.slug)">
    ...
    <Space v-else :size="4" class="row-actions-desktop" @click.stop>
        ...
        <Tooltip v-if="canEdit" :title="t('price_lists.edit_hint')">
            <Link :href="route('business_management.price_lists.edit', record.slug)">
