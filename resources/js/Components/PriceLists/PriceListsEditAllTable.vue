<script setup>
/**
 * Tabla editable in-line del flujo Edit-All de PriceLists. Recibe `draft` por
...
<template>
    <table v-if="draft.length > 0" class="edit-table">
        <thead>
            <tr>
                ...
                <th class="col-name">{{ $t('price_lists.table_headers.editable_name') }}</th>
                <th class="col-status">{{ $t('price_lists.table_headers.editable_status') }}</th>
            </tr>
        </thead>
        <tbody>
            <tr
    ...
    <div v-else class="empty">
        {{ $t('price_lists.edit_all_no_results') }}
    </div>
</template>
