 ...
 *  1) Confirmacion del display sintetico (USD/PEN @ YYYY-MM-DD).
 *  2) Motivo obligatorio (min 10 chars).
 *  3) Solo super llega aqui (gated en backend + UI).
 */
...
const expectedDisplay = (target) => target?.display_name ?? '';

const okDisabled = (target) => !target
    || form.value.display_confirmation !== expectedDisplay(target)
...
<template>
    ...
    >
        ...
        <p class="force-msg">
            {{ $t('exchange_rates.force_delete_display_prompt', { display: expectedDisplay(target) }) }}
        </p>
        <Input
            v-model:value="form.display_confirmation"
            :placeholder="expectedDisplay(target)"
            :status="errors.display_confirmation ? 'error' : ''"
            size="large"
            class="mb-3"
        />
        <div v-if="errors.display_confirmation" class="field-error">
            {{ errors.display_confirmation }}
        </div>
