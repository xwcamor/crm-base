<script setup>
/**
 * Tabla editable in-line del flujo Edit-All. Recibe `draft` por v-model y
 * predicado isDirty del composable useEditAllDraft.
 */
import { InputNumber, Switch } from 'ant-design-vue';

const props = defineProps({
    isDirty:       { type: Function, required: true },
    duplicateRows: { type: Set,      required: true },
});

const draft = defineModel('draft', { type: Array, required: true });
</script>

<template>
    <table v-if="draft.length > 0" class="edit-table">
        <thead>
            <tr>
                <th class="col-id">ID</th>
                <th class="col-pair">{{ $t('exchange_rates.pair') }}</th>
                <th class="col-valid">{{ $t('exchange_rates.valid_at') }}</th>
                <th class="col-rate">{{ $t('exchange_rates.table_headers.editable_rate') }}</th>
                <th class="col-status">{{ $t('exchange_rates.table_headers.editable_status') }}</th>
            </tr>
        </thead>
        <tbody>
            <tr
                v-for="(row, i) in draft"
                :key="row.id"
                :class="{ 'is-dirty': props.isDirty(i) }"
            >
                <td class="col-id">{{ row.id }}</td>
                <td class="col-pair">
                    <code>{{ row.base_code }}/{{ row.quote_code }}</code>
                </td>
                <td class="col-valid">{{ row.valid_at }}</td>
                <td class="col-rate">
                    <InputNumber
                        v-model:value="row.rate"
                        :min="0"
                        :step="0.000001"
                        size="small"
                        style="width:160px"
                        :status="props.isDirty(i) ? 'warning' : ''"
                    />
                </td>
                <td class="col-status">
                    <Switch
                        v-model:checked="row.is_active"
                        :checked-children="$t('global.active')"
                        :un-checked-children="$t('global.inactive')"
                    />
                </td>
            </tr>
        </tbody>
    </table>

    <div v-else class="empty">
        {{ $t('exchange_rates.edit_all_no_results') }}
    </div>
</template>

<style scoped>
.edit-table {
    width: 100%;
    border-collapse: collapse;
    font-size: 0.875rem;
}
.edit-table thead th {
    background: var(--color-surface-alt);
    color: var(--color-text-strong);
    font-weight: 600;
    font-size: 0.78rem;
    text-transform: uppercase;
    letter-spacing: 0.04em;
    text-align: left;
    padding: 12px 14px;
    border-bottom: 1px solid var(--color-border);
}
.edit-table tbody td {
    padding: 8px 14px;
    border-bottom: 1px solid var(--color-border-soft);
    vertical-align: middle;
}
.edit-table tbody tr:last-child td { border-bottom: 0; }
.edit-table .col-id      { width: 80px;  color: var(--color-text-muted); }
.edit-table .col-pair    { width: 130px; font-family: ui-monospace, Consolas, monospace; }
.edit-table .col-valid   { width: 180px; color: var(--color-text-muted); font-size: 0.8125rem; }
.edit-table .col-rate    { width: 180px; }
.edit-table .col-status  { width: 160px; }
.edit-table tbody tr.is-dirty { background: var(--tint-dirty); }

.empty {
    padding: 48px 16px;
    text-align: center;
    color: var(--color-text-muted);
    font-size: 0.9rem;
}

@media (max-width: 768px) {
    .edit-table .col-id { display: none; }
    .edit-table thead th:first-child,
    .edit-table tbody td:first-child { display: none; }
}
</style>
