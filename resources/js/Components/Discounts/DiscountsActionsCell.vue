...
<template>
    <div v-if="isMobile" class="row-actions-mobile" @click.stop>
        <Tooltip :title="t('global.view')">
            <Link :href="route('business_management.discounts.show', record.slug)">
                <Button type="text" class="row-icon-btn" :aria-label="t('global.view')">
                    <EyeOutlined />
                </Button>
            </Link>
        </Tooltip>
        <Tooltip v-if="canEdit" :title="t('discounts.edit_hint')">
        ...
        <Tooltip v-if="canDelete" :title="t('discounts.delete_hint')">
            <Link :href="route('business_management.discounts.delete', record.slug)">
    ...
    <Space v-else :size="4" class="row-actions-desktop" @click.stop>
        ...
        <Tooltip v-if="canEdit" :title="t('discounts.edit_hint')">
            <Link :href="route('business_management.discounts.edit', record.slug)">
