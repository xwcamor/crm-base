<script setup>
/**
 * Tabla editable in-line del flujo Edit-All de Discounts. Recibe `draft` por
...
<template>
    <table v-if="draft.length > 0" class="edit-table">
        <thead>
            <tr>
                ...
                <th class="col-code">{{ $t('discounts.code') }}</th>
                <th class="col-name">{{ $t('discounts.table_headers.editable_name') }}</th>
                <th class="col-status">{{ $t('discounts.table_headers.editable_status') }}</th>
            </tr>
        </thead>
        <tbody>
            <tr
            ...
            >
                ...
                <td class="col-code"><code>{{ row.code }}</code></td>
                <td class="col-name">
                    <Input
    ...
    <div v-else class="empty">
        {{ $t('discounts.edit_all_no_results') }}
    </div>
</template>
...
.edit-table .col-code   { width: 160px; font-family: ui-monospace, Consolas, monospace; }
