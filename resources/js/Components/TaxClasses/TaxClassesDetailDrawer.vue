...
<template>
    <Drawer
        :open="open"
        :title="$t('tax_classes.singular') + ' — ' + $t('global.show')"
        :width="width"
        placement="right"
        @update:open="emit('update:open', $event)"
    >
        <template v-if="taxClass">
            <div class="drawer-hero">
                ...
                <div>
                    <h2>{{ taxClass.name }}</h2>
                    <Tag :color="taxClass.is_active ? 'success' : 'error'" :bordered="false">
...
<DescriptionsItem :label="$t('global.created_at')">
                ...
                <DescriptionsItem v-if="taxClass.creator" :label="$t('global.created_by')">
                    ...
                    <span class="audit-email">({{ taxClass.creator.email }})</span>
                </DescriptionsItem>
            </Descriptions>
        </template>

        <template #footer>
            <div v-if="taxClass" class="drawer-footer" :class="{ 'drawer-footer--mobile': isMobile }">
                <Link :href="route('business_management.tax_classes.show', taxClass.slug)">
                ...
                <Link
                    v-if="canDelete"
                    :href="route('business_management.tax_classes.delete', taxClass.slug)"
                >
                ...
                <Button
                    v-if="canCreate"
                    :block="isMobile"
                    :loading="duplicatingId === taxClass.id"
                    @click="emit('duplicate', taxClass)"
                >
                    <CopyOutlined /> {{ $t('global.duplicate') }}
                </Button>
                <Link
                    v-if="canEdit"
                    :href="route('business_management.tax_classes.edit', taxClass.slug)"
                >
