<script setup>
/**
 * Tabla editable in-line del flujo Edit-All de TaxClasses. Recibe `draft` por
    ...
    <table v-if="draft.length > 0" class="edit-table">
        <thead>
            <tr>
                ...
                <th class="col-name">{{ $t('tax_classes.table_headers.editable_name') }}</th>
                <th class="col-status">{{ $t('tax_classes.table_headers.editable_status') }}</th>
            </tr>
        </thead>
        <tbody>
            <tr
...
<td class="col-status">
    ...
    <div v-else class="empty">
        {{ $t('tax_classes.edit_all_no_results') }}
    </div>
</template>
