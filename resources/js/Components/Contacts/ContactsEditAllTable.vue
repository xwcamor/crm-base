<script setup>
/**
 * Tabla editable in-line del flujo Edit-All de Contacts. Recibe `draft` por
    ...
    <table v-if="draft.length > 0" class="edit-table">
        <thead>
            <tr>
                ...
                <th class="col-name">{{ $t('contacts.table_headers.editable_name') }}</th>
                <th class="col-status">{{ $t('contacts.table_headers.editable_status') }}</th>
            </tr>
        </thead>
        <tbody>
            <tr
...
<td class="col-status">
    ...
    <div v-else class="empty">
        {{ $t('contacts.edit_all_no_results') }}
    </div>
</template>
