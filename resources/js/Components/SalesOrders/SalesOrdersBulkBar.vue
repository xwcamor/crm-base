<script setup>
/**
 * Bulk bar de SalesOrders. A diferencia de Customer (activate/deactivate),
 * SalesOrders expone bulk update de status: el padre muestra un Select para
 * elegir el status objetivo via el evento `set-active` (que en este modulo
 * realmente significa "set status").
 */
import { ref } from 'vue';
        ...
        <Space wrap>
            ...
            <template v-if="canEdit">
                ...
                <Button size="small" :loading="bulkActivating" @click="applyStatus">
                    <CheckCircleOutlined /> {{ $t('sales_orders.bulk_set_status') }}
                </Button>
            </template>
            <Button
                v-if="canDelete"
...
.bulk-bar--mobile-sticky {
    ...
    border: 0; border-top: 1px solid var(--color-border-overlay);
    background: var(--color-surface-overlay); color: var(--color-text-on-dark);
    padding: 12px 16px calc(env(safe-area-inset-bottom, 0px) + 12px);
    box-shadow: 0 -8px 24px rgba(0, 0, 0, 0.18); flex-wrap: nowrap;
}
