...
<template>
    <div v-if="isMobile" class="row-actions-mobile" @click.stop>
        <Tooltip :title="t('global.view')">
            <Link :href="route('business_management.sales_orders.show', record.slug)">
                <Button type="text" class="row-icon-btn" :aria-label="t('global.view')">
                    <EyeOutlined />
                </Button>
            </Link>
        </Tooltip>
        <Tooltip v-if="canEdit" :title="t('sales_orders.edit_hint')">
        ...
        <Tooltip v-if="canDelete" :title="t('sales_orders.delete_hint')">
            <Link :href="route('business_management.sales_orders.delete', record.slug)">
    ...
    <Space v-else :size="4" class="row-actions-desktop" @click.stop>
        ...
        <Tooltip v-if="canEdit" :title="t('sales_orders.edit_hint')">
            <Link :href="route('business_management.sales_orders.edit', record.slug)">
