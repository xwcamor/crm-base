...
<template>
    <div v-if="isMobile" class="row-actions-mobile" @click.stop>
        <Tooltip :title="t('global.view')">
            <Link :href="route('business_management.deliveries.show', record.slug)">
                <Button type="text" class="row-icon-btn" :aria-label="t('global.view')">
                    <EyeOutlined />
                </Button>
            </Link>
        </Tooltip>
        <Tooltip v-if="canEdit" :title="t('deliveries.edit_hint')">
        ...
        <Tooltip v-if="canDelete" :title="t('deliveries.delete_hint')">
            <Link :href="route('business_management.deliveries.delete', record.slug)">
    ...
    <Space v-else :size="4" class="row-actions-desktop" @click.stop>
        ...
        <Tooltip v-if="canEdit" :title="t('deliveries.edit_hint')">
            <Link :href="route('business_management.deliveries.edit', record.slug)">
