...
<template>
    <table v-if="draft.length > 0" class="edit-table">
        <thead>
            <tr>
                ...
                <th class="col-name">{{ $t('product_variants.table_headers.editable_name') }}</th>
                <th class="col-sku">{{ $t('product_variants.table_headers.editable_sku') }}</th>
                <th class="col-price">{{ $t('product_variants.table_headers.editable_price') }}</th>
                <th class="col-status">{{ $t('product_variants.table_headers.editable_status') }}</th>
            </tr>
        </thead>
        <tbody>
            <tr
    ...
    <div v-else class="empty">
        {{ $t('product_variants.edit_all_no_results') }}
    </div>
</template>
...
.edit-table .col-sku    { width: 180px; }
.edit-table .col-price  { width: 140px; }
