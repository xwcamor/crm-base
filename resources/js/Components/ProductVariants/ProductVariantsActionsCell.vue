...
<template>
    <div v-if="isMobile" class="row-actions-mobile" @click.stop>
        <Tooltip :title="t('global.view')">
            <Link :href="route('business_management.product_variants.show', record.slug)">
                <Button type="text" class="row-icon-btn" :aria-label="t('global.view')">
                    <EyeOutlined />
                </Button>
            </Link>
        </Tooltip>
        <Tooltip v-if="canEdit" :title="t('product_variants.edit_hint')">
        ...
        <Tooltip v-if="canDelete" :title="t('product_variants.delete_hint')">
            <Link :href="route('business_management.product_variants.delete', record.slug)">
    ...
    <Space v-else :size="4" class="row-actions-desktop" @click.stop>
        ...
        <Tooltip v-if="canEdit" :title="t('product_variants.edit_hint')">
            <Link :href="route('business_management.product_variants.edit', record.slug)">
