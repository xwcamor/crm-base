<script setup>
/** Barra de acciones masivas para Messages (seleccion multiple). Mobile: sticky al pie. */
...
<template>
    <div class="bulk-bar" :class="{ 'bulk-bar--mobile-sticky': isMobile }">
        ...
        <Space wrap>
            ...
            <Button size="small" :loading="bulkActivating" @click="$emit('set-active', true)">
                <CheckCircleOutlined /> {{ $t('global.bulk_activate') }}
            </Button>
            <Button size="small" :loading="bulkActivating" @click="$emit('set-active', false)">
                <StopOutlined /> {{ $t('global.bulk_deactivate') }}
            </Button>
            <Button size="small" danger type="primary" @click="$emit('delete')">
