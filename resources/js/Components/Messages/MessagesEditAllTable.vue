<script setup>
/**
 * Tabla editable in-line del flujo Edit-All de Messages. Recibe `draft` por
...
<template>
    <table v-if="draft.length > 0" class="edit-table">
        <thead>
            <tr>
                ...
                <th class="col-subject">{{ $t('messages.subject') }}</th>
                <th class="col-status">{{ $t('messages.is_active') }}</th>
            </tr>
        </thead>
        <tbody>
            <tr
    ...
    <div v-else class="empty">
        {{ $t('messages.edit_all_no_results') }}
    </div>
</template>
...
.edit-table tbody td { padding: 8px 14px; border-bottom: 1px solid var(--color-border-soft); vertical-align: middle; }
