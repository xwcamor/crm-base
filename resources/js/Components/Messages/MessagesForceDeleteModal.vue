 ...
 *  1) Confirmacion de asunto (deben tipear el subject exacto).
...
const okDisabled = (target) => !target
    || form.value.subject_confirmation !== target.subject
...
<template>
    ...
    >
        ...
        <p class="force-msg">
            {{ $t('global.force_delete_name_prompt', { name: target?.subject }) }}
        </p>
        <Input
            v-model:value="form.subject_confirmation"
            :placeholder="target?.subject"
            :status="errors.subject_confirmation ? 'error' : ''"
            size="large"
            class="mb-3"
        />
        <div v-if="errors.subject_confirmation" class="field-error">
            {{ errors.subject_confirmation }}
        </div>
