<script setup>
/**
 * Tabla editable in-line del flujo Edit-All de Products. Recibe `draft` por
    ...
    <table v-if="draft.length > 0" class="edit-table">
        <thead>
            <tr>
                ...
                <th class="col-name">{{ $t('products.table_headers.editable_name') }}</th>
                <th class="col-status">{{ $t('products.table_headers.editable_status') }}</th>
            </tr>
        </thead>
        <tbody>
            <tr
...
<td class="col-status">
    ...
    <div v-else class="empty">
        {{ $t('products.edit_all_no_results') }}
    </div>
</template>
