<script setup>
/**
 * Tabla editable in-line del flujo Edit-All de Pipelines. Recibe `draft` por
    ...
    <table v-if="draft.length > 0" class="edit-table">
        <thead>
            <tr>
                ...
                <th class="col-name">{{ $t('pipelines.table_headers.editable_name') }}</th>
                <th class="col-status">{{ $t('pipelines.table_headers.editable_status') }}</th>
            </tr>
        </thead>
        <tbody>
            <tr
...
<td class="col-status">
    ...
    <div v-else class="empty">
        {{ $t('pipelines.edit_all_no_results') }}
    </div>
</template>
