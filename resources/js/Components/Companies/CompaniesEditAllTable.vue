    ...
    <table v-if="draft.length > 0" class="edit-table">
        <thead>
            <tr>
                ...
                <th class="col-name">{{ $t('companies.table_headers.editable_name') }}</th>
                <th class="col-status">{{ $t('companies.table_headers.editable_status') }}</th>
            </tr>
        </thead>
        <tbody>
            <tr
...
<td class="col-status">
    ...
    <div v-else class="empty">
        {{ $t('companies.edit_all_no_results') }}
    </div>
</template>
