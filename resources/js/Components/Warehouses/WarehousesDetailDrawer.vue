...
<template>
    <Drawer
        :open="open"
        :title="$t('warehouses.singular') + ' — ' + $t('global.show')"
        :width="width"
        placement="right"
...
<DescriptionsItem :label="$t('global.created_at')">
                ...
                <DescriptionsItem v-if="warehouse.creator" :label="$t('global.created_by')">
                    ...
                    <span class="audit-email">({{ warehouse.creator.email }})</span>
                </DescriptionsItem>
            </Descriptions>
        </template>

        <template #footer>
            <div v-if="warehouse" class="drawer-footer" :class="{ 'drawer-footer--mobile': isMobile }">
                <Link :href="route('business_management.warehouses.show', warehouse.slug)">
                ...
                <Link
                    v-if="canDelete"
                    :href="route('business_management.warehouses.delete', warehouse.slug)"
                >
                ...
                <Button
                    v-if="canCreate"
                    :block="isMobile"
                    :loading="duplicatingId === warehouse.id"
                    @click="emit('duplicate', warehouse)"
                >
                    <CopyOutlined /> {{ $t('global.duplicate') }}
                </Button>
                <Link
                    v-if="canEdit"
                    :href="route('business_management.warehouses.edit', warehouse.slug)"
                >
