 ...
 */
export const contactsFilterFields = (t) => [
    { key: 'name',           label: t('contacts.filter_name'), type: 'tags' },
{ key: 'is_active',      label: t('contacts.is_active'),   type: 'select', options: [
...
export const contactsEmptyFilters = () => ({
    name: [],
    is_active: null,
    created_at: null,
...
export const hydrateContactsFilters = (server) => ({
...
export const contactsFiltersToQuery = (f) => ({
...
export const contactsFiltersSummary = (f, t) => {
    const parts = [];
    if (f.name?.length)        parts.push(`${t('contacts.filter_name')}: ${f.name.join(', ')}`);
if (f.is_active !== null && f.is_active !== undefined) {
        parts.push(`${t('contacts.is_active')}: ${f.is_active ? t('global.active') : t('global.inactive')}`);
    }
