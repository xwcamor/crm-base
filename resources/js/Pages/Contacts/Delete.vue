...
const submit = () => {
    form.delete(route('crm.contacts.deleteSave', props.contact.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('contacts.singular')" />

    <div class="delete-page">
        <SectionHeader
            :back-href="route('crm.contacts.index')"
            :title="$t('global.delete') + ' ' + $t('contacts.record')"
            :subtitle="$t('contacts.delete_hint')"
        ...
        <Card class="delete-card" :bodyStyle="{ padding: '24px 28px' }">
            ...
            >
                <template #message>
                    {{ $t('contacts.delete_about', { name: contact.name }) }}
                </template>
            ...
            <div class="record-summary">
                <div class="record-summary__row">
                    ...
                    <span class="record-summary__value">{{ contact.id }}</span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('contacts.name') }}</span>
                    <span class="record-summary__value">{{ contact.name }}</span>
                </div>
<div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('contacts.is_active') }}</span>
