...
import ContactsTrashBulkBar from '@/Components/Contacts/ContactsTrashBulkBar.vue';
import ContactsForceDeleteModal from '@/Components/Contacts/ContactsForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { contactsTrashColumns } from './config/trashColumns';
...
const {
    ...
    restoreRouteName:     'crm.contacts.restore',
    bulkRestoreRouteName: 'crm.contacts.bulk_restore',
});
...
const openForceDelete = (contact) => {
    forceDeleteTarget.value = contact;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('crm.contacts.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => contactsTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.contacts.total === 1 ? t('global.record') : t('global.records');
    return `${props.contacts.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('contacts.plural')" />

    <div v-if="isSuper">
        <SectionHeader
            :back-href="route('crm.contacts.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('contacts.plural')"
            :subtitle="subtitle"
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('contacts.restore_hint')"
