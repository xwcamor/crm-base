...
import ContactsBulkBar from '@/Components/Contacts/ContactsBulkBar.vue';
import ContactsBulkDeleteModal from '@/Components/Contacts/ContactsBulkDeleteModal.vue';
import ContactsFavoriteCell from '@/Components/Contacts/ContactsFavoriteCell.vue';
import ContactsDetailDrawer from '@/Components/Contacts/ContactsDetailDrawer.vue';
import ContactsMobileBottomBar from '@/Components/Contacts/ContactsMobileBottomBar.vue';
import ContactsMobileDrawers from '@/Components/Contacts/ContactsMobileDrawers.vue';
import ContactsPageHeader from '@/Components/Contacts/ContactsPageHeader.vue';
import ContactsActionsCell from '@/Components/Contacts/ContactsActionsCell.vue';
import ContactsEmptyState from '@/Components/Contacts/ContactsEmptyState.vue';
...
// El toolbar inline de Contacts (no usa ModuleToolbar) repite manualmente
// estos gates para no mostrar botones que no funcionan al user free/basic.
const { canUse: canUsePlanFeature } = usePlanFeatures();

import {
    contactsFilterFields, contactsEmptyFilters, hydrateContactsFilters,
    contactsFiltersToQuery, contactsFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { contactsTableColumns } from './config/columns';
import { contactsExportableColumns, contactsExportEndpoints } from './config/exports';
import { contactsTourSteps } from './config/tour';
...
// Helper: resuelve dataIndex anidados tipo ['owner', 'name'].
...
const lifecycleColor = (stage) => ({
    lead: 'default', mql: 'cyan', sql: 'blue', opportunity: 'orange',
    customer: 'success', evangelist: 'purple', other: 'default',
}[stage] ?? 'default');

// ─── Filtros (schema + (de)serialización en config/filters.js) ──────────────
const filterFields = computed(() =>
    contactsFilterFields(t, {
    }),
);
...
const {
    ...
    hydrate:       hydrateContactsFilters,
    toQuery:       contactsFiltersToQuery,
    summary:       contactsFiltersSummary,
    empty:         contactsEmptyFilters,
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.contacts),
    hasActiveFilters,
    t,
});
...
const allColumns = computed(() =>
    contactsTableColumns(t, { isSuper: isSuper.value }),
);
...
useModuleUndoToast('crm.contacts.undo_last_delete');

// ─── Favoritos polimorficos ────────────────────────────────────────────────
const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('contacts', 'contacts');
...
const { open: drawerVisible, selected: selectedContact, openDetails } = useModuleDrawer({ module: 'contacts' });
...
const {
    ...
    bulkSetActiveRoute: 'crm.contacts.bulk_set_active',
    bulkDeleteRoute:    'crm.contacts.bulk_delete',
    resourceLabel:      t('contacts.records'),
});
...
const duplicate = (record) => {
    ...
    router.post(route('crm.contacts.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => contactsExportableColumns(t));
const exportEndpoints   = computed(() => contactsExportEndpoints());
...
const tour = useModuleTour({ module: 'contacts', steps: () => contactsTourSteps(t) });

// ─── Mobile: drawers + navegación (patrón Regions: app-like en mobile) ──────
const filtersDrawerOpen = ref(false);
const otrosDrawerOpen   = ref(false);
const goToCreate  = () => router.visit(route('crm.contacts.create'));
const goToTrash   = () => router.visit(route('crm.contacts.trash'));
const goToAudit   = () => router.visit(route('system_management.audit_logs.index', { module: 'contacts' }));
const goToEditAll = () => router.visit(route('crm.contacts.edit_all'));

// ─── Keyboard shortcuts ────────────────────────────────────────────────────
useKeyboardShortcuts({
    'ctrl+n': () => can('contacts.create') && router.visit(route('crm.contacts.create')),
    'esc': () => {
...
// ─── Acciones ───────────────────────────────────────────────────────────────
const goEdit   = (record) => router.visit(route('crm.contacts.edit',   record.slug));
const goDelete = (record) => router.visit(route('crm.contacts.delete', record.slug));
</script>

<template>
    <Head :title="$t('contacts.plural')" />

    <div>
        <div class="page-header">
            <ContactsPageHeader
                :title="$t('contacts.plural')"
            ...
            <Space wrap class="hide-on-mobile">
                ...
                <span data-tour="export-import">
                    ...
                    <Tooltip v-if="can('contacts.create') && canUsePlanFeature('imports')" :title="$t('global.import_hint')">
                        <Button style="margin-left: 8px;" @click="importOpen = true">
                            <UploadOutlined /> {{ $t('global.import') }}
                        </Button>
                    </Tooltip>
                </span>
                <Tooltip v-if="can('contacts.edit') && canUsePlanFeature('edit_all')" :title="$t('global.edit_all_hint')">
                    <Link :href="route('crm.contacts.edit_all')" data-tour="edit-all">
                        <Button>
                ...
                <Tooltip v-if="isSuper" :title="$t('global.view_deleted_hint')">
                    <Link :href="route('crm.contacts.trash')" data-tour="trash">
                        <Button>
                ...
                <Tooltip v-if="canSeeAudit" :title="$t('global.audit_hint')">
                    <Link :href="route('system_management.audit_logs.index', { module: 'contacts' })" data-tour="audit">
                        <Button>
                            <AuditOutlined /> {{ $t('sidebar.group_audit') }}
                        </Button>
                    </Link>
                </Tooltip>
                <Tooltip v-if="can('contacts.create')" :title="$t('global.create_record_hint')">
                    <Link :href="route('crm.contacts.create')">
                        <Button type="primary">
                            <PlusOutlined /> {{ $t('contacts.new') }}
                        </Button>
                    </Link>
                </Tooltip>
            </Space>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                ...
                :row-selection="(can('contacts.delete') || can('contacts.edit')) ? rowSelection : null"
                rowKey="id"
            ...
            >
                ...
                <template #bodyCell="{ column, record, text, isMobile }">
                    ...
                    <template v-else-if="column.key === 'email' || column.key === 'phone' || column.key === 'position' || column.key === 'owner'">
                        <span v-if="resolveNested(record, column.dataIndex)">{{ resolveNested(record, column.dataIndex) }}</span>
                        <span v-else class="muted">—</span>
                    </template>
...
<template v-else-if="column.key === 'tenant'">
        ...
        <ContactsBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('contacts.record') : $t('contacts.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('contacts.export_title')"
            :endpoints="exportEndpoints"
            :total-rows="contacts.total ?? 0"
            :total-unfiltered="contacts.total_unfiltered ?? contacts.total ?? 0"
        />

        <ImportDialog
            v-model:open="importOpen"
            :endpoint="route('crm.contacts.import')"
            :template-url="route('crm.contacts.import_template')"
            :resource-label="$t('contacts.records')"
        />

        <!-- ── Mobile: bottom bar fijo + drawers (patrón app real, como Regions) ── -->
        <ContactsMobileBottomBar
