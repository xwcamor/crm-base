...
import QuotesBulkBar from '@/Components/Quotes/QuotesBulkBar.vue';
import QuotesBulkDeleteModal from '@/Components/Quotes/QuotesBulkDeleteModal.vue';
import QuotesFavoriteCell from '@/Components/Quotes/QuotesFavoriteCell.vue';
import QuotesDetailDrawer from '@/Components/Quotes/QuotesDetailDrawer.vue';
import QuotesMobileBottomBar from '@/Components/Quotes/QuotesMobileBottomBar.vue';
import QuotesMobileDrawers from '@/Components/Quotes/QuotesMobileDrawers.vue';
import QuotesPageHeader from '@/Components/Quotes/QuotesPageHeader.vue';
import QuotesActionsCell from '@/Components/Quotes/QuotesActionsCell.vue';
import QuotesEmptyState from '@/Components/Quotes/QuotesEmptyState.vue';
...
import {
    quotesFilterFields, quotesEmptyFilters, hydrateQuotesFilters,
    quotesFiltersToQuery, quotesFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { quotesTableColumns } from './config/columns';
import { quotesExportableColumns, quotesExportEndpoints } from './config/exports';
import { quotesTourSteps } from './config/tour';
...
const {
    ...
    hydrate:       hydrateQuotesFilters,
    toQuery:       quotesFiltersToQuery,
    summary:       quotesFiltersSummary,
    empty:         quotesEmptyFilters,
    only:          ['quotes', 'filters'],
    t,
});
...
useModuleUndoToast('business_management.quotes.undo_last_delete');

const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('quotes', 'quotes');

const { isMobile: isMobileScreen } = useViewport(768);
const drawerWidth = computed(() => isMobileScreen.value ? '100%' : 480);
const { open: drawerVisible, selected: selectedQuote, openDetails } = useModuleDrawer({ module: 'quotes' });
...
const {
    ...
    bulkSetActiveRoute: 'business_management.quotes.bulk_set_active',
    bulkDeleteRoute:    'business_management.quotes.bulk_delete',
    resourceLabel:      t('quotes.records'),
});

const duplicating = ref(null);
const duplicate = (record) => {
    duplicating.value = record.id;
    router.post(route('business_management.quotes.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => quotesExportableColumns(t));
const exportEndpoints   = computed(() => quotesExportEndpoints());
...
const tour = useModuleTour({ module: 'quotes', steps: () => quotesTourSteps(t) });

const filtersDrawerOpen = ref(false);
const otrosDrawerOpen   = ref(false);
const goToCreate  = () => router.visit(route('business_management.quotes.create'));
const goToTrash   = () => router.visit(route('business_management.quotes.trash'));
const goToAudit   = () => router.visit(route('system_management.audit_logs.index', { module: 'quotes' }));
const goToEditAll = () => router.visit(route('business_management.quotes.edit_all'));

useKeyboardShortcuts({
    'ctrl+n': () => can('quotes.create') && router.visit(route('business_management.quotes.create')),
    'esc': () => {
...
const goEdit   = (record) => router.visit(route('business_management.quotes.edit',   record.slug));
const goDelete = (record) => router.visit(route('business_management.quotes.delete', record.slug));
...
<template>
    <Head :title="$t('quotes.plural')" />

    <div>
        <div class="page-header">
            <QuotesPageHeader
                :title="$t('quotes.plural')"
            ...
            <Space wrap class="hide-on-mobile">
                ...
                <span data-tour="export-import">
                    ...
                    <Tooltip v-if="can('quotes.create') && canUsePlanFeature('imports')" :title="$t('global.import_hint')">
                        <Button style="margin-left: 8px;" @click="importOpen = true">
                            <UploadOutlined /> {{ $t('global.import') }}
                        </Button>
                    </Tooltip>
                </span>
                <Tooltip v-if="can('quotes.edit') && canUsePlanFeature('edit_all')" :title="$t('global.edit_all_hint')">
                    <Link :href="route('business_management.quotes.edit_all')" data-tour="edit-all">
                        <Button>
                ...
                <Tooltip v-if="isSuper" :title="$t('global.view_deleted_hint')">
                    <Link :href="route('business_management.quotes.trash')" data-tour="trash">
                        <Button>
                ...
                <Tooltip v-if="canSeeAudit" :title="$t('global.audit_hint')">
                    <Link :href="route('system_management.audit_logs.index', { module: 'quotes' })" data-tour="audit">
                        <Button>
                            <AuditOutlined /> {{ $t('sidebar.group_audit') }}
                        </Button>
                    </Link>
                </Tooltip>
                <Tooltip v-if="can('quotes.create')" :title="$t('global.create_record_hint')">
                    <Link :href="route('business_management.quotes.create')">
                        <Button type="primary">
                            <PlusOutlined /> {{ $t('quotes.new') }}
                        </Button>
                    </Link>
                </Tooltip>
            </Space>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            <div v-auto-animate>
                <QuotesBulkBar
                    ...
                    :can-edit="can('quotes.edit')"
                    :can-delete="can('quotes.delete')"
            ...
            <ResponsiveTable
                ...
                :row-selection="(can('quotes.delete') || can('quotes.edit')) ? rowSelection : null"
                rowKey="id"
            ...
            >
                ...
                <template #bodyCell="{ column, record, text, isMobile }">
                    <QuotesFavoriteCell
                        ...
                        :data-tour="record === quotes.data[0] ? 'favorites' : null"
                    ...
                    <template v-else-if="column.key === 'contact'">
                        <Link v-if="record.contact" :href="route('crm.contacts.show', record.contact.slug ?? record.contact.id)">{{ record.contact.name }}</Link>
                        <span v-else class="muted">—</span>
                    </template>

                    <template v-else-if="column.key === 'deal'">
                        <Link v-if="record.deal" :href="route('crm.deals.show', record.deal.slug ?? record.deal.id)">
                            <Tag :bordered="false">{{ record.deal.name }}</Tag>
                        </Link>
                        <span v-else class="muted">—</span>
                    </template>
                    ...
                    <template v-else-if="column.key === 'status'">
                        <Tag :color="statusColor(record.status)" :bordered="false">
                            {{ $t('quotes.status_options.' + record.status) }}
                        </Tag>
                    </template>

                    <template v-else-if="column.key === 'issue_date'">
                        <span v-if="record.issue_date">{{ formatDate(record.issue_date) }}</span>
                        <span v-else class="muted">—</span>
                    </template>

                    <template v-else-if="column.key === 'valid_until'">
                        <span v-if="record.valid_until">{{ formatDate(record.valid_until) }}</span>
                        <span v-else class="muted">—</span>
                    </template>
        ...
        <QuotesBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('quotes.record') : $t('quotes.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('quotes.export_title')"
            :endpoints="exportEndpoints"
            :total-rows="quotes.total ?? 0"
            :total-unfiltered="quotes.total_unfiltered ?? quotes.total ?? 0"
        />

        <ImportDialog
            v-model:open="importOpen"
            :endpoint="route('business_management.quotes.import')"
            :template-url="route('business_management.quotes.import_template')"
            :resource-label="$t('quotes.records')"
        />

        <QuotesMobileBottomBar
