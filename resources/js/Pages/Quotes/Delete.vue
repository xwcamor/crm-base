...
const submit = () => {
    form.delete(route('business_management.quotes.deleteSave', props.quote.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('quotes.singular')" />

    <div class="delete-page">
        <SectionHeader
            :back-href="route('business_management.quotes.index')"
            :title="$t('global.delete') + ' ' + $t('quotes.record')"
            :subtitle="$t('quotes.delete_hint')"
        ...
        <Card class="delete-card" :bodyStyle="{ padding: '24px 28px' }">
            ...
            >
                <template #message>
                    {{ $t('quotes.delete_about', { name: quote.reference ?? '#' + quote.id }) }}
                </template>
            ...
            <div class="record-summary">
                <div class="record-summary__row">
                    ...
                    <span class="record-summary__value">{{ quote.id }}</span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('quotes.reference') }}</span>
                    <span class="record-summary__value">{{ quote.reference ?? '#' + quote.id }}</span>
                </div>
<div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('quotes.is_active') }}</span>
                    <span class="record-summary__value">
                        <Tag :color="quote.is_active ? 'success' : 'error'" :bordered="false">
                            {{ quote.is_active ? $t('global.active') : $t('global.inactive') }}
                        </Tag>
                    </span>
                </div>
            </div>
            ...
            <Form layout="vertical" @submit.prevent="submit">
                ...
                <DeleteFooter
                    :cancel-href="route('business_management.quotes.index')"
