...
import QuotesEditAllTable from '@/Components/Quotes/QuotesEditAllTable.vue';
...
/**
 * Edit-All de Quotes. Mantenemos snapshot `original` + `draft` mutable
...
const source = computed(() => props.quotes.data);
...
const saveAll = () => {
    ...
    router.post(
        route('business_management.quotes.edit_all.update'),
...
<template>
    <Head :title="$t('quotes.edit_all_title')" />

    <div class="edit-all">
        <SectionHeader
            :back-href="route('business_management.quotes.index')"
            :title="$t('global.edit_all') + ' — ' + $t('quotes.plural')"
            :subtitle="$t('quotes.edit_all_subtitle')"
        >
            <template #icon><EditOutlined /></template>
            <template #actions>
                <Button :disabled="dirtyCount === 0 || submitting" @click="discardAll">
                    <UndoOutlined /> {{ $t('quotes.edit_all_discard') }}
                </Button>
                <Button
                    type="primary"
                    :loading="submitting"
                    ...
                    @click="saveAll"
                >
                    <SaveOutlined /> {{ $t('quotes.edit_all_save_all') }}
                </Button>
            </template>
        </SectionHeader>
        ...
        <Alert v-if="dirtyCount > 0" type="info" show-icon class="status-bar">
            <template #message>
                {{ $t('quotes.edit_all_changes', { count: dirtyCount }) }}
            </template>
        </Alert>
        ...
        <div v-if="quotes.total > quotes.per_page" class="pagination">
            <Pagination
                :current="quotes.current_page"
                :pageSize="quotes.per_page"
                :total="quotes.total"
