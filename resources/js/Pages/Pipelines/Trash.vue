...
import PipelinesTrashBulkBar from '@/Components/Pipelines/PipelinesTrashBulkBar.vue';
import PipelinesForceDeleteModal from '@/Components/Pipelines/PipelinesForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { pipelinesTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('crm.pipelines.index'));
}
...
const {
    ...
    restoreRouteName:     'crm.pipelines.restore',
    bulkRestoreRouteName: 'crm.pipelines.bulk_restore',
});
...
const openForceDelete = (pipeline) => {
    forceDeleteTarget.value = pipeline;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('crm.pipelines.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => pipelinesTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.pipelines.total === 1 ? t('global.record') : t('global.records');
    return `${props.pipelines.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('pipelines.plural')" />

    <div v-if="isSuper">
        <SectionHeader
            :back-href="route('crm.pipelines.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('pipelines.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('pipelines.restore_hint')"
            ...
            <Empty
                v-if="pipelines.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <PipelinesForceDeleteModal
