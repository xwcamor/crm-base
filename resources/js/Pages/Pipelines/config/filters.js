 ...
 */
export const pipelinesFilterFields = (t) => [
    { key: 'name',           label: t('pipelines.filter_name'), type: 'tags' },
{ key: 'is_active',      label: t('pipelines.is_active'),   type: 'select', options: [
...
export const pipelinesEmptyFilters = () => ({
    name: [],
    is_active: null,
    created_at: null,
...
export const hydratePipelinesFilters = (server) => ({
...
export const pipelinesFiltersToQuery = (f) => ({
...
export const pipelinesFiltersSummary = (f, t) => {
    const parts = [];
    if (f.name?.length)        parts.push(`${t('pipelines.filter_name')}: ${f.name.join(', ')}`);
if (f.is_active !== null && f.is_active !== undefined) {
        parts.push(`${t('pipelines.is_active')}: ${f.is_active ? t('global.active') : t('global.inactive')}`);
    }
