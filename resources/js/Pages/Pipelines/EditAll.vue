...
import PipelinesEditAllTable from '@/Components/Pipelines/PipelinesEditAllTable.vue';
...
/**
 * Edit-All de Pipelines. Mantenemos snapshot `original` + `draft` mutable
...
const source = computed(() => props.pipelines.data);
...
const saveAll = () => {
    ...
    router.post(
        route('crm.pipelines.edit_all.update'),
...
<template>
    <Head :title="$t('pipelines.edit_all_title')" />

    <div class="edit-all">
        <SectionHeader
            :back-href="route('crm.pipelines.index')"
            :title="$t('global.edit_all') + ' — ' + $t('pipelines.plural')"
            :subtitle="$t('pipelines.edit_all_subtitle')"
        >
            <template #icon><EditOutlined /></template>
            <template #actions>
                <Button :disabled="dirtyCount === 0 || submitting" @click="discardAll">
                    <UndoOutlined /> {{ $t('pipelines.edit_all_discard') }}
                </Button>
                <Button
                    type="primary"
                    :loading="submitting"
                    ...
                    @click="saveAll"
                >
                    <SaveOutlined /> {{ $t('pipelines.edit_all_save_all') }}
                </Button>
            </template>
        </SectionHeader>
        ...
        <Alert v-if="dirtyCount > 0" type="info" show-icon class="status-bar">
            <template #message>
                {{ $t('pipelines.edit_all_changes', { count: dirtyCount }) }}
            </template>
        </Alert>
        ...
        <Alert
            ...
            type="error"
            show-icon
            :message="$t('pipelines.name_unique')"
            class="status-bar"
        />

        <Card :bodyStyle="{ padding: 0 }" class="edit-table-card">
            <PipelinesEditAllTable
                v-model:draft="draft"
                :is-dirty="isDirty"
        ...
        <div v-if="pipelines.total > pipelines.per_page" class="pagination">
            <Pagination
                :current="pipelines.current_page"
                :pageSize="pipelines.per_page"
