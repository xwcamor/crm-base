...
const props = defineProps({
    ...
    openDealsCount: { type: Number, default: 0 },
});

const hasDependents = computed(() => Object.keys(props.dependents).length > 0);
const blocked = computed(() => props.openDealsCount > 0);
...
const submit = () => {
    ...
    form.delete(route('crm.pipelines.deleteSave', props.pipeline.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('pipelines.singular')" />

    <div class="delete-page">
        <SectionHeader
            :back-href="route('crm.pipelines.index')"
            :title="$t('global.delete') + ' ' + $t('pipelines.record')"
            :subtitle="$t('pipelines.delete_hint')"
        ...
        <Card class="delete-card" :bodyStyle="{ padding: '24px 28px' }">
            ...
            >
                <template #message>
                    {{ $t('pipelines.delete_about', { name: pipeline.name }) }}
                </template>
            ...
            >
                <template #message>
                    {{ $t('pipelines.has_open_deals_title') }}
                </template>
                <template #description>
                    {{ $t('pipelines.has_open_deals', { count: openDealsCount }) }}
                </template>
            </Alert>
            ...
            <div class="record-summary">
                <div class="record-summary__row">
                    ...
                    <span class="record-summary__value">{{ pipeline.id }}</span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('pipelines.name') }}</span>
                    <span class="record-summary__value">{{ pipeline.name }}</span>
                </div>
<div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('pipelines.is_active') }}</span>
                    <span class="record-summary__value">
                        <Tag :color="pipeline.is_active ? 'success' : 'error'" :bordered="false">
