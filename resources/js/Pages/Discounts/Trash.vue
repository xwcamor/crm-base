...
import DiscountsTrashBulkBar from '@/Components/Discounts/DiscountsTrashBulkBar.vue';
import DiscountsForceDeleteModal from '@/Components/Discounts/DiscountsForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { discountsTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.discounts.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.discounts.restore',
    bulkRestoreRouteName: 'business_management.discounts.bulk_restore',
});
...
const openForceDelete = (discount) => {
    forceDeleteTarget.value = discount;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.discounts.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => discountsTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.discounts.total === 1 ? t('global.record') : t('global.records');
    return `${props.discounts.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('discounts.plural')" />

    <div v-if="isSuper">
        <SectionHeader
            :back-href="route('business_management.discounts.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('discounts.plural')"
            :subtitle="subtitle"
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('discounts.restore_hint')"
