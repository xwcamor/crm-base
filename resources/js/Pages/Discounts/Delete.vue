...
const submit = () => {
    form.delete(route('business_management.discounts.deleteSave', props.discount.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('discounts.singular')" />

    <div class="delete-page">
        <SectionHeader
            :back-href="route('business_management.discounts.index')"
            :title="$t('global.delete') + ' ' + $t('discounts.record')"
            :subtitle="$t('discounts.delete_hint')"
        ...
        <Card class="delete-card" :bodyStyle="{ padding: '24px 28px' }">
            <Alert type="warning" show-icon class="mb-4">
                <template #message>
                    {{ $t('discounts.delete_about', { name: discount.name }) }}
                </template>
            ...
            <div class="record-summary">
                <div class="record-summary__row">
                    ...
                    <span class="record-summary__value">{{ discount.id }}</span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('discounts.code') }}</span>
                    <span class="record-summary__value"><code>{{ discount.code }}</code></span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('discounts.name') }}</span>
                    <span class="record-summary__value">{{ discount.name }}</span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('discounts.is_active') }}</span>
