...
import {
    ...
    PercentageOutlined, DollarOutlined, CarOutlined,
...
import DiscountsBulkBar from '@/Components/Discounts/DiscountsBulkBar.vue';
import DiscountsBulkDeleteModal from '@/Components/Discounts/DiscountsBulkDeleteModal.vue';
import DiscountsFavoriteCell from '@/Components/Discounts/DiscountsFavoriteCell.vue';
import DiscountsDetailDrawer from '@/Components/Discounts/DiscountsDetailDrawer.vue';
import DiscountsMobileBottomBar from '@/Components/Discounts/DiscountsMobileBottomBar.vue';
import DiscountsMobileDrawers from '@/Components/Discounts/DiscountsMobileDrawers.vue';
import DiscountsPageHeader from '@/Components/Discounts/DiscountsPageHeader.vue';
import DiscountsActionsCell from '@/Components/Discounts/DiscountsActionsCell.vue';
import DiscountsEmptyState from '@/Components/Discounts/DiscountsEmptyState.vue';
...
import {
    discountsFilterFields, discountsEmptyFilters, hydrateDiscountsFilters,
    discountsFiltersToQuery, discountsFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { discountsTableColumns } from './config/columns';
import { discountsExportableColumns, discountsExportEndpoints } from './config/exports';
import { discountsTourSteps } from './config/tour';
...
const filterFields = computed(() =>
    discountsFilterFields(t, { typeOptions: props.typeOptions }),
);
...
const {
    ...
    hydrate:       hydrateDiscountsFilters,
    toQuery:       discountsFiltersToQuery,
    summary:       discountsFiltersSummary,
    empty:         discountsEmptyFilters,
    only:          ['discounts', 'filters'],
    t,
});
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.discounts),
    hasActiveFilters,
    t,
});

const allColumns = computed(() =>
    discountsTableColumns(t, { isSuper: isSuper.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

const tablePagination = computed(() => ({
    current:  props.discounts.current_page,
    pageSize: props.discounts.per_page,
    total:    props.discounts.total,
...
useModuleUndoToast('business_management.discounts.undo_last_delete');

const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('discounts', 'discounts');

const { isMobile: isMobileScreen } = useViewport(768);
const drawerWidth = computed(() => isMobileScreen.value ? '100%' : 480);
const { open: drawerVisible, selected: selectedDiscount, openDetails } = useModuleDrawer({ module: 'discounts' });
...
const {
    ...
    bulkSetActiveRoute: 'business_management.discounts.bulk_set_active',
    bulkDeleteRoute:    'business_management.discounts.bulk_delete',
    resourceLabel:      t('discounts.records'),
});

const duplicating = ref(null);
const duplicate = (record) => {
    duplicating.value = record.id;
    router.post(route('business_management.discounts.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => discountsExportableColumns(t));
const exportEndpoints   = computed(() => discountsExportEndpoints());

const typeColorMap = { percentage: 'blue', fixed_amount: 'green', free_shipping: 'gold' };

const formatValue = (record) => {
    if (record.type === 'percentage') return `${record.value}%`;
    if (record.type === 'fixed_amount') return `${record.currency_code || ''} ${Number(record.value).toFixed(2)}`;
    return '—';
};
...
const tour = useModuleTour({ module: 'discounts', steps: () => discountsTourSteps(t) });

const filtersDrawerOpen = ref(false);
const otrosDrawerOpen   = ref(false);
const goToCreate  = () => router.visit(route('business_management.discounts.create'));
const goToTrash   = () => router.visit(route('business_management.discounts.trash'));
const goToAudit   = () => router.visit(route('system_management.audit_logs.index', { module: 'discounts' }));
const goToEditAll = () => router.visit(route('business_management.discounts.edit_all'));

useKeyboardShortcuts({
    'ctrl+n': () => can('discounts.create') && router.visit(route('business_management.discounts.create')),
    'esc': () => {
...
const goEdit   = (record) => router.visit(route('business_management.discounts.edit',   record.slug));
const goDelete = (record) => router.visit(route('business_management.discounts.delete', record.slug));
</script>

<template>
    <Head :title="$t('discounts.plural')" />
    ...
    <div>
        <div class="page-header">
            ...
            <Space wrap class="hide-on-mobile">
                ...
                <span data-tour="export-import">
                    ...
                    <Tooltip v-if="can('discounts.create') && canUsePlanFeature('imports')" :title="$t('global.import_hint')">
                        <Button style="margin-left: 8px;" @click="importOpen = true">
                            <UploadOutlined /> {{ $t('global.import') }}
                        </Button>
                    </Tooltip>
                </span>
                <Tooltip v-if="can('discounts.edit') && canUsePlanFeature('edit_all')" :title="$t('global.edit_all_hint')">
                    <Link :href="route('business_management.discounts.edit_all')" data-tour="edit-all">
                        <Button>
                ...
                <Tooltip v-if="isSuper" :title="$t('global.view_deleted_hint')">
                    <Link :href="route('business_management.discounts.trash')" data-tour="trash">
                        <Button>
                ...
                <Tooltip v-if="canSeeAudit" :title="$t('global.audit_hint')">
                    <Link :href="route('system_management.audit_logs.index', { module: 'discounts' })" data-tour="audit">
                        <Button>
                            <AuditOutlined /> {{ $t('sidebar.group_audit') }}
                        </Button>
                    </Link>
                </Tooltip>
                <Tooltip v-if="can('discounts.create')" :title="$t('global.create_record_hint')">
                    <Link :href="route('business_management.discounts.create')">
                        <Button type="primary">
                            <PlusOutlined /> {{ $t('discounts.new') }}
                        </Button>
                    </Link>
                </Tooltip>
            </Space>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                ...
                :row-selection="(can('discounts.delete') || can('discounts.edit')) ? rowSelection : null"
                rowKey="id"
                ...
                <template #bodyCell="{ column, record, text, isMobile }">
                    ...
                    <template v-else-if="column.key === 'type'">
                        <Tag :color="typeColorMap[record.type] || 'default'" :bordered="false">
                            <component :is="typeIconMap[record.type] || PercentageOutlined" style="margin-right: 4px" />
                            {{ $t('discounts.type_options.' + record.type) }}
                        </Tag>
                    </template>
                    ...
                    <template v-else-if="column.key === 'valid_until'">
                        {{ record.valid_until ? formatDate(record.valid_until) : '—' }}
                    </template>
        ...
        <DiscountsBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('discounts.record') : $t('discounts.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('discounts.export_title')"
            :endpoints="exportEndpoints"
            :total-rows="discounts.total ?? 0"
            :total-unfiltered="discounts.total_unfiltered ?? discounts.total ?? 0"
        />

        <ImportDialog
            v-model:open="importOpen"
            :endpoint="route('business_management.discounts.import')"
            :template-url="route('business_management.discounts.import_template')"
            :resource-label="$t('discounts.records')"
        />

        <DiscountsMobileBottomBar
