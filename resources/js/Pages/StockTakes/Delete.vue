...
const submit = () => {
    form.delete(route('business_management.stock_takes.deleteSave', props.take.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('stock_takes.singular')" />

    <div class="delete-page">
        <SectionHeader
            :back-href="route('business_management.stock_takes.index')"
            :title="$t('global.delete') + ' ' + $t('stock_takes.record')"
            :subtitle="$t('stock_takes.delete_hint')"
        ...
        <Card class="delete-card" :bodyStyle="{ padding: '24px 28px' }">
            <Alert type="warning" show-icon class="mb-4">
                <template #message>
                    {{ $t('stock_takes.delete_about', { name: take.reference }) }}
                </template>
            ...
            <div class="record-summary">
                <div class="record-summary__row">
                    ...
                    <span class="record-summary__value">{{ take.id }}</span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('stock_takes.reference') }}</span>
                    <span class="record-summary__value"><strong>{{ take.reference }}</strong></span>
                </div>
                <div v-if="take.warehouse" class="record-summary__row">
                    <span class="record-summary__label">{{ $t('stock_takes.warehouse') }}</span>
                    <span class="record-summary__value">{{ take.warehouse.name }} ({{ take.warehouse.code }})</span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('stock_takes.status') }}</span>
            ...
            <Form layout="vertical" @submit.prevent="submit">
                ...
                <DeleteFooter
                    :cancel-href="route('business_management.stock_takes.index')"
