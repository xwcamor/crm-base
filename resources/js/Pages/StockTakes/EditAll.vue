...
import StockTakesEditAllTable from '@/Components/StockTakes/StockTakesEditAllTable.vue';
...
const saveAll = () => {
    ...
    router.post(
        route('business_management.stock_takes.edit_all.update'),
...
<template>
    <Head :title="$t('stock_takes.edit_all_title')" />

    <div class="edit-all">
        <SectionHeader
            :back-href="route('business_management.stock_takes.index')"
            :title="$t('global.edit_all') + ' — ' + $t('stock_takes.plural')"
            :subtitle="$t('stock_takes.edit_all_subtitle')"
        >
            <template #icon><EditOutlined /></template>
            <template #actions>
                <Button :disabled="dirtyCount === 0 || submitting" @click="discardAll">
                    <UndoOutlined /> {{ $t('stock_takes.edit_all_discard') }}
                </Button>
                <Button
                    type="primary"
                    :loading="submitting"
                    ...
                    @click="saveAll"
                >
                    <SaveOutlined /> {{ $t('stock_takes.edit_all_save_all') }}
                </Button>
            </template>
        </SectionHeader>
        ...
        <Alert v-if="dirtyCount > 0" type="info" show-icon class="status-bar">
            <template #message>
                {{ $t('stock_takes.edit_all_changes', { count: dirtyCount }) }}
            </template>
        </Alert>
        ...
        <Alert
            ...
            type="error"
            show-icon
            :message="$t('stock_takes.reference_unique')"
            class="status-bar"
        />

        <Card :bodyStyle="{ padding: 0 }" class="edit-table-card">
            <StockTakesEditAllTable
                v-model:draft="draft"
                :is-dirty="isDirty"
        ...
        <div v-if="takes.total > takes.per_page" class="pagination">
            <Pagination
                :current="takes.current_page"
                :pageSize="takes.per_page"
                :total="takes.total"
