...
import StockTakesBulkBar from '@/Components/StockTakes/StockTakesBulkBar.vue';
import StockTakesBulkDeleteModal from '@/Components/StockTakes/StockTakesBulkDeleteModal.vue';
import StockTakesFavoriteCell from '@/Components/StockTakes/StockTakesFavoriteCell.vue';
import StockTakesDetailDrawer from '@/Components/StockTakes/StockTakesDetailDrawer.vue';
import StockTakesMobileBottomBar from '@/Components/StockTakes/StockTakesMobileBottomBar.vue';
import StockTakesMobileDrawers from '@/Components/StockTakes/StockTakesMobileDrawers.vue';
import StockTakesPageHeader from '@/Components/StockTakes/StockTakesPageHeader.vue';
import StockTakesActionsCell from '@/Components/StockTakes/StockTakesActionsCell.vue';
import StockTakesEmptyState from '@/Components/StockTakes/StockTakesEmptyState.vue';
...
import {
    stockTakesFilterFields, stockTakesEmptyFilters, hydrateStockTakesFilters,
    stockTakesFiltersToQuery, stockTakesFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { stockTakesTableColumns } from './config/columns';
import { stockTakesExportableColumns, stockTakesExportEndpoints } from './config/exports';
import { stockTakesTourSteps } from './config/tour';
...
const props = defineProps({
    takes:            { type: Object, required: true },
...
const {
    ...
    hydrate:       hydrateStockTakesFilters,
    toQuery:       stockTakesFiltersToQuery,
    summary:       stockTakesFiltersSummary,
    empty:         stockTakesEmptyFilters,
    only:          ['takes', 'filters'],
    t,
});
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.takes),
    hasActiveFilters,
    t,
});

const allColumns = computed(() =>
    stockTakesTableColumns(t, { isSuper: isSuper.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

const tablePagination = computed(() => ({
    current:  props.takes.current_page,
    pageSize: props.takes.per_page,
    total:    props.takes.total,
...
useModuleUndoToast('business_management.stock_takes.undo_last_delete');

const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('stock_takes', 'stock_takes');

const { isMobile: isMobileScreen } = useViewport(768);
const drawerWidth = computed(() => isMobileScreen.value ? '100%' : 480);
const { open: drawerVisible, selected: selectedTake, openDetails } = useModuleDrawer({ module: 'stock_takes' });
...
const {
    ...
    bulkSetActiveRoute: 'business_management.stock_takes.bulk_set_active',
    bulkDeleteRoute:    'business_management.stock_takes.bulk_delete',
    resourceLabel:      t('stock_takes.records'),
});

// Bulk set status: el endpoint stock_takes.bulk_set_active recibe status (enum)
// en lugar de is_active (boolean). Llamamos directamente al endpoint.
const bulkSetStatus = (status) => {
    if (selectedRowKeys.value.length === 0) return;
    router.post(
        route('business_management.stock_takes.bulk_set_active'),
...
const duplicate = (record) => {
    ...
    router.post(route('business_management.stock_takes.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => stockTakesExportableColumns(t));
const exportEndpoints   = computed(() => stockTakesExportEndpoints());
...
const tour = useModuleTour({ module: 'stock_takes', steps: () => stockTakesTourSteps(t) });

const filtersDrawerOpen = ref(false);
const otrosDrawerOpen   = ref(false);
const goToCreate  = () => router.visit(route('business_management.stock_takes.create'));
const goToTrash   = () => router.visit(route('business_management.stock_takes.trash'));
const goToAudit   = () => router.visit(route('system_management.audit_logs.index', { module: 'stock_takes' }));
const goToEditAll = () => router.visit(route('business_management.stock_takes.edit_all'));

useKeyboardShortcuts({
    'ctrl+n': () => can('stock_takes.create') && router.visit(route('business_management.stock_takes.create')),
    'esc': () => {
...
const goEdit   = (record) => router.visit(route('business_management.stock_takes.edit',   record.slug));
const goDelete = (record) => router.visit(route('business_management.stock_takes.delete', record.slug));
...
<template>
    <Head :title="$t('stock_takes.plural')" />

    <div>
        <div class="page-header">
            <StockTakesPageHeader
                :title="$t('stock_takes.plural')"
            ...
            <Space wrap class="hide-on-mobile">
                ...
                <span data-tour="export-import">
                    ...
                    <Tooltip v-if="can('stock_takes.create') && canUsePlanFeature('imports')" :title="$t('global.import_hint')">
                        <Button style="margin-left: 8px;" @click="importOpen = true">
                            <UploadOutlined /> {{ $t('global.import') }}
                        </Button>
                    </Tooltip>
                </span>
                <Tooltip v-if="can('stock_takes.edit') && canUsePlanFeature('edit_all')" :title="$t('global.edit_all_hint')">
                    <Link :href="route('business_management.stock_takes.edit_all')" data-tour="edit-all">
                        <Button>
                ...
                <Tooltip v-if="isSuper" :title="$t('global.view_deleted_hint')">
                    <Link :href="route('business_management.stock_takes.trash')" data-tour="trash">
                        <Button>
                ...
                <Tooltip v-if="canSeeAudit" :title="$t('global.audit_hint')">
                    <Link :href="route('system_management.audit_logs.index', { module: 'stock_takes' })" data-tour="audit">
                        <Button>
                            <AuditOutlined /> {{ $t('sidebar.group_audit') }}
                        </Button>
                    </Link>
                </Tooltip>
                <Tooltip v-if="can('stock_takes.create')" :title="$t('global.create_record_hint')">
                    <Link :href="route('business_management.stock_takes.create')">
                        <Button type="primary">
                            <PlusOutlined /> {{ $t('stock_takes.new') }}
                        </Button>
                    </Link>
                </Tooltip>
            </Space>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                :dataSource="takes.data"
                :columns="columns"
                :pagination="tablePagination"
                :row-selection="(can('stock_takes.delete') || can('stock_takes.edit')) ? rowSelection : null"
                rowKey="id"
                ...
                <template #bodyCell="{ column, record, text, isMobile }">
                    ...
                    <template v-else-if="column.key === 'status'">
                        <Tag :color="statusColor(record.status)" :bordered="false">
                            {{ $t('stock_takes.status_options.' + record.status) }}
                        </Tag>
                    </template>
                    ...
                    <template v-else-if="column.key === 'started_at'">
                        <span v-if="record.started_at">{{ formatDateTime(record.started_at) }}</span>
                        <span v-else class="muted">—</span>
                    </template>

                    <template v-else-if="column.key === 'completed_at'">
                        <span v-if="record.completed_at">{{ formatDateTime(record.completed_at) }}</span>
                        <span v-else class="muted">—</span>
                    </template>
        ...
        <StockTakesBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('stock_takes.record') : $t('stock_takes.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('stock_takes.export_title')"
            :endpoints="exportEndpoints"
            :total-rows="takes.total ?? 0"
            :total-unfiltered="takes.total_unfiltered ?? takes.total ?? 0"
        />

        <ImportDialog
            v-model:open="importOpen"
            :endpoint="route('business_management.stock_takes.import')"
            :template-url="route('business_management.stock_takes.import_template')"
            :resource-label="$t('stock_takes.records')"
        />

        <StockTakesMobileBottomBar
