...
import StockTakesTrashBulkBar from '@/Components/StockTakes/StockTakesTrashBulkBar.vue';
import StockTakesForceDeleteModal from '@/Components/StockTakes/StockTakesForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { stockTakesTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.stock_takes.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.stock_takes.restore',
    bulkRestoreRouteName: 'business_management.stock_takes.bulk_restore',
});
...
const openForceDelete = (take) => {
    forceDeleteTarget.value = take;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.stock_takes.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => stockTakesTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.takes.total === 1 ? t('global.record') : t('global.records');
    return `${props.takes.total} ${word} . ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('stock_takes.plural')" />

    <div v-if="isSuper">
        <SectionHeader
            :back-href="route('business_management.stock_takes.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('stock_takes.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('stock_takes.restore_hint')"
            ...
            <Empty
                v-if="takes.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <StockTakesForceDeleteModal
