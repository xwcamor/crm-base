...
import LeadSourcesEditAllTable from '@/Components/LeadSources/LeadSourcesEditAllTable.vue';
...
const source = computed(() => props.leadSources.data);
...
const saveAll = () => {
    ...
    router.post(
        route('business_management.lead_sources.edit_all.update'),
...
<template>
    <Head :title="$t('lead_sources.edit_all_title')" />

    <div class="edit-all">
        <SectionHeader
            :back-href="route('business_management.lead_sources.index')"
            :title="$t('global.edit_all') + ' — ' + $t('lead_sources.plural')"
            :subtitle="$t('lead_sources.edit_all_subtitle')"
        >
            <template #icon><EditOutlined /></template>
            <template #actions>
                <Button :disabled="dirtyCount === 0 || submitting" @click="discardAll">
                    <UndoOutlined /> {{ $t('lead_sources.edit_all_discard') }}
                </Button>
                <Button
                    type="primary"
                    :loading="submitting"
                    ...
                    @click="saveAll"
                >
                    <SaveOutlined /> {{ $t('lead_sources.edit_all_save_all') }}
                </Button>
            </template>
        </SectionHeader>
        ...
        <Alert v-if="dirtyCount > 0" type="info" show-icon class="status-bar">
            <template #message>
                {{ $t('lead_sources.edit_all_changes', { count: dirtyCount }) }}
            </template>
        </Alert>
        ...
        <Alert
            ...
            type="error"
            show-icon
            :message="$t('lead_sources.name_unique')"
            class="status-bar"
        />

        <Card :bodyStyle="{ padding: 0 }" class="edit-table-card">
            <LeadSourcesEditAllTable
                v-model:draft="draft"
                :is-dirty="isDirty"
        ...
        <div v-if="leadSources.total > leadSources.per_page" class="pagination">
            <Pagination
                :current="leadSources.current_page"
                :pageSize="leadSources.per_page"
