...
const submit = () => {
    form.delete(route('business_management.lead_sources.deleteSave', props.leadSource.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('lead_sources.singular')" />

    <div class="delete-page">
        <SectionHeader
            :back-href="route('business_management.lead_sources.index')"
            :title="$t('global.delete') + ' ' + $t('lead_sources.record')"
            :subtitle="$t('lead_sources.delete_hint')"
        ...
        <Card class="delete-card" :bodyStyle="{ padding: '24px 28px' }">
            <Alert type="warning" show-icon class="mb-4">
                <template #message>
                    {{ $t('lead_sources.delete_about', { name: leadSource.name }) }}
                </template>
            ...
            <div class="record-summary">
                <div class="record-summary__row">
                    ...
                    <span class="record-summary__value">{{ leadSource.id }}</span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('lead_sources.name') }}</span>
                    <span class="record-summary__value">{{ leadSource.name }}</span>
                </div>
                <div v-if="leadSource.category" class="record-summary__row">
                    <span class="record-summary__label">{{ $t('lead_sources.category') }}</span>
                    <span class="record-summary__value">{{ leadSource.category }}</span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('lead_sources.is_active') }}</span>
