...
import LeadSourcesTrashBulkBar from '@/Components/LeadSources/LeadSourcesTrashBulkBar.vue';
import LeadSourcesForceDeleteModal from '@/Components/LeadSources/LeadSourcesForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { leadSourcesTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.lead_sources.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.lead_sources.restore',
    bulkRestoreRouteName: 'business_management.lead_sources.bulk_restore',
});
...
const openForceDelete = (leadSource) => {
    forceDeleteTarget.value = leadSource;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.lead_sources.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => leadSourcesTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.leadSources.total === 1 ? t('global.record') : t('global.records');
    return `${props.leadSources.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('lead_sources.plural')" />

    <div v-if="isSuper">
        <SectionHeader
            :back-href="route('business_management.lead_sources.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('lead_sources.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('lead_sources.restore_hint')"
            ...
            <Empty
                v-if="leadSources.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <LeadSourcesForceDeleteModal
