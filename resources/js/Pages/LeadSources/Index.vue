...
import {
    ...
    AuditOutlined, FilterOutlined, CloseCircleFilled, CompassOutlined,
...
import LeadSourcesBulkBar from '@/Components/LeadSources/LeadSourcesBulkBar.vue';
import LeadSourcesBulkDeleteModal from '@/Components/LeadSources/LeadSourcesBulkDeleteModal.vue';
import LeadSourcesFavoriteCell from '@/Components/LeadSources/LeadSourcesFavoriteCell.vue';
import LeadSourcesDetailDrawer from '@/Components/LeadSources/LeadSourcesDetailDrawer.vue';
import LeadSourcesMobileBottomBar from '@/Components/LeadSources/LeadSourcesMobileBottomBar.vue';
import LeadSourcesMobileDrawers from '@/Components/LeadSources/LeadSourcesMobileDrawers.vue';
import LeadSourcesPageHeader from '@/Components/LeadSources/LeadSourcesPageHeader.vue';
import LeadSourcesActionsCell from '@/Components/LeadSources/LeadSourcesActionsCell.vue';
import LeadSourcesEmptyState from '@/Components/LeadSources/LeadSourcesEmptyState.vue';
...
import {
    leadSourcesFilterFields, leadSourcesEmptyFilters, hydrateLeadSourcesFilters,
    leadSourcesFiltersToQuery, leadSourcesFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { leadSourcesTableColumns } from './config/columns';
import { leadSourcesExportableColumns, leadSourcesExportEndpoints } from './config/exports';
import { leadSourcesTourSteps } from './config/tour';
...
const props = defineProps({
    leadSources:   { type: Object, required: true },
    filters:       { type: Object, default: () => ({}) },
    filterSchema:  { type: Array,  default: () => [] },
});

const filterFields = computed(() => leadSourcesFilterFields(t));

const {
    filters, reload, hasActiveFilters, clearFilters, filtersSummary, buildQueryData,
} = useModuleFilters({
    serverFilters: props.filters,
    hydrate:       hydrateLeadSourcesFilters,
    toQuery:       leadSourcesFiltersToQuery,
    summary:       leadSourcesFiltersSummary,
    empty:         leadSourcesEmptyFilters,
    only:          ['leadSources', 'filters'],
    t,
});
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.leadSources),
    hasActiveFilters,
    t,
});

const allColumns = computed(() =>
    leadSourcesTableColumns(t, { isSuper: isSuper.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

const tablePagination = computed(() => ({
    current:  props.leadSources.current_page,
    pageSize: props.leadSources.per_page,
    total:    props.leadSources.total,
...
useModuleUndoToast('business_management.lead_sources.undo_last_delete');

const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('lead_sources', 'lead_sources');

const { isMobile: isMobileScreen } = useViewport(768);
const drawerWidth = computed(() => isMobileScreen.value ? '100%' : 480);
const { open: drawerVisible, selected: selectedLeadSource, openDetails } = useModuleDrawer({ module: 'lead_sources' });
...
const {
    ...
    bulkSetActiveRoute: 'business_management.lead_sources.bulk_set_active',
    bulkDeleteRoute:    'business_management.lead_sources.bulk_delete',
    resourceLabel:      t('lead_sources.records'),
});

const duplicating = ref(null);
const duplicate = (record) => {
    duplicating.value = record.id;
    router.post(route('business_management.lead_sources.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => leadSourcesExportableColumns(t));
const exportEndpoints   = computed(() => leadSourcesExportEndpoints());
...
const tour = useModuleTour({ module: 'lead_sources', steps: () => leadSourcesTourSteps(t) });

const filtersDrawerOpen = ref(false);
const otrosDrawerOpen   = ref(false);
const goToCreate  = () => router.visit(route('business_management.lead_sources.create'));
const goToTrash   = () => router.visit(route('business_management.lead_sources.trash'));
const goToAudit   = () => router.visit(route('system_management.audit_logs.index', { module: 'lead_sources' }));
const goToEditAll = () => router.visit(route('business_management.lead_sources.edit_all'));

useKeyboardShortcuts({
    'ctrl+n': () => can('lead_sources.create') && router.visit(route('business_management.lead_sources.create')),
    'esc': () => {
...
const goEdit   = (record) => router.visit(route('business_management.lead_sources.edit',   record.slug));
const goDelete = (record) => router.visit(route('business_management.lead_sources.delete', record.slug));
</script>

<template>
    <Head :title="$t('lead_sources.plural')" />

    <div>
        <div class="page-header">
            <LeadSourcesPageHeader
                :title="$t('lead_sources.plural')"
            ...
            <Space wrap class="hide-on-mobile">
                ...
                <span data-tour="export-import">
                    ...
                    <Tooltip v-if="can('lead_sources.create') && canUsePlanFeature('imports')" :title="$t('global.import_hint')">
                        <Button style="margin-left: 8px;" @click="importOpen = true">
                            <UploadOutlined /> {{ $t('global.import') }}
                        </Button>
                    </Tooltip>
                </span>
                <Tooltip v-if="can('lead_sources.edit') && canUsePlanFeature('edit_all')" :title="$t('global.edit_all_hint')">
                    <Link :href="route('business_management.lead_sources.edit_all')" data-tour="edit-all">
                        <Button>
                ...
                <Tooltip v-if="isSuper" :title="$t('global.view_deleted_hint')">
                    <Link :href="route('business_management.lead_sources.trash')" data-tour="trash">
                        <Button>
                ...
                <Tooltip v-if="canSeeAudit" :title="$t('global.audit_hint')">
                    <Link :href="route('system_management.audit_logs.index', { module: 'lead_sources' })" data-tour="audit">
                        <Button>
                            <AuditOutlined /> {{ $t('sidebar.group_audit') }}
                        </Button>
                    </Link>
                </Tooltip>
                <Tooltip v-if="can('lead_sources.create')" :title="$t('global.create_record_hint')">
                    <Link :href="route('business_management.lead_sources.create')">
                        <Button type="primary">
                            <PlusOutlined /> {{ $t('lead_sources.new') }}
                        </Button>
                    </Link>
                </Tooltip>
            </Space>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                :dataSource="leadSources.data"
                :columns="columns"
                :pagination="tablePagination"
                :row-selection="(can('lead_sources.delete') || can('lead_sources.edit')) ? rowSelection : null"
                rowKey="id"
        ...
        <LeadSourcesBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('lead_sources.record') : $t('lead_sources.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('lead_sources.export_title')"
            :endpoints="exportEndpoints"
            :total-rows="leadSources.total ?? 0"
            :total-unfiltered="leadSources.total_unfiltered ?? leadSources.total ?? 0"
        />

        <ImportDialog
            v-model:open="importOpen"
            :endpoint="route('business_management.lead_sources.import')"
            :template-url="route('business_management.lead_sources.import_template')"
            :resource-label="$t('lead_sources.records')"
        />

        <LeadSourcesMobileBottomBar
