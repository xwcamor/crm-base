...
import PaymentMethodsTrashBulkBar from '@/Components/PaymentMethods/PaymentMethodsTrashBulkBar.vue';
import PaymentMethodsForceDeleteModal from '@/Components/PaymentMethods/PaymentMethodsForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { paymentMethodsTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.payment_methods.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.payment_methods.restore',
    bulkRestoreRouteName: 'business_management.payment_methods.bulk_restore',
});
...
const openForceDelete = (method) => {
    forceDeleteTarget.value = method;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.payment_methods.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => paymentMethodsTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.methods.total === 1 ? t('global.record') : t('global.records');
    return `${props.methods.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('payment_methods.plural')" />

    <div v-if="isSuper">
        <SectionHeader
            :back-href="route('business_management.payment_methods.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('payment_methods.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('payment_methods.restore_hint')"
            ...
            <Empty
                v-if="methods.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <PaymentMethodsForceDeleteModal
