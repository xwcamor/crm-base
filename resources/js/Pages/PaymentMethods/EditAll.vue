...
import PaymentMethodsEditAllTable from '@/Components/PaymentMethods/PaymentMethodsEditAllTable.vue';
...
const source = computed(() => props.methods.data);
...
const saveAll = () => {
    ...
    router.post(
        route('business_management.payment_methods.edit_all.update'),
...
<template>
    <Head :title="$t('payment_methods.edit_all_title')" />

    <div class="edit-all">
        <SectionHeader
            :back-href="route('business_management.payment_methods.index')"
            :title="$t('global.edit_all') + ' — ' + $t('payment_methods.plural')"
            :subtitle="$t('payment_methods.edit_all_subtitle')"
        >
            <template #icon><EditOutlined /></template>
            <template #actions>
                <Button :disabled="dirtyCount === 0 || submitting" @click="discardAll">
                    <UndoOutlined /> {{ $t('payment_methods.edit_all_discard') }}
                </Button>
                <Button
                    type="primary"
                    :loading="submitting"
        ...
        <Alert v-if="dirtyCount > 0" type="info" show-icon class="status-bar">
            <template #message>
                {{ $t('payment_methods.edit_all_changes', { count: dirtyCount }) }}
            </template>
        </Alert>
        ...
        <Alert
            ...
            type="error"
            show-icon
            :message="$t('payment_methods.name_unique')"
            class="status-bar"
        />

        <Card :bodyStyle="{ padding: 0 }" class="edit-table-card">
            <PaymentMethodsEditAllTable
                v-model:draft="draft"
                :is-dirty="isDirty"
        ...
        <div v-if="methods.total > methods.per_page" class="pagination">
            <Pagination
                :current="methods.current_page"
                :pageSize="methods.per_page"
                :total="methods.total"
