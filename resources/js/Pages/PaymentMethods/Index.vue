...
import PaymentMethodsBulkBar from '@/Components/PaymentMethods/PaymentMethodsBulkBar.vue';
import PaymentMethodsBulkDeleteModal from '@/Components/PaymentMethods/PaymentMethodsBulkDeleteModal.vue';
import PaymentMethodsFavoriteCell from '@/Components/PaymentMethods/PaymentMethodsFavoriteCell.vue';
import PaymentMethodsDetailDrawer from '@/Components/PaymentMethods/PaymentMethodsDetailDrawer.vue';
import PaymentMethodsMobileBottomBar from '@/Components/PaymentMethods/PaymentMethodsMobileBottomBar.vue';
import PaymentMethodsMobileDrawers from '@/Components/PaymentMethods/PaymentMethodsMobileDrawers.vue';
import PaymentMethodsPageHeader from '@/Components/PaymentMethods/PaymentMethodsPageHeader.vue';
import PaymentMethodsActionsCell from '@/Components/PaymentMethods/PaymentMethodsActionsCell.vue';
import PaymentMethodsEmptyState from '@/Components/PaymentMethods/PaymentMethodsEmptyState.vue';
...
import {
    paymentMethodsFilterFields, paymentMethodsEmptyFilters, hydratePaymentMethodsFilters,
    paymentMethodsFiltersToQuery, paymentMethodsFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { paymentMethodsTableColumns } from './config/columns';
import { paymentMethodsExportableColumns, paymentMethodsExportEndpoints } from './config/exports';
import { paymentMethodsTourSteps } from './config/tour';
...
const filterFields = computed(() => paymentMethodsFilterFields(t));

const {
    filters, reload, hasActiveFilters, clearFilters, filtersSummary, buildQueryData,
} = useModuleFilters({
    serverFilters: props.filters,
    hydrate:       hydratePaymentMethodsFilters,
    toQuery:       paymentMethodsFiltersToQuery,
    summary:       paymentMethodsFiltersSummary,
    empty:         paymentMethodsEmptyFilters,
    only:          ['methods', 'filters'],
    t,
});
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.methods),
    hasActiveFilters,
    t,
});

const allColumns = computed(() =>
    paymentMethodsTableColumns(t, { isSuper: isSuper.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

const tablePagination = computed(() => ({
    current:  props.methods.current_page,
    pageSize: props.methods.per_page,
    total:    props.methods.total,
...
useModuleUndoToast('business_management.payment_methods.undo_last_delete');

const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('payment_methods', 'payment_methods');

const { isMobile: isMobileScreen } = useViewport(768);
const drawerWidth = computed(() => isMobileScreen.value ? '100%' : 480);
const { open: drawerVisible, selected: selectedMethod, openDetails } = useModuleDrawer({ module: 'payment_methods' });
...
const {
    ...
    bulkSetActiveRoute: 'business_management.payment_methods.bulk_set_active',
    bulkDeleteRoute:    'business_management.payment_methods.bulk_delete',
    resourceLabel:      t('payment_methods.records'),
});

const duplicating = ref(null);
const duplicate = (record) => {
    duplicating.value = record.id;
    router.post(route('business_management.payment_methods.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => paymentMethodsExportableColumns(t));
const exportEndpoints   = computed(() => paymentMethodsExportEndpoints());
...
const tour = useModuleTour({ module: 'payment_methods', steps: () => paymentMethodsTourSteps(t) });

const filtersDrawerOpen = ref(false);
const otrosDrawerOpen   = ref(false);
const goToCreate  = () => router.visit(route('business_management.payment_methods.create'));
const goToTrash   = () => router.visit(route('business_management.payment_methods.trash'));
const goToAudit   = () => router.visit(route('system_management.audit_logs.index', { module: 'payment_methods' }));
const goToEditAll = () => router.visit(route('business_management.payment_methods.edit_all'));

useKeyboardShortcuts({
    'ctrl+n': () => can('payment_methods.create') && router.visit(route('business_management.payment_methods.create')),
    'esc': () => {
...
const goEdit   = (record) => router.visit(route('business_management.payment_methods.edit',   record.slug));
const goDelete = (record) => router.visit(route('business_management.payment_methods.delete', record.slug));
</script>

<template>
    <Head :title="$t('payment_methods.plural')" />

    <div>
        <div class="page-header">
            <PaymentMethodsPageHeader
                :title="$t('payment_methods.plural')"
            ...
            <Space wrap class="hide-on-mobile">
                ...
                <span data-tour="export-import">
                    ...
                    <Tooltip v-if="can('payment_methods.create') && canUsePlanFeature('imports')" :title="$t('global.import_hint')">
                        <Button style="margin-left: 8px;" @click="importOpen = true">
                            <UploadOutlined /> {{ $t('global.import') }}
                        </Button>
                    </Tooltip>
                </span>
                <Tooltip v-if="can('payment_methods.edit') && canUsePlanFeature('edit_all')" :title="$t('global.edit_all_hint')">
                    <Link :href="route('business_management.payment_methods.edit_all')" data-tour="edit-all">
                        <Button>
                ...
                <Tooltip v-if="isSuper" :title="$t('global.view_deleted_hint')">
                    <Link :href="route('business_management.payment_methods.trash')" data-tour="trash">
                        <Button>
                ...
                <Tooltip v-if="canSeeAudit" :title="$t('global.audit_hint')">
                    <Link :href="route('system_management.audit_logs.index', { module: 'payment_methods' })" data-tour="audit">
                        <Button>
                            <AuditOutlined /> {{ $t('sidebar.group_audit') }}
                        </Button>
                    </Link>
                </Tooltip>
                <Tooltip v-if="can('payment_methods.create')" :title="$t('global.create_record_hint')">
                    <Link :href="route('business_management.payment_methods.create')">
                        <Button type="primary">
                            <PlusOutlined /> {{ $t('payment_methods.new') }}
                        </Button>
                    </Link>
                </Tooltip>
            </Space>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                ...
                :row-selection="(can('payment_methods.delete') || can('payment_methods.edit')) ? rowSelection : null"
                rowKey="id"
            ...
            >
                ...
                <template #bodyCell="{ column, record, text, isMobile }">
                    ...
                    <template v-else-if="column.key === 'requires_reference'">
                        <Tag :color="record.requires_reference ? 'blue' : 'default'" :bordered="false">
                            {{ record.requires_reference ? $t('global.yes') : $t('global.no') }}
                        </Tag>
                    </template>
        ...
        <PaymentMethodsBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('payment_methods.record') : $t('payment_methods.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('payment_methods.export_title')"
            :endpoints="exportEndpoints"
            :total-rows="methods.total ?? 0"
            :total-unfiltered="methods.total_unfiltered ?? methods.total ?? 0"
        />

        <ImportDialog
            v-model:open="importOpen"
            :endpoint="route('business_management.payment_methods.import')"
            :template-url="route('business_management.payment_methods.import_template')"
            :resource-label="$t('payment_methods.records')"
        />

        <PaymentMethodsMobileBottomBar
