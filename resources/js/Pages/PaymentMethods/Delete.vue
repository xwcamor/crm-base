...
const submit = () => {
    form.delete(route('business_management.payment_methods.deleteSave', props.method.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('payment_methods.singular')" />

    <div class="delete-page">
        <SectionHeader
            :back-href="route('business_management.payment_methods.index')"
            :title="$t('global.delete') + ' ' + $t('payment_methods.record')"
            :subtitle="$t('payment_methods.delete_hint')"
        ...
        <Card class="delete-card" :bodyStyle="{ padding: '24px 28px' }">
            <Alert type="warning" show-icon class="mb-4">
                <template #message>
                    {{ $t('payment_methods.delete_about', { name: method.name }) }}
                </template>
            ...
            <div class="record-summary">
                <div class="record-summary__row">
                    ...
                    <span class="record-summary__value">{{ method.id }}</span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('payment_methods.name') }}</span>
                    <span class="record-summary__value">{{ method.name }}</span>
                </div>
                <div v-if="method.code" class="record-summary__row">
                    <span class="record-summary__label">{{ $t('payment_methods.code') }}</span>
                    <span class="record-summary__value">{{ method.code }}</span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('payment_methods.is_active') }}</span>
