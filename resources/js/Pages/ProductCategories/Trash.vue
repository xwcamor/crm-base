...
import ProductCategoriesTrashBulkBar from '@/Components/ProductCategories/ProductCategoriesTrashBulkBar.vue';
import ProductCategoriesForceDeleteModal from '@/Components/ProductCategories/ProductCategoriesForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { productCategoriesTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.product_categories.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.product_categories.restore',
    bulkRestoreRouteName: 'business_management.product_categories.bulk_restore',
});
...
const openForceDelete = (category) => {
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.product_categories.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => productCategoriesTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.categories.total === 1 ? t('global.record') : t('global.records');
    return `${props.categories.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('product_categories.plural')" />

    <div v-if="isSuper">
        <SectionHeader
            :back-href="route('business_management.product_categories.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('product_categories.plural')"
            :subtitle="subtitle"
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('product_categories.restore_hint')"
        ...
        <ProductCategoriesForceDeleteModal
