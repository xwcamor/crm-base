...
import {
    ...
    AuditOutlined, FilterOutlined, CloseCircleFilled, TagsOutlined,
...
import ProductCategoriesBulkBar from '@/Components/ProductCategories/ProductCategoriesBulkBar.vue';
import ProductCategoriesBulkDeleteModal from '@/Components/ProductCategories/ProductCategoriesBulkDeleteModal.vue';
import ProductCategoriesFavoriteCell from '@/Components/ProductCategories/ProductCategoriesFavoriteCell.vue';
import ProductCategoriesDetailDrawer from '@/Components/ProductCategories/ProductCategoriesDetailDrawer.vue';
import ProductCategoriesMobileBottomBar from '@/Components/ProductCategories/ProductCategoriesMobileBottomBar.vue';
import ProductCategoriesMobileDrawers from '@/Components/ProductCategories/ProductCategoriesMobileDrawers.vue';
import ProductCategoriesPageHeader from '@/Components/ProductCategories/ProductCategoriesPageHeader.vue';
import ProductCategoriesActionsCell from '@/Components/ProductCategories/ProductCategoriesActionsCell.vue';
import ProductCategoriesEmptyState from '@/Components/ProductCategories/ProductCategoriesEmptyState.vue';
...
import {
    productCategoriesFilterFields, productCategoriesEmptyFilters, hydrateProductCategoriesFilters,
    productCategoriesFiltersToQuery, productCategoriesFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { productCategoriesTableColumns } from './config/columns';
import { productCategoriesExportableColumns, productCategoriesExportEndpoints } from './config/exports';
import { productCategoriesTourSteps } from './config/tour';
...
const filterFields = computed(() =>
    productCategoriesFilterFields(t, { parentOptions: props.parentOptions }),
);
...
const {
    ...
    hydrate:       hydrateProductCategoriesFilters,
    toQuery:       productCategoriesFiltersToQuery,
    summary:       productCategoriesFiltersSummary,
    empty:         productCategoriesEmptyFilters,
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.categories),
    hasActiveFilters,
    t,
});

const allColumns = computed(() =>
    productCategoriesTableColumns(t, { isSuper: isSuper.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

const tablePagination = computed(() => ({
    current:  props.categories.current_page,
    pageSize: props.categories.per_page,
...
useModuleUndoToast('business_management.product_categories.undo_last_delete');

const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('product_categories', 'product_categories');

const { isMobile: isMobileScreen } = useViewport(768);
const drawerWidth = computed(() => isMobileScreen.value ? '100%' : 480);
const { open: drawerVisible, selected: selectedCategory, openDetails } = useModuleDrawer({ module: 'product_categories' });
...
const {
    ...
    bulkSetActiveRoute: 'business_management.product_categories.bulk_set_active',
    bulkDeleteRoute:    'business_management.product_categories.bulk_delete',
    resourceLabel:      t('product_categories.records'),
});

const duplicating = ref(null);
const duplicate = (record) => {
    duplicating.value = record.id;
    router.post(route('business_management.product_categories.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => productCategoriesExportableColumns(t));
const exportEndpoints   = computed(() => productCategoriesExportEndpoints());
...
const tour = useModuleTour({ module: 'product_categories', steps: () => productCategoriesTourSteps(t) });

const filtersDrawerOpen = ref(false);
const otrosDrawerOpen   = ref(false);
const goToCreate  = () => router.visit(route('business_management.product_categories.create'));
const goToTrash   = () => router.visit(route('business_management.product_categories.trash'));
const goToAudit   = () => router.visit(route('system_management.audit_logs.index', { module: 'product_categories' }));
const goToEditAll = () => router.visit(route('business_management.product_categories.edit_all'));

useKeyboardShortcuts({
    'ctrl+n': () => can('product_categories.create') && router.visit(route('business_management.product_categories.create')),
    'esc': () => {
...
const goEdit   = (record) => router.visit(route('business_management.product_categories.edit',   record.slug));
const goDelete = (record) => router.visit(route('business_management.product_categories.delete', record.slug));
</script>

<template>
    <Head :title="$t('product_categories.plural')" />

    <div>
        <div class="page-header">
            <ProductCategoriesPageHeader
                :title="$t('product_categories.plural')"
            ...
            <Space wrap class="hide-on-mobile">
                ...
                <span data-tour="columns">
                    <ColumnSelector
                        ...
                        storage-key="product_categories"
                    />
                </span>
                <span data-tour="export-import">
                    ...
                    <Tooltip v-if="can('product_categories.create') && canUsePlanFeature('imports')" :title="$t('global.import_hint')">
                        <Button style="margin-left: 8px;" @click="importOpen = true">
                            <UploadOutlined /> {{ $t('global.import') }}
                        </Button>
                    </Tooltip>
                </span>
                <Tooltip v-if="can('product_categories.edit') && canUsePlanFeature('edit_all')" :title="$t('global.edit_all_hint')">
                    <Link :href="route('business_management.product_categories.edit_all')" data-tour="edit-all">
                        <Button>
                ...
                <Tooltip v-if="isSuper" :title="$t('global.view_deleted_hint')">
                    <Link :href="route('business_management.product_categories.trash')" data-tour="trash">
                        <Button>
                ...
                <Tooltip v-if="canSeeAudit" :title="$t('global.audit_hint')">
                    <Link :href="route('system_management.audit_logs.index', { module: 'product_categories' })" data-tour="audit">
                        <Button>
                            <AuditOutlined /> {{ $t('sidebar.group_audit') }}
                        </Button>
                    </Link>
                </Tooltip>
                <Tooltip v-if="can('product_categories.create')" :title="$t('global.create_record_hint')">
                    <Link :href="route('business_management.product_categories.create')">
                        <Button type="primary">
                            <PlusOutlined /> {{ $t('product_categories.new') }}
                        </Button>
                    </Link>
                </Tooltip>
            </Space>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                ...
                :row-selection="(can('product_categories.delete') || can('product_categories.edit')) ? rowSelection : null"
                rowKey="id"
        ...
        <ProductCategoriesBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('product_categories.record') : $t('product_categories.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('product_categories.export_title')"
            :endpoints="exportEndpoints"
            :total-rows="categories.total ?? 0"
            :total-unfiltered="categories.total_unfiltered ?? categories.total ?? 0"
        />

        <ImportDialog
            v-model:open="importOpen"
            :endpoint="route('business_management.product_categories.import')"
            :template-url="route('business_management.product_categories.import_template')"
            :resource-label="$t('product_categories.records')"
        />

        <ProductCategoriesMobileBottomBar
        ...
        <ProductCategoriesMobileDrawers
