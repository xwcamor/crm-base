...
const submit = () => {
    form.delete(route('business_management.product_categories.deleteSave', props.category.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('product_categories.singular')" />

    <div class="delete-page">
        <SectionHeader
            :back-href="route('business_management.product_categories.index')"
            :title="$t('global.delete') + ' ' + $t('product_categories.record')"
            :subtitle="$t('product_categories.delete_hint')"
        ...
        <Card class="delete-card" :bodyStyle="{ padding: '24px 28px' }">
            <Alert type="warning" show-icon class="mb-4">
                <template #message>
                    {{ $t('product_categories.delete_about', { name: category.name }) }}
                </template>
            ...
            <div class="record-summary">
                <div class="record-summary__row">
                    ...
                    <span class="record-summary__value">{{ category.id }}</span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('product_categories.name') }}</span>
                    <span class="record-summary__value">{{ category.name }}</span>
                </div>
                <div v-if="category.parent" class="record-summary__row">
                    <span class="record-summary__label">{{ $t('product_categories.parent') }}</span>
                    <span class="record-summary__value">{{ category.parent.name }}</span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('product_categories.is_active') }}</span>
