...
import ProductCategoriesEditAllTable from '@/Components/ProductCategories/ProductCategoriesEditAllTable.vue';
...
const source = computed(() => props.categories.data);
...
const saveAll = () => {
    ...
    router.post(
        route('business_management.product_categories.edit_all.update'),
...
<template>
    <Head :title="$t('product_categories.edit_all_title')" />

    <div class="edit-all">
        <SectionHeader
            :back-href="route('business_management.product_categories.index')"
            :title="$t('global.edit_all') + ' — ' + $t('product_categories.plural')"
            :subtitle="$t('product_categories.edit_all_subtitle')"
        >
            <template #icon><EditOutlined /></template>
            <template #actions>
                <Button :disabled="dirtyCount === 0 || submitting" @click="discardAll">
                    <UndoOutlined /> {{ $t('product_categories.edit_all_discard') }}
                </Button>
                <Button
                    type="primary"
                    :loading="submitting"
                    ...
                    @click="saveAll"
                >
                    <SaveOutlined /> {{ $t('product_categories.edit_all_save_all') }}
                </Button>
            </template>
        </SectionHeader>
        ...
        <Alert v-if="dirtyCount > 0" type="info" show-icon class="status-bar">
            <template #message>
                {{ $t('product_categories.edit_all_changes', { count: dirtyCount }) }}
            </template>
        </Alert>
        ...
        <Alert
            ...
            type="error"
            show-icon
            :message="$t('product_categories.name_unique')"
            class="status-bar"
        />

        <Card :bodyStyle="{ padding: 0 }" class="edit-table-card">
            <ProductCategoriesEditAllTable
                v-model:draft="draft"
                :is-dirty="isDirty"
        ...
        <div v-if="categories.total > categories.per_page" class="pagination">
            <Pagination
