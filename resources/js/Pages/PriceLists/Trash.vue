...
import PriceListsTrashBulkBar from '@/Components/PriceLists/PriceListsTrashBulkBar.vue';
import PriceListsForceDeleteModal from '@/Components/PriceLists/PriceListsForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { priceListsTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.price_lists.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.price_lists.restore',
    bulkRestoreRouteName: 'business_management.price_lists.bulk_restore',
});
...
const openForceDelete = (priceList) => {
    forceDeleteTarget.value = priceList;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.price_lists.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => priceListsTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.priceLists.total === 1 ? t('global.record') : t('global.records');
    return `${props.priceLists.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('price_lists.plural')" />

    <div v-if="isSuper">
        <SectionHeader
            :back-href="route('business_management.price_lists.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('price_lists.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('price_lists.restore_hint')"
            ...
            <Empty
                v-if="priceLists.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <PriceListsForceDeleteModal
