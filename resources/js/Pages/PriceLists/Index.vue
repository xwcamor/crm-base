...
import {
    ...
    TagsOutlined, CheckCircleFilled,
...
import PriceListsBulkBar from '@/Components/PriceLists/PriceListsBulkBar.vue';
import PriceListsBulkDeleteModal from '@/Components/PriceLists/PriceListsBulkDeleteModal.vue';
import PriceListsFavoriteCell from '@/Components/PriceLists/PriceListsFavoriteCell.vue';
import PriceListsDetailDrawer from '@/Components/PriceLists/PriceListsDetailDrawer.vue';
import PriceListsMobileBottomBar from '@/Components/PriceLists/PriceListsMobileBottomBar.vue';
import PriceListsMobileDrawers from '@/Components/PriceLists/PriceListsMobileDrawers.vue';
import PriceListsPageHeader from '@/Components/PriceLists/PriceListsPageHeader.vue';
import PriceListsActionsCell from '@/Components/PriceLists/PriceListsActionsCell.vue';
import PriceListsEmptyState from '@/Components/PriceLists/PriceListsEmptyState.vue';
...
import {
    priceListsFilterFields, priceListsEmptyFilters, hydratePriceListsFilters,
    priceListsFiltersToQuery, priceListsFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { priceListsTableColumns } from './config/columns';
import { priceListsExportableColumns, priceListsExportEndpoints } from './config/exports';
import { priceListsTourSteps } from './config/tour';
...
const filterFields = computed(() =>
    priceListsFilterFields(t, { currencyOptions: props.currencyOptions }),
);
...
const {
    ...
    hydrate:       hydratePriceListsFilters,
    toQuery:       priceListsFiltersToQuery,
    summary:       priceListsFiltersSummary,
    empty:         priceListsEmptyFilters,
    only:          ['priceLists', 'filters'],
    t,
});
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.priceLists),
    hasActiveFilters,
    t,
});

const allColumns = computed(() =>
    priceListsTableColumns(t, { isSuper: isSuper.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

const tablePagination = computed(() => ({
    current:  props.priceLists.current_page,
    pageSize: props.priceLists.per_page,
    total:    props.priceLists.total,
...
useModuleUndoToast('business_management.price_lists.undo_last_delete');

const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('price_lists', 'priceLists');

const { isMobile: isMobileScreen } = useViewport(768);
const drawerWidth = computed(() => isMobileScreen.value ? '100%' : 480);
const { open: drawerVisible, selected: selectedPriceList, openDetails } = useModuleDrawer({ module: 'price_lists' });
...
const {
    ...
    bulkSetActiveRoute: 'business_management.price_lists.bulk_set_active',
    bulkDeleteRoute:    'business_management.price_lists.bulk_delete',
    resourceLabel:      t('price_lists.records'),
});

const duplicating = ref(null);
const duplicate = (record) => {
    duplicating.value = record.id;
    router.post(route('business_management.price_lists.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => priceListsExportableColumns(t));
const exportEndpoints   = computed(() => priceListsExportEndpoints());
...
const tour = useModuleTour({ module: 'price_lists', steps: () => priceListsTourSteps(t) });

const filtersDrawerOpen = ref(false);
const otrosDrawerOpen   = ref(false);
const goToCreate  = () => router.visit(route('business_management.price_lists.create'));
const goToTrash   = () => router.visit(route('business_management.price_lists.trash'));
const goToAudit   = () => router.visit(route('system_management.audit_logs.index', { module: 'price_lists' }));
const goToEditAll = () => router.visit(route('business_management.price_lists.edit_all'));

useKeyboardShortcuts({
    'ctrl+n': () => can('price_lists.create') && router.visit(route('business_management.price_lists.create')),
    'esc': () => {
...
const goEdit   = (record) => router.visit(route('business_management.price_lists.edit',   record.slug));
const goDelete = (record) => router.visit(route('business_management.price_lists.delete', record.slug));
</script>

<template>
    <Head :title="$t('price_lists.plural')" />

    <div>
        <div class="page-header">
            <PriceListsPageHeader
                :title="$t('price_lists.plural')"
            ...
            <Space wrap class="hide-on-mobile">
                ...
                <span data-tour="export-import">
                    ...
                    <Tooltip v-if="can('price_lists.create') && canUsePlanFeature('imports')" :title="$t('global.import_hint')">
                        <Button style="margin-left: 8px;" @click="importOpen = true">
                            <UploadOutlined /> {{ $t('global.import') }}
                        </Button>
                    </Tooltip>
                </span>
                <Tooltip v-if="can('price_lists.edit') && canUsePlanFeature('edit_all')" :title="$t('global.edit_all_hint')">
                    <Link :href="route('business_management.price_lists.edit_all')" data-tour="edit-all">
                        <Button>
                ...
                <Tooltip v-if="isSuper" :title="$t('global.view_deleted_hint')">
                    <Link :href="route('business_management.price_lists.trash')" data-tour="trash">
                        <Button>
                ...
                <Tooltip v-if="canSeeAudit" :title="$t('global.audit_hint')">
                    <Link :href="route('system_management.audit_logs.index', { module: 'price_lists' })" data-tour="audit">
                        <Button>
                            <AuditOutlined /> {{ $t('sidebar.group_audit') }}
                        </Button>
                    </Link>
                </Tooltip>
                <Tooltip v-if="can('price_lists.create')" :title="$t('global.create_record_hint')">
                    <Link :href="route('business_management.price_lists.create')">
                        <Button type="primary">
                            <PlusOutlined /> {{ $t('price_lists.new') }}
                        </Button>
                    </Link>
                </Tooltip>
            </Space>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                :dataSource="priceLists.data"
                :columns="columns"
                :pagination="tablePagination"
                :row-selection="(can('price_lists.delete') || can('price_lists.edit')) ? rowSelection : null"
                rowKey="id"
        ...
        <PriceListsBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('price_lists.record') : $t('price_lists.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('price_lists.export_title')"
            :endpoints="exportEndpoints"
            :total-rows="priceLists.total ?? 0"
            :total-unfiltered="priceLists.total_unfiltered ?? priceLists.total ?? 0"
        />

        <ImportDialog
            v-model:open="importOpen"
            :endpoint="route('business_management.price_lists.import')"
            :template-url="route('business_management.price_lists.import_template')"
            :resource-label="$t('price_lists.records')"
        />

        <PriceListsMobileBottomBar
