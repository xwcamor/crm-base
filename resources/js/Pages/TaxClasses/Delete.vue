...
const submit = () => {
    form.delete(route('business_management.tax_classes.deleteSave', props.taxClass.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('tax_classes.singular')" />

    <div class="delete-page">
        <SectionHeader
            :back-href="route('business_management.tax_classes.index')"
            :title="$t('global.delete') + ' ' + $t('tax_classes.record')"
            :subtitle="$t('tax_classes.delete_hint')"
        ...
        <Card class="delete-card" :bodyStyle="{ padding: '24px 28px' }">
            ...
            >
                <template #message>
                    {{ $t('tax_classes.delete_about', { name: taxClass.name }) }}
                </template>
            ...
            <div class="record-summary">
                <div class="record-summary__row">
                    ...
                    <span class="record-summary__value">{{ taxClass.id }}</span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('tax_classes.name') }}</span>
                    <span class="record-summary__value">{{ taxClass.name }}</span>
                </div>
<div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('tax_classes.is_active') }}</span>
