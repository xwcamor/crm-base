/**
 * Columnas de la tabla principal de TaxClasses.
 *
 ...
 *   - Workspace (tenant): cruz-tenant, super ve tax_classes de varios
 *     workspaces. Admin solo ve los suyos, la columna seria redundante.
 */
export const tax_classesTableColumns = (t, { isSuper = false } = {}) => [
    ...
    { title: t('tax_classes.code'),     dataIndex: 'code',         key: 'code',       width: 130, sorter: (a, b) => (a.code ?? '').localeCompare(b.code ?? ''), mobile: { role: 'meta' } },
    { title: t('tax_classes.name'),      dataIndex: 'name',        key: 'name',       sorter: (a, b) => a.name.localeCompare(b.name), alwaysVisible: true, mobile: { role: 'title' } },
    { title: t('tax_classes.is_default'), dataIndex: 'is_default', key: 'is_default', width: 110, align: 'center', mobile: { role: 'meta' } },
...(isSuper ? [
        { title: t('tenants.singular'), dataIndex: ['tenant', 'name'], key: 'tenant', width: 180, mobile: { role: 'meta' } },
    ] : []),
    { title: t('tax_classes.is_active'), dataIndex: 'is_active',   key: 'status',     width: 110, align: 'center', mobile: { role: 'status' } },
