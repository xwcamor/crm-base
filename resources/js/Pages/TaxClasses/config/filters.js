 ...
 */
export const tax_classesFilterFields = (t) => [
    { key: 'name',           label: t('tax_classes.filter_name'), type: 'tags' },
{ key: 'is_active',      label: t('tax_classes.is_active'),   type: 'select', options: [
...
export const tax_classesEmptyFilters = () => ({
    name: [],
    is_active: null,
    created_at: null,
...
export const hydrateTaxClassesFilters = (server) => ({
...
export const tax_classesFiltersToQuery = (f) => ({
...
export const tax_classesFiltersSummary = (f, t) => {
    const parts = [];
    if (f.name?.length)        parts.push(`${t('tax_classes.filter_name')}: ${f.name.join(', ')}`);
if (f.is_active !== null && f.is_active !== undefined) {
        parts.push(`${t('tax_classes.is_active')}: ${f.is_active ? t('global.active') : t('global.inactive')}`);
    }
