...
import ProductsEditAllTable from '@/Components/Products/ProductsEditAllTable.vue';
...
/**
 * Edit-All de Products. Mantenemos snapshot `original` + `draft` mutable
...
const source = computed(() => props.products.data);
...
const saveAll = () => {
    ...
    router.post(
        route('business_management.products.edit_all.update'),
...
<template>
    <Head :title="$t('products.edit_all_title')" />

    <div class="edit-all">
        <SectionHeader
            :back-href="route('business_management.products.index')"
            :title="$t('global.edit_all') + ' — ' + $t('products.plural')"
            :subtitle="$t('products.edit_all_subtitle')"
        >
            <template #icon><EditOutlined /></template>
            <template #actions>
                <Button :disabled="dirtyCount === 0 || submitting" @click="discardAll">
                    <UndoOutlined /> {{ $t('products.edit_all_discard') }}
                </Button>
                <Button
                    type="primary"
                    :loading="submitting"
                    ...
                    @click="saveAll"
                >
                    <SaveOutlined /> {{ $t('products.edit_all_save_all') }}
                </Button>
            </template>
        </SectionHeader>
        ...
        <Alert v-if="dirtyCount > 0" type="info" show-icon class="status-bar">
            <template #message>
                {{ $t('products.edit_all_changes', { count: dirtyCount }) }}
            </template>
        </Alert>
        ...
        <Alert
            ...
            type="error"
            show-icon
            :message="$t('products.name_unique')"
            class="status-bar"
        />

        <Card :bodyStyle="{ padding: 0 }" class="edit-table-card">
            <ProductsEditAllTable
                v-model:draft="draft"
                :is-dirty="isDirty"
        ...
        <div v-if="products.total > products.per_page" class="pagination">
            <Pagination
