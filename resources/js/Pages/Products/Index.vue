...
import {
    ...
    AppstoreOutlined, ToolOutlined, ReloadOutlined, BlockOutlined,
} from '@ant-design/icons-vue';

// Iconos por tipo de producto (Ant Design — sin emojis).
const typeIconMap = {
    good:         AppstoreOutlined,
    service:      ToolOutlined,
    subscription: ReloadOutlined,
    bundle:       BlockOutlined,
};
const typeColorMap = { good: 'blue', service: 'green', subscription: 'purple', bundle: 'orange' };
...
import ProductsBulkBar from '@/Components/Products/ProductsBulkBar.vue';
import ProductsBulkDeleteModal from '@/Components/Products/ProductsBulkDeleteModal.vue';
import ProductsFavoriteCell from '@/Components/Products/ProductsFavoriteCell.vue';
import ProductsDetailDrawer from '@/Components/Products/ProductsDetailDrawer.vue';
import ProductsMobileBottomBar from '@/Components/Products/ProductsMobileBottomBar.vue';
import ProductsMobileDrawers from '@/Components/Products/ProductsMobileDrawers.vue';
import ProductsPageHeader from '@/Components/Products/ProductsPageHeader.vue';
import ProductsActionsCell from '@/Components/Products/ProductsActionsCell.vue';
...
// El toolbar inline de Products (no usa ModuleToolbar) repite manualmente
// estos gates para no mostrar botones que no funcionan al user free/basic.
const { canUse: canUsePlanFeature } = usePlanFeatures();

import {
    productsFilterFields, productsEmptyFilters, hydrateProductsFilters,
    productsFiltersToQuery, productsFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { productsTableColumns } from './config/columns';
import { productsExportableColumns, productsExportEndpoints } from './config/exports';
import { productsTourSteps } from './config/tour';
...
const {
    ...
    hydrate:       hydrateProductsFilters,
    toQuery:       productsFiltersToQuery,
    summary:       productsFiltersSummary,
    empty:         productsEmptyFilters,
...
const allColumns = computed(() =>
    productsTableColumns(t, { isSuper: isSuper.value }),
);
...
useModuleUndoToast('business_management.products.undo_last_delete');

// ─── Favoritos polimorficos ────────────────────────────────────────────────
const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('products', 'products');
...
const { open: drawerVisible, selected: selectedProduct, openDetails } = useModuleDrawer({ module: 'products' });
...
const {
    ...
    bulkSetActiveRoute: 'business_management.products.bulk_set_active',
    bulkDeleteRoute:    'business_management.products.bulk_delete',
    resourceLabel:      t('products.records'),
});
...
const duplicate = (record) => {
    ...
    router.post(route('business_management.products.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => productsExportableColumns(t));
const exportEndpoints   = computed(() => productsExportEndpoints());
...
const tour = useModuleTour({ module: 'products', steps: () => productsTourSteps(t) });

// ─── Mobile: drawers + navegación (patrón Regions: app-like en mobile) ──────
const filtersDrawerOpen = ref(false);
const otrosDrawerOpen   = ref(false);
const goToCreate  = () => router.visit(route('business_management.products.create'));
const goToTrash   = () => router.visit(route('business_management.products.trash'));
const goToAudit   = () => router.visit(route('system_management.audit_logs.index', { module: 'products' }));
const goToEditAll = () => router.visit(route('business_management.products.edit_all'));

// ─── Keyboard shortcuts ────────────────────────────────────────────────────
useKeyboardShortcuts({
    'ctrl+n': () => can('products.create') && router.visit(route('business_management.products.create')),
    'esc': () => {
...
// ─── Acciones ───────────────────────────────────────────────────────────────
const goEdit   = (record) => router.visit(route('business_management.products.edit',   record.slug));
const goDelete = (record) => router.visit(route('business_management.products.delete', record.slug));
</script>

<template>
    <Head :title="$t('products.plural')" />
    ...
    <div>
        <div class="page-header">
            ...
            <Space wrap class="hide-on-mobile">
                ...
                <span data-tour="export-import">
                    ...
                    <Tooltip v-if="can('products.create') && canUsePlanFeature('imports')" :title="$t('global.import_hint')">
                        <Button style="margin-left: 8px;" @click="importOpen = true">
                            <UploadOutlined /> {{ $t('global.import') }}
                        </Button>
                    </Tooltip>
                </span>
                <Tooltip v-if="can('products.edit') && canUsePlanFeature('edit_all')" :title="$t('global.edit_all_hint')">
                    <Link :href="route('business_management.products.edit_all')" data-tour="edit-all">
                        <Button>
                ...
                <Tooltip v-if="isSuper" :title="$t('global.view_deleted_hint')">
                    <Link :href="route('business_management.products.trash')" data-tour="trash">
                        <Button>
                ...
                <Tooltip v-if="canSeeAudit" :title="$t('global.audit_hint')">
                    <Link :href="route('system_management.audit_logs.index', { module: 'products' })" data-tour="audit">
                        <Button>
                            <AuditOutlined /> {{ $t('sidebar.group_audit') }}
                        </Button>
                    </Link>
                </Tooltip>
                <Tooltip v-if="can('products.create')" :title="$t('global.create_record_hint')">
                    <Link :href="route('business_management.products.create')">
                        <Button type="primary">
                            <PlusOutlined /> {{ $t('products.new') }}
                        </Button>
                    </Link>
                </Tooltip>
            </Space>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                ...
                :row-selection="(can('products.delete') || can('products.edit')) ? rowSelection : null"
                rowKey="id"
                ...
                <template #bodyCell="{ column, record, text, isMobile }">
                    ...
                    <template v-else-if="column.key === 'type'">
                        <Tag :color="typeColorMap[record.type] || 'default'" :bordered="false">
                            <component :is="typeIconMap[record.type] || AppstoreOutlined" style="margin-right: 4px" />
                            {{ $t('products.type_options.' + record.type) }}
                        </Tag>
                    </template>

                    <template v-else-if="column.key === 'list_price'">
                        <strong>{{ fmtMoney(record.list_price) }}</strong>
                    </template>

                    <template v-else-if="column.key === 'sku'">
                        <code v-if="record.sku" class="muted-code">{{ record.sku }}</code>
...
<template v-else-if="column.key === 'tenant'">
        ...
        <ProductsBulkDeleteModal
            ...
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('products.record') : $t('products.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('products.export_title')"
            :endpoints="exportEndpoints"
            :total-rows="products.total ?? 0"
            :total-unfiltered="products.total_unfiltered ?? products.total ?? 0"
        />

        <ImportDialog
            v-model:open="importOpen"
            :endpoint="route('business_management.products.import')"
            :template-url="route('business_management.products.import_template')"
            :resource-label="$t('products.records')"
        />
