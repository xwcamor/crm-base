...
import ProductsTrashBulkBar from '@/Components/Products/ProductsTrashBulkBar.vue';
import ProductsForceDeleteModal from '@/Components/Products/ProductsForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { productsTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.products.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.products.restore',
    bulkRestoreRouteName: 'business_management.products.bulk_restore',
});
...
const openForceDelete = (product) => {
    forceDeleteTarget.value = product;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.products.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => productsTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.products.total === 1 ? t('global.record') : t('global.records');
    return `${props.products.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('products.plural')" />

    <div v-if="isSuper">
        <SectionHeader
            :back-href="route('business_management.products.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('products.plural')"
            :subtitle="subtitle"
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('products.restore_hint')"
