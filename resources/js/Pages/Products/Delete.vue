...
const submit = () => {
    form.delete(route('business_management.products.deleteSave', props.product.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('products.singular')" />

    <div class="delete-page">
        <SectionHeader
            :back-href="route('business_management.products.index')"
            :title="$t('global.delete') + ' ' + $t('products.record')"
            :subtitle="$t('products.delete_hint')"
        ...
        <Card class="delete-card" :bodyStyle="{ padding: '24px 28px' }">
            ...
            >
                <template #message>
                    {{ $t('products.delete_about', { name: product.name }) }}
                </template>
            ...
            <div class="record-summary">
                <div class="record-summary__row">
                    ...
                    <span class="record-summary__value">{{ product.id }}</span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('products.name') }}</span>
                    <span class="record-summary__value">{{ product.name }}</span>
                </div>
<div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('products.is_active') }}</span>
