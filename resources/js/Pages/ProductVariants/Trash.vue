...
import ProductVariantsTrashBulkBar from '@/Components/ProductVariants/ProductVariantsTrashBulkBar.vue';
import ProductVariantsForceDeleteModal from '@/Components/ProductVariants/ProductVariantsForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { productVariantsTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.product_variants.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.product_variants.restore',
    bulkRestoreRouteName: 'business_management.product_variants.bulk_restore',
});
...
const openForceDelete = (variant) => {
    forceDeleteTarget.value = variant;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.product_variants.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => productVariantsTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.variants.total === 1 ? t('global.record') : t('global.records');
    return `${props.variants.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('product_variants.plural')" />

    <div v-if="isSuper">
        <SectionHeader
            :back-href="route('business_management.product_variants.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('product_variants.plural')"
            :subtitle="subtitle"
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('product_variants.restore_hint')"
        ...
        <ProductVariantsForceDeleteModal
