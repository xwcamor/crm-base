...
import CompaniesTrashBulkBar from '@/Components/Companies/CompaniesTrashBulkBar.vue';
import CompaniesForceDeleteModal from '@/Components/Companies/CompaniesForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { companiesTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('crm.companies.index'));
}
...
const {
    ...
    restoreRouteName:     'crm.companies.restore',
    bulkRestoreRouteName: 'crm.companies.bulk_restore',
});
...
const openForceDelete = (company) => {
    forceDeleteTarget.value = company;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('crm.companies.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => companiesTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.companies.total === 1 ? t('global.record') : t('global.records');
    return `${props.companies.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('companies.plural')" />

    <div v-if="isSuper">
        <SectionHeader
            :back-href="route('crm.companies.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('companies.plural')"
            :subtitle="subtitle"
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('companies.restore_hint')"
