...
import CompaniesEditAllTable from '@/Components/Companies/CompaniesEditAllTable.vue';
...
/**
 * Edit-All de Companies. Mantenemos snapshot `original` + `draft` mutable
...
const saveAll = () => {
    ...
    router.post(
        route('crm.companies.edit_all.update'),
...
<template>
    <Head :title="$t('companies.edit_all_title')" />

    <div class="edit-all">
        <SectionHeader
            :back-href="route('crm.companies.index')"
            :title="$t('global.edit_all') + ' — ' + $t('companies.plural')"
            :subtitle="$t('companies.edit_all_subtitle')"
        >
            <template #icon><EditOutlined /></template>
            <template #actions>
                <Button :disabled="dirtyCount === 0 || submitting" @click="discardAll">
                    <UndoOutlined /> {{ $t('companies.edit_all_discard') }}
                </Button>
                <Button
                    type="primary"
                    :loading="submitting"
                    ...
                    @click="saveAll"
                >
                    <SaveOutlined /> {{ $t('companies.edit_all_save_all') }}
                </Button>
            </template>
        </SectionHeader>
        ...
        <Alert v-if="dirtyCount > 0" type="info" show-icon class="status-bar">
            <template #message>
                {{ $t('companies.edit_all_changes', { count: dirtyCount }) }}
            </template>
        </Alert>
        ...
        <Alert
            ...
            type="error"
            show-icon
            :message="$t('companies.name_unique')"
            class="status-bar"
        />
        ...
        <div v-if="companies.total > companies.per_page" class="pagination">
            <Pagination
