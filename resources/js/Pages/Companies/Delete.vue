...
const submit = () => {
    form.delete(route('crm.companies.deleteSave', props.company.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    ...
    <div class="delete-page">
        <SectionHeader
            ...
            :subtitle="$t('companies.delete_hint')"
        ...
        <Card class="delete-card" :bodyStyle="{ padding: '24px 28px' }">
            ...
            >
                <template #message>
                    {{ $t('companies.delete_about', { name: company.name }) }}
                </template>
            ...
            <div class="record-summary">
                <div class="record-summary__row">
                    ...
                    <span class="record-summary__value">{{ company.id }}</span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('companies.name') }}</span>
                    <span class="record-summary__value">{{ company.name }}</span>
                </div>
<div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('companies.is_active') }}</span>
