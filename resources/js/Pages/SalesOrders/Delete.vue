...
const submit = () => {
    form.delete(route('business_management.sales_orders.deleteSave', props.order.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('sales_orders.singular')" />

    <div class="delete-page">
        <SectionHeader
            :back-href="route('business_management.sales_orders.index')"
            :title="$t('global.delete') + ' ' + $t('sales_orders.record')"
            :subtitle="$t('sales_orders.delete_hint')"
        ...
        <Card class="delete-card" :bodyStyle="{ padding: '24px 28px' }">
            <Alert type="warning" show-icon class="mb-4">
                <template #message>
                    {{ $t('sales_orders.delete_about', { name: order.reference }) }}
                </template>
            ...
            <div class="record-summary">
                ...
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('sales_orders.reference') }}</span>
                    <span class="record-summary__value"><strong>{{ order.reference }}</strong></span>
                </div>
                <div v-if="order.company" class="record-summary__row">
                    <span class="record-summary__label">{{ $t('sales_orders.company') }}</span>
                    <span class="record-summary__value">{{ order.company.name }}</span>
                </div>
                <div v-if="order.warehouse" class="record-summary__row">
                    <span class="record-summary__label">{{ $t('sales_orders.warehouse') }}</span>
                    <span class="record-summary__value">{{ order.warehouse.name }} ({{ order.warehouse.code }})</span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('sales_orders.status') }}</span>
                ...
                <div v-if="order.grand_total != null" class="record-summary__row">
                    <span class="record-summary__label">{{ $t('sales_orders.grand_total') }}</span>
                    <span class="record-summary__value">
                        <strong>{{ order.currency_code }} {{ Number(order.grand_total).toFixed(2) }}</strong>
                    </span>
                </div>
            </div>
            ...
            <Form layout="vertical" @submit.prevent="submit">
                ...
                <DeleteFooter
                    :cancel-href="route('business_management.sales_orders.index')"
...
.record-summary__row { display: flex; justify-content: space-between; align-items: flex-start; gap: 12px; font-size: 0.875rem; min-width: 0; }
.record-summary__label { color: var(--color-text-muted); font-weight: 500; flex-shrink: 0; }
.record-summary__value { color: var(--color-text); text-align: right; word-break: break-word; overflow-wrap: anywhere; min-width: 0; flex: 1 1 auto; }
