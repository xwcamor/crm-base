...
import SalesOrdersTrashBulkBar from '@/Components/SalesOrders/SalesOrdersTrashBulkBar.vue';
import SalesOrdersForceDeleteModal from '@/Components/SalesOrders/SalesOrdersForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { salesOrdersTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.sales_orders.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.sales_orders.restore',
    bulkRestoreRouteName: 'business_management.sales_orders.bulk_restore',
});
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.sales_orders.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => salesOrdersTrashColumns(t));
...
<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('sales_orders.plural')" />

    <div v-if="isSuper">
        <SectionHeader
            :back-href="route('business_management.sales_orders.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('sales_orders.plural')"
            :subtitle="subtitle"
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('sales_orders.restore_hint')"
