...
import { ShoppingCartOutlined, PlusOutlined, DeleteOutlined, DragOutlined, ShoppingOutlined } from '@ant-design/icons-vue';
...
function emptyLine() { return { product_id: null, name: '', sku: '', quantity: 1, unit_price: 0, discount_pct: 0, tax_pct: 18 }; }

const form = useForm({
    reference:     props.order?.reference ?? props.nextReference ?? '',
    company_id:    props.order?.company_id ?? null,
    contact_id:    props.order?.contact_id ?? null,
    ...
    expected_delivery_date: props.order?.expected_delivery_date ?? in7,
    ...
    items:         props.order?.items?.length
        ? props.order.items.map(i => ({
            product_id: i.product_id ?? null, name: i.name, sku: i.sku ?? '',
...
const onProductSelect = (idx, productId) => {
    ...
    if (p) { form.items[idx].name = p.name; form.items[idx].sku = p.sku ?? ''; form.items[idx].unit_price = p.price ?? 0; }
};
...
const submit = () => isEdit.value
    ? form.put(route('business_management.sales_orders.update', props.order.slug))
    : form.post(route('business_management.sales_orders.store'));
</script>

<template>
    <Head :title="isEdit ? 'Editar OV' : 'Nueva OV'" />
    <div class="so-form">
        <SectionHeader :back-href="route('business_management.sales_orders.index')"
            :title="isEdit ? 'Editar orden de venta' : 'Nueva orden de venta'" :subtitle="form.reference || 'Borrador'">
        ...
        <Form layout="vertical" @submit.prevent="submit">
            <Row :gutter="[16, 16]">
                    ...
                    <Card title="Datos del documento">
                        <Row :gutter="[16, 0]">
                            <Col :xs="12" :md="8"><FormItem>
                                <template #label><LabelWithHelp :label="$t('sales_orders.reference')" :help="$t('sales_orders.reference_hint')" /></template>
                                <Input v-model:value="form.reference" size="large" :maxlength="30" />
                            </FormItem></Col>
                            <Col :xs="12" :md="8"><FormItem required>
                                <template #label><LabelWithHelp :label="$t('sales_orders.status')" :help="$t('sales_orders.status_hint')" /></template>
                                <Select v-model:value="form.status" :options="statusOptions" size="large" />
                            </FormItem></Col>
                            <Col :xs="12" :md="8"><FormItem required>
                                <template #label><LabelWithHelp :label="$t('sales_orders.payment_status')" :help="$t('sales_orders.payment_status_hint')" /></template>
                                <Select v-model:value="form.payment_status" :options="paymentStatusOptions" size="large" />
                            </FormItem></Col>

                            <Col :xs="24" :md="12"><FormItem required :validate-status="form.errors.company_id ? 'error' : ''" :help="form.errors.company_id">
                                <template #label><LabelWithHelp :label="$t('sales_orders.company')" :help="$t('sales_orders.company_hint')" /></template>
                                <Select v-model:value="form.company_id" :options="companyOptions" size="large" show-search allow-clear @change="form.contact_id = null"
                                    :filter-option="(i,o)=>(o.label??'').toLowerCase().includes(i.toLowerCase())" />
                            </FormItem></Col>
                            <Col :xs="24" :md="12"><FormItem>
                                <template #label><LabelWithHelp :label="$t('sales_orders.contact')" :help="$t('sales_orders.contact_hint')" /></template>
                            ...
                            <Col :xs="12" :md="6"><FormItem required>
                                <template #label><LabelWithHelp :label="$t('sales_orders.order_date')" :help="$t('sales_orders.order_date_hint')" /></template>
                                <DatePicker v-model:value="form.order_date" valueFormat="YYYY-MM-DD" size="large" style="width:100%" />
                            </FormItem></Col>
                            <Col :xs="12" :md="6"><FormItem>
                                <template #label><LabelWithHelp :label="$t('sales_orders.expected_delivery_date')" :help="$t('sales_orders.expected_delivery_date_hint')" /></template>
                                <DatePicker v-model:value="form.expected_delivery_date" valueFormat="YYYY-MM-DD" size="large" style="width:100%" />
                            </FormItem></Col>
                            <Col :xs="12" :md="6"><FormItem required :validate-status="form.errors.warehouse_id ? 'error' : ''">
                                <template #label><LabelWithHelp :label="$t('sales_orders.warehouse')" :help="$t('sales_orders.warehouse_hint')" /></template>
                            ...
                            <Col :xs="12" :md="6"><FormItem>
                                <template #label><LabelWithHelp :label="$t('sales_orders.payment_terms_days')" :help="$t('sales_orders.payment_terms_days_hint')" /></template>
                                <InputNumber v-model:value="form.payment_terms_days" :min="0" size="large" style="width:100%" />
                            </FormItem></Col>

                            <Col :xs="12" :md="8"><FormItem>
                                <template #label><LabelWithHelp :label="$t('sales_orders.owner')" :help="$t('sales_orders.owner_hint')" /></template>
                            ...
                            <Col :xs="12" :md="8"><FormItem>
                                <template #label><LabelWithHelp :label="$t('sales_orders.currency')" :help="$t('sales_orders.currency_hint')" /></template>
                ...
                <Col :xs="24">
                    <Card>
                        ...
                        <div class="items-table">
                            <div class="items-header"><div></div><div>Producto</div><div>Cant.</div><div>Precio unit.</div><div>% Desc</div><div>% Imp</div><div style="text-align:right">Total</div><div></div></div>
                ...
                <Col :xs="24" :md="12"><Card title="Notas cliente"><Input.TextArea v-model:value="form.notes" :rows="4" :maxlength="2000" show-count /></Card></Col>
                <Col :xs="24" :md="12"><Card title="Notas internas"><Input.TextArea v-model:value="form.internal_notes" :rows="4" :maxlength="2000" show-count /></Card></Col>
            </Row>
            <FormFooter :cancel-href="route('business_management.sales_orders.index')" :is-edit="isEdit" :processing="form.processing" create-label-key="global.save_changes" />
        </Form>
    </div>
</template>

<style scoped>
.so-form > * + * { margin-top: 16px; }
