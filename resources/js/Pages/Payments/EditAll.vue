...
import PaymentsEditAllTable from '@/Components/Payments/PaymentsEditAllTable.vue';
...
/**
 * Edit-All de Payments. Mantenemos snapshot `original` + `draft` mutable
...
const source = computed(() => props.payments.data);
...
const saveAll = () => {
    ...
    router.post(
        route('business_management.payments.edit_all.update'),
...
<template>
    <Head :title="$t('payments.edit_all_title')" />

    <div class="edit-all">
        <SectionHeader
            :back-href="route('business_management.payments.index')"
            :title="$t('global.edit_all') + ' — ' + $t('payments.plural')"
            :subtitle="$t('payments.edit_all_subtitle')"
        >
            <template #icon><EditOutlined /></template>
            <template #actions>
                <Button :disabled="dirtyCount === 0 || submitting" @click="discardAll">
                    <UndoOutlined /> {{ $t('payments.edit_all_discard') }}
                </Button>
                <Button
                    type="primary"
                    :loading="submitting"
                    ...
                    @click="saveAll"
                >
                    <SaveOutlined /> {{ $t('payments.edit_all_save_all') }}
                </Button>
            </template>
        </SectionHeader>
        ...
        <Alert v-if="dirtyCount > 0" type="info" show-icon class="status-bar">
            <template #message>
                {{ $t('payments.edit_all_changes', { count: dirtyCount }) }}
            </template>
        </Alert>
        ...
        <Alert
            ...
            type="error"
            show-icon
            :message="$t('payments.name_unique')"
            class="status-bar"
        />

        <Card :bodyStyle="{ padding: 0 }" class="edit-table-card">
            <PaymentsEditAllTable
                v-model:draft="draft"
                :is-dirty="isDirty"
        ...
        <div v-if="payments.total > payments.per_page" class="pagination">
            <Pagination
