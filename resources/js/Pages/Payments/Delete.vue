...
const submit = () => {
    form.delete(route('business_management.payments.deleteSave', props.payment.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('payments.singular')" />

    <div class="delete-page">
        <SectionHeader
            :back-href="route('business_management.payments.index')"
            :title="$t('global.delete') + ' ' + $t('payments.record')"
            :subtitle="$t('payments.delete_hint')"
        ...
        <Card class="delete-card" :bodyStyle="{ padding: '24px 28px' }">
            ...
            >
                <template #message>
                    {{ $t('payments.delete_about', { name: payment.name }) }}
                </template>
            ...
            <div class="record-summary">
                <div class="record-summary__row">
                    ...
                    <span class="record-summary__value">{{ payment.id }}</span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('payments.name') }}</span>
                    <span class="record-summary__value">{{ payment.name }}</span>
                </div>
<div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('payments.is_active') }}</span>
                    <span class="record-summary__value">
                        <Tag :color="payment.is_active ? 'success' : 'error'" :bordered="false">
                            {{ payment.is_active ? $t('global.active') : $t('global.inactive') }}
                        </Tag>
                    </span>
                </div>
            </div>
            ...
            <Form layout="vertical" @submit.prevent="submit">
                ...
                <DeleteFooter
                    :cancel-href="route('business_management.payments.index')"
