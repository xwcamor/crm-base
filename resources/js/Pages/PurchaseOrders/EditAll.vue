...
import PurchaseOrdersEditAllTable from '@/Components/PurchaseOrders/PurchaseOrdersEditAllTable.vue';
...
const saveAll = () => {
    ...
    router.post(
        route('business_management.purchase_orders.edit_all.update'),
...
<template>
    <Head :title="$t('purchase_orders.edit_all_title')" />

    <div class="edit-all">
        <SectionHeader
            :back-href="route('business_management.purchase_orders.index')"
            :title="$t('global.edit_all') + ' — ' + $t('purchase_orders.plural')"
            :subtitle="$t('purchase_orders.edit_all_subtitle')"
        >
            <template #icon><EditOutlined /></template>
            <template #actions>
                <Button :disabled="dirtyCount === 0 || submitting" @click="discardAll">
                    <UndoOutlined /> {{ $t('purchase_orders.edit_all_discard') }}
                </Button>
                <Button
                    type="primary"
                    :loading="submitting"
                    ...
                    @click="saveAll"
                >
                    <SaveOutlined /> {{ $t('purchase_orders.edit_all_save_all') }}
                </Button>
            </template>
        </SectionHeader>
        ...
        <Alert v-if="dirtyCount > 0" type="info" show-icon class="status-bar">
            <template #message>
                {{ $t('purchase_orders.edit_all_changes', { count: dirtyCount }) }}
            </template>
        </Alert>
        ...
        <Alert
            ...
            type="error"
            show-icon
            :message="$t('purchase_orders.name_unique')"
            class="status-bar"
        />

        <Card :bodyStyle="{ padding: 0 }" class="edit-table-card">
            <PurchaseOrdersEditAllTable
                v-model:draft="draft"
                :is-dirty="isDirty"
