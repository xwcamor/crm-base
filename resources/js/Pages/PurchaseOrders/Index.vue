...
import PurchaseOrdersBulkBar from '@/Components/PurchaseOrders/PurchaseOrdersBulkBar.vue';
import PurchaseOrdersBulkDeleteModal from '@/Components/PurchaseOrders/PurchaseOrdersBulkDeleteModal.vue';
import PurchaseOrdersFavoriteCell from '@/Components/PurchaseOrders/PurchaseOrdersFavoriteCell.vue';
import PurchaseOrdersDetailDrawer from '@/Components/PurchaseOrders/PurchaseOrdersDetailDrawer.vue';
import PurchaseOrdersMobileBottomBar from '@/Components/PurchaseOrders/PurchaseOrdersMobileBottomBar.vue';
import PurchaseOrdersMobileDrawers from '@/Components/PurchaseOrders/PurchaseOrdersMobileDrawers.vue';
import PurchaseOrdersPageHeader from '@/Components/PurchaseOrders/PurchaseOrdersPageHeader.vue';
import PurchaseOrdersActionsCell from '@/Components/PurchaseOrders/PurchaseOrdersActionsCell.vue';
import PurchaseOrdersEmptyState from '@/Components/PurchaseOrders/PurchaseOrdersEmptyState.vue';
...
import {
    purchaseOrdersFilterFields, purchaseOrdersEmptyFilters, hydratePurchaseOrdersFilters,
    purchaseOrdersFiltersToQuery, purchaseOrdersFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { purchaseOrdersTableColumns } from './config/columns';
import { purchaseOrdersExportableColumns, purchaseOrdersExportEndpoints } from './config/exports';
import { purchaseOrdersTourSteps } from './config/tour';
...
const {
    ...
    hydrate:       hydratePurchaseOrdersFilters,
    toQuery:       purchaseOrdersFiltersToQuery,
    summary:       purchaseOrdersFiltersSummary,
    empty:         purchaseOrdersEmptyFilters,
    only:          ['orders', 'filters'],
    t,
});
...
const allColumns = computed(() =>
    purchaseOrdersTableColumns(t, { isSuper: isSuper.value }),
);
...
useModuleUndoToast('business_management.purchase_orders.undo_last_delete');

const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('purchase_orders', 'purchase_orders');

const { isMobile: isMobileScreen } = useViewport(768);
const drawerWidth = computed(() => isMobileScreen.value ? '100%' : 480);
const { open: drawerVisible, selected: selectedOrder, openDetails } = useModuleDrawer({ module: 'purchase_orders' });
...
const {
    ...
    bulkSetActiveRoute: 'business_management.purchase_orders.bulk_set_active',
    bulkDeleteRoute:    'business_management.purchase_orders.bulk_delete',
    resourceLabel:      t('purchase_orders.records'),
});

// Bulk set status: el endpoint purchase_orders.bulk_set_active recibe status (enum)
// en lugar de is_active (boolean). Llamamos directamente al endpoint.
const bulkSetStatus = (status) => {
    if (selectedRowKeys.value.length === 0) return;
    router.post(
        route('business_management.purchase_orders.bulk_set_active'),
...
const duplicate = (record) => {
    ...
    router.post(route('business_management.purchase_orders.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => purchaseOrdersExportableColumns(t));
const exportEndpoints   = computed(() => purchaseOrdersExportEndpoints());
...
const tour = useModuleTour({ module: 'purchase_orders', steps: () => purchaseOrdersTourSteps(t) });

const filtersDrawerOpen = ref(false);
const otrosDrawerOpen   = ref(false);
const goToCreate  = () => router.visit(route('business_management.purchase_orders.create'));
const goToTrash   = () => router.visit(route('business_management.purchase_orders.trash'));
const goToAudit   = () => router.visit(route('system_management.audit_logs.index', { module: 'purchase_orders' }));
const goToEditAll = () => router.visit(route('business_management.purchase_orders.edit_all'));

useKeyboardShortcuts({
    'ctrl+n': () => can('purchase_orders.create') && router.visit(route('business_management.purchase_orders.create')),
    'esc': () => {
...
const goEdit   = (record) => router.visit(route('business_management.purchase_orders.edit',   record.slug));
const goDelete = (record) => router.visit(route('business_management.purchase_orders.delete', record.slug));
...
<template>
    <Head :title="$t('purchase_orders.plural')" />

    <div>
        <div class="page-header">
            <PurchaseOrdersPageHeader
                :title="$t('purchase_orders.plural')"
            ...
            <Space wrap class="hide-on-mobile">
                ...
                <span data-tour="export-import">
                    ...
                    <Tooltip v-if="can('purchase_orders.create') && canUsePlanFeature('imports')" :title="$t('global.import_hint')">
                        <Button style="margin-left: 8px;" @click="importOpen = true">
                            <UploadOutlined /> {{ $t('global.import') }}
                        </Button>
                    </Tooltip>
                </span>
                <Tooltip v-if="can('purchase_orders.edit') && canUsePlanFeature('edit_all')" :title="$t('global.edit_all_hint')">
                    <Link :href="route('business_management.purchase_orders.edit_all')" data-tour="edit-all">
                        <Button>
                ...
                <Tooltip v-if="isSuper" :title="$t('global.view_deleted_hint')">
                    <Link :href="route('business_management.purchase_orders.trash')" data-tour="trash">
                        <Button>
                ...
                <Tooltip v-if="canSeeAudit" :title="$t('global.audit_hint')">
                    <Link :href="route('system_management.audit_logs.index', { module: 'purchase_orders' })" data-tour="audit">
                        <Button>
                            <AuditOutlined /> {{ $t('sidebar.group_audit') }}
                        </Button>
                    </Link>
                </Tooltip>
                <Tooltip v-if="can('purchase_orders.create')" :title="$t('global.create_record_hint')">
                    <Link :href="route('business_management.purchase_orders.create')">
                        <Button type="primary">
                            <PlusOutlined /> {{ $t('purchase_orders.new') }}
                        </Button>
                    </Link>
                </Tooltip>
            </Space>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            <div v-auto-animate>
                <PurchaseOrdersBulkBar
                    ...
                    :can-edit="can('purchase_orders.edit')"
                    :can-delete="can('purchase_orders.delete')"
            ...
            <ResponsiveTable
                ...
                :row-selection="(can('purchase_orders.delete') || can('purchase_orders.edit')) ? rowSelection : null"
                rowKey="id"
            ...
            >
                ...
                <template #bodyCell="{ column, record, text, isMobile }">
                    ...
                    <template v-else-if="column.key === 'supplier'">
                        <span v-if="record.supplier">{{ record.supplier.name }}</span>
                        <span v-else class="muted">—</span>
                    </template>

                    <template v-else-if="column.key === 'warehouse'">
                        <Tag v-if="record.warehouse" :bordered="false">
                            <template v-if="record.warehouse.code">{{ record.warehouse.code }} · </template>{{ record.warehouse.name }}
                        </Tag>
                        <span v-else class="muted">—</span>
                    </template>
                    ...
                    <template v-else-if="column.key === 'eta'">
                        <span v-if="record.expected_delivery_date">{{ formatDate(record.expected_delivery_date) }}</span>
                        <span v-else class="muted">—</span>
                    </template>
        ...
        <PurchaseOrdersBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('purchase_orders.record') : $t('purchase_orders.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('purchase_orders.export_title')"
        ...
        <ImportDialog
            ...
            :endpoint="route('business_management.purchase_orders.import')"
            :template-url="route('business_management.purchase_orders.import_template')"
            :resource-label="$t('purchase_orders.records')"
        />

        <PurchaseOrdersMobileBottomBar
