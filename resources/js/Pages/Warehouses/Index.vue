...
import {
    ...
    HomeOutlined, BankOutlined, CarOutlined, CloudOutlined, SwapOutlined,
    CheckCircleFilled,
} from '@ant-design/icons-vue';

// Iconos por tipo de almacen — visual professional (Ant Design Icons, no emojis).
const typeIconMap = {
    main:        HomeOutlined,
    branch:      BankOutlined,
    dropship:    CarOutlined,
    virtual:     CloudOutlined,
    consignment: SwapOutlined,
};
...
import WarehousesBulkBar from '@/Components/Warehouses/WarehousesBulkBar.vue';
import WarehousesBulkDeleteModal from '@/Components/Warehouses/WarehousesBulkDeleteModal.vue';
import WarehousesFavoriteCell from '@/Components/Warehouses/WarehousesFavoriteCell.vue';
import WarehousesDetailDrawer from '@/Components/Warehouses/WarehousesDetailDrawer.vue';
import WarehousesMobileBottomBar from '@/Components/Warehouses/WarehousesMobileBottomBar.vue';
import WarehousesMobileDrawers from '@/Components/Warehouses/WarehousesMobileDrawers.vue';
import WarehousesPageHeader from '@/Components/Warehouses/WarehousesPageHeader.vue';
import WarehousesActionsCell from '@/Components/Warehouses/WarehousesActionsCell.vue';
import WarehousesEmptyState from '@/Components/Warehouses/WarehousesEmptyState.vue';
...
// El toolbar inline de Warehouses (no usa ModuleToolbar) repite manualmente
// estos gates para no mostrar botones que no funcionan al user free/basic.
const { canUse: canUsePlanFeature } = usePlanFeatures();

import {
    warehousesFilterFields, warehousesEmptyFilters, hydrateWarehousesFilters,
    warehousesFiltersToQuery, warehousesFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { warehousesTableColumns } from './config/columns';
import { warehousesExportableColumns, warehousesExportEndpoints } from './config/exports';
import { warehousesTourSteps } from './config/tour';
...
const {
    ...
    hydrate:       hydrateWarehousesFilters,
    toQuery:       warehousesFiltersToQuery,
    summary:       warehousesFiltersSummary,
    empty:         warehousesEmptyFilters,
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.warehouses),
    hasActiveFilters,
    t,
});
...
const allColumns = computed(() =>
    warehousesTableColumns(t, { isSuper: isSuper.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

// ─── Paginacion + sort ──────────────────────────────────────────────────────
const tablePagination = computed(() => ({
    current:  props.warehouses.current_page,
    pageSize: props.warehouses.per_page,
...
useModuleUndoToast('business_management.warehouses.undo_last_delete');

// ─── Favoritos polimorficos ────────────────────────────────────────────────
const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('warehouses', 'warehouses');
...
const { open: drawerVisible, selected: selectedWarehouse, openDetails } = useModuleDrawer({ module: 'warehouses' });
...
const {
    ...
    bulkSetActiveRoute: 'business_management.warehouses.bulk_set_active',
    bulkDeleteRoute:    'business_management.warehouses.bulk_delete',
    resourceLabel:      t('warehouses.records'),
});
...
const duplicate = (record) => {
    ...
    router.post(route('business_management.warehouses.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => warehousesExportableColumns(t));
const exportEndpoints   = computed(() => warehousesExportEndpoints());

// Color del tag de tipo (visual en columna type del listado).
const typeColorMap = { main: 'blue', branch: 'cyan', dropship: 'orange', virtual: 'purple', consignment: 'gold' };
...
const tour = useModuleTour({ module: 'warehouses', steps: () => warehousesTourSteps(t) });

// ─── Mobile: drawers + navegación (patrón Regions: app-like en mobile) ──────
const filtersDrawerOpen = ref(false);
const otrosDrawerOpen   = ref(false);
const goToCreate  = () => router.visit(route('business_management.warehouses.create'));
const goToTrash   = () => router.visit(route('business_management.warehouses.trash'));
const goToAudit   = () => router.visit(route('system_management.audit_logs.index', { module: 'warehouses' }));
const goToEditAll = () => router.visit(route('business_management.warehouses.edit_all'));

// ─── Keyboard shortcuts ────────────────────────────────────────────────────
useKeyboardShortcuts({
    'ctrl+n': () => can('warehouses.create') && router.visit(route('business_management.warehouses.create')),
    'esc': () => {
...
// ─── Acciones ───────────────────────────────────────────────────────────────
const goEdit   = (record) => router.visit(route('business_management.warehouses.edit',   record.slug));
const goDelete = (record) => router.visit(route('business_management.warehouses.delete', record.slug));
</script>

<template>
    <Head :title="$t('warehouses.plural')" />

    <div>
        <div class="page-header">
            <WarehousesPageHeader
                :title="$t('warehouses.plural')"
            ...
            <Space wrap class="hide-on-mobile">
                ...
                <span data-tour="export-import">
                    ...
                    <Tooltip v-if="can('warehouses.create') && canUsePlanFeature('imports')" :title="$t('global.import_hint')">
                        <Button style="margin-left: 8px;" @click="importOpen = true">
                            <UploadOutlined /> {{ $t('global.import') }}
                        </Button>
                    </Tooltip>
                </span>
                <Tooltip v-if="can('warehouses.edit') && canUsePlanFeature('edit_all')" :title="$t('global.edit_all_hint')">
                    <Link :href="route('business_management.warehouses.edit_all')" data-tour="edit-all">
                        <Button>
                ...
                <Tooltip v-if="isSuper" :title="$t('global.view_deleted_hint')">
                    <Link :href="route('business_management.warehouses.trash')" data-tour="trash">
                        <Button>
                ...
                <Tooltip v-if="canSeeAudit" :title="$t('global.audit_hint')">
                    <Link :href="route('system_management.audit_logs.index', { module: 'warehouses' })" data-tour="audit">
                        <Button>
                            <AuditOutlined /> {{ $t('sidebar.group_audit') }}
                        </Button>
                    </Link>
                </Tooltip>
                <Tooltip v-if="can('warehouses.create')" :title="$t('global.create_record_hint')">
                    <Link :href="route('business_management.warehouses.create')">
                        <Button type="primary">
                            <PlusOutlined /> {{ $t('warehouses.new') }}
                        </Button>
                    </Link>
                </Tooltip>
            </Space>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                ...
                :row-selection="(can('warehouses.delete') || can('warehouses.edit')) ? rowSelection : null"
                rowKey="id"
            ...
            >
                ...
                <template #bodyCell="{ column, record, text, isMobile }">
                    ...
                    <template v-else-if="column.key === 'type'">
                        <Tag :color="typeColorMap[record.type] || 'default'" :bordered="false">
                            <component :is="typeIconMap[record.type] || HomeOutlined" style="margin-right: 4px" />
                            {{ $t('warehouses.type_options.' + record.type) }}
                        </Tag>
                    </template>

                    <template v-else-if="column.key === 'is_default'">
                        <Tag v-if="record.is_default" color="gold" :bordered="false">
                            <CheckCircleFilled style="margin-right: 4px" /> {{ $t('warehouses.is_default') }}
                        </Tag>
                        <span v-else>—</span>
                    </template>
...
<template v-else-if="column.key === 'tenant'">
        ...
        <WarehousesBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('warehouses.record') : $t('warehouses.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('warehouses.export_title')"
            :endpoints="exportEndpoints"
            :total-rows="warehouses.total ?? 0"
            :total-unfiltered="warehouses.total_unfiltered ?? warehouses.total ?? 0"
        />

        <ImportDialog
            v-model:open="importOpen"
            :endpoint="route('business_management.warehouses.import')"
            :template-url="route('business_management.warehouses.import_template')"
            :resource-label="$t('warehouses.records')"
        />

        <!-- ── Mobile: bottom bar fijo + drawers (patrón app real, como Regions) ── -->
        <WarehousesMobileBottomBar
