...
import WarehousesEditAllTable from '@/Components/Warehouses/WarehousesEditAllTable.vue';
...
/**
 * Edit-All de Warehouses. Mantenemos snapshot `original` + `draft` mutable
...
const source = computed(() => props.warehouses.data);
...
const saveAll = () => {
    ...
    router.post(
        route('business_management.warehouses.edit_all.update'),
...
<template>
    <Head :title="$t('warehouses.edit_all_title')" />

    <div class="edit-all">
        <SectionHeader
            :back-href="route('business_management.warehouses.index')"
            :title="$t('global.edit_all') + ' — ' + $t('warehouses.plural')"
            :subtitle="$t('warehouses.edit_all_subtitle')"
        >
            <template #icon><EditOutlined /></template>
            <template #actions>
                <Button :disabled="dirtyCount === 0 || submitting" @click="discardAll">
                    <UndoOutlined /> {{ $t('warehouses.edit_all_discard') }}
                </Button>
                <Button
                    type="primary"
                    :loading="submitting"
                    ...
                    @click="saveAll"
                >
                    <SaveOutlined /> {{ $t('warehouses.edit_all_save_all') }}
                </Button>
            </template>
        </SectionHeader>
        ...
        <Alert v-if="dirtyCount > 0" type="info" show-icon class="status-bar">
            <template #message>
                {{ $t('warehouses.edit_all_changes', { count: dirtyCount }) }}
            </template>
        </Alert>
        ...
        <Alert
            ...
            type="error"
            show-icon
            :message="$t('warehouses.name_unique')"
            class="status-bar"
        />

        <Card :bodyStyle="{ padding: 0 }" class="edit-table-card">
            <WarehousesEditAllTable
                v-model:draft="draft"
                :is-dirty="isDirty"
        ...
        <div v-if="warehouses.total > warehouses.per_page" class="pagination">
            <Pagination
                :current="warehouses.current_page"
                :pageSize="warehouses.per_page"
