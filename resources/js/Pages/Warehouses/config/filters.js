 ...
 */
export const warehousesFilterFields = (t) => [
    { key: 'name',           label: t('warehouses.filter_name'), type: 'tags' },
{ key: 'is_active',      label: t('warehouses.is_active'),   type: 'select', options: [
...
export const warehousesEmptyFilters = () => ({
    name: [],
    is_active: null,
    created_at: null,
...
export const hydrateWarehousesFilters = (server) => ({
...
export const warehousesFiltersToQuery = (f) => ({
...
export const warehousesFiltersSummary = (f, t) => {
    const parts = [];
    if (f.name?.length)        parts.push(`${t('warehouses.filter_name')}: ${f.name.join(', ')}`);
if (f.is_active !== null && f.is_active !== undefined) {
        parts.push(`${t('warehouses.is_active')}: ${f.is_active ? t('global.active') : t('global.inactive')}`);
    }
