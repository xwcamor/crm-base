...
const submit = () => {
    form.delete(route('business_management.warehouses.deleteSave', props.warehouse.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('warehouses.singular')" />

    <div class="delete-page">
        <SectionHeader
            :back-href="route('business_management.warehouses.index')"
            :title="$t('global.delete') + ' ' + $t('warehouses.record')"
            :subtitle="$t('warehouses.delete_hint')"
        ...
        <Card class="delete-card" :bodyStyle="{ padding: '24px 28px' }">
            ...
            >
                <template #message>
                    {{ $t('warehouses.delete_about', { name: warehouse.name }) }}
                </template>
            ...
            <div class="record-summary">
                <div class="record-summary__row">
                    ...
                    <span class="record-summary__value">{{ warehouse.id }}</span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('warehouses.name') }}</span>
                    <span class="record-summary__value">{{ warehouse.name }}</span>
                </div>
<div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('warehouses.is_active') }}</span>
                    <span class="record-summary__value">
                        <Tag :color="warehouse.is_active ? 'success' : 'error'" :bordered="false">
