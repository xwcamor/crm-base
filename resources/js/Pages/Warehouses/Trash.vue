...
import WarehousesTrashBulkBar from '@/Components/Warehouses/WarehousesTrashBulkBar.vue';
import WarehousesForceDeleteModal from '@/Components/Warehouses/WarehousesForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { warehousesTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.warehouses.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.warehouses.restore',
    bulkRestoreRouteName: 'business_management.warehouses.bulk_restore',
});
...
const openForceDelete = (warehouse) => {
    forceDeleteTarget.value = warehouse;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.warehouses.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => warehousesTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.warehouses.total === 1 ? t('global.record') : t('global.records');
    return `${props.warehouses.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('warehouses.plural')" />

    <div v-if="isSuper">
        <SectionHeader
            :back-href="route('business_management.warehouses.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('warehouses.plural')"
            :subtitle="subtitle"
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('warehouses.restore_hint')"
