...
import ExchangeRatesTrashBulkBar from '@/Components/ExchangeRates/ExchangeRatesTrashBulkBar.vue';
import ExchangeRatesForceDeleteModal from '@/Components/ExchangeRates/ExchangeRatesForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { exchangeRatesTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.exchange_rates.index'));
}

const searchTerm = ref(props.filters.term ?? '');
...
watch(searchTerm, (val) => {
    ...
    searchTimer = setTimeout(() => {
        router.reload({
            only: ['rates', 'filters'],
            data: { term: val || undefined, page: 1 },
            preserveState: true,
            preserveScroll: true,
            replace: true,
        });
...
const {
    ...
    restoreRouteName:     'business_management.exchange_rates.restore',
    bulkRestoreRouteName: 'business_management.exchange_rates.bulk_restore',
});

const forceDeleteOpen       = ref(false);
const forceDeleteTarget     = ref(null);
const forceDeleteForm       = ref({ display_confirmation: '', reason: '' });
const forceDeleteSubmitting = ref(false);
const forceDeleteErrors     = ref({});

const buildDisplay = (rate) => {
    const date = rate.valid_at ? String(rate.valid_at).substring(0, 10) : '---';
    return `${rate.base_code ?? '???'}/${rate.quote_code ?? '???'} @ ${date}`;
};

const openForceDelete = (rate) => {
    forceDeleteTarget.value = { ...rate, display_name: buildDisplay(rate) };
    forceDeleteForm.value = { display_confirmation: '', reason: '' };
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.exchange_rates.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => exchangeRatesTrashColumns(t));
...
const onTableChange = (pag) => {
    router.reload({
        only: ['rates', 'filters'],
        data: { page: pag.current, per_page: pag.pageSize, term: searchTerm.value || undefined },
        preserveState: true,
        preserveScroll: true,
        replace: true,
    });
...
const subtitle = computed(() => {
    const word = props.rates.total === 1 ? t('global.record') : t('global.records');
    return `${props.rates.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('exchange_rates.plural')" />

    <div v-if="isSuper">
        <SectionHeader
            :back-href="route('business_management.exchange_rates.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('exchange_rates.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            >
                <template #bodyCell="{ column, record }">
                    ...
                    <template v-else-if="column.key === 'valid_at'">
                        {{ formatDateTime(record.valid_at) }}
                    </template>

                    <template v-else-if="column.key === 'base_code' || column.key === 'quote_code'">
                        <code>{{ record[column.dataIndex] }}</code>
                    </template>
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('exchange_rates.restore_hint')"
            ...
            <Empty
                v-if="rates.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <ExchangeRatesForceDeleteModal
