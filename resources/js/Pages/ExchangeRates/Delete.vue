...
const submit = () => {
    form.delete(route('business_management.exchange_rates.deleteSave', props.rate.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('exchange_rates.singular')" />

    <div class="delete-page">
        <SectionHeader
            :back-href="route('business_management.exchange_rates.index')"
            :title="$t('global.delete') + ' ' + $t('exchange_rates.record')"
            :subtitle="$t('exchange_rates.delete_hint')"
        ...
        <Card class="delete-card" :bodyStyle="{ padding: '24px 28px' }">
            <Alert type="warning" show-icon class="mb-4">
                <template #message>
                    {{ $t('exchange_rates.delete_about', { display: rate.display_name }) }}
                </template>
            ...
            <div class="record-summary">
                <div class="record-summary__row">
                    ...
                    <span class="record-summary__value">{{ rate.id }}</span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('exchange_rates.base_code') }} / {{ $t('exchange_rates.quote_code') }}</span>
                    <span class="record-summary__value"><code>{{ rate.base_code }} / {{ rate.quote_code }}</code></span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('exchange_rates.rate') }}</span>
                    <span class="record-summary__value"><strong>{{ Number(rate.rate).toFixed(6) }}</strong></span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('exchange_rates.valid_at') }}</span>
                    <span class="record-summary__value">{{ rate.valid_at }}</span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('exchange_rates.is_active') }}</span>
