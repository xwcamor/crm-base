...
const submit = () => {
    form.delete(route('crm.deals.deleteSave', props.deal.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('deals.singular')" />

    <div class="delete-page">
        <SectionHeader
            :back-href="route('crm.deals.index')"
            :title="$t('global.delete') + ' ' + $t('deals.record')"
            :subtitle="$t('deals.delete_hint')"
        ...
        <Card class="delete-card" :bodyStyle="{ padding: '24px 28px' }">
            ...
            >
                <template #message>
                    {{ $t('deals.delete_about', { name: deal.name }) }}
                </template>
            ...
            <div class="record-summary">
                <div class="record-summary__row">
                    ...
                    <span class="record-summary__value">{{ deal.id }}</span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('deals.name') }}</span>
                    <span class="record-summary__value">{{ deal.name }}</span>
                </div>
<div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('deals.is_active') }}</span>
                    <span class="record-summary__value">
                        <Tag :color="deal.is_active ? 'success' : 'error'" :bordered="false">
                            {{ deal.is_active ? $t('global.active') : $t('global.inactive') }}
                        </Tag>
                    </span>
                </div>
            </div>
