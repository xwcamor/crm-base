...
import DealsTrashBulkBar from '@/Components/Deals/DealsTrashBulkBar.vue';
import DealsForceDeleteModal from '@/Components/Deals/DealsForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { dealsTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('crm.deals.index'));
}
...
const {
    ...
    restoreRouteName:     'crm.deals.restore',
    bulkRestoreRouteName: 'crm.deals.bulk_restore',
});
...
const openForceDelete = (deal) => {
    forceDeleteTarget.value = deal;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('crm.deals.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => dealsTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.deals.total === 1 ? t('global.record') : t('global.records');
    return `${props.deals.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('deals.plural')" />

    <div v-if="isSuper">
        <SectionHeader
            :back-href="route('crm.deals.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('deals.plural')"
            :subtitle="subtitle"
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('deals.restore_hint')"
