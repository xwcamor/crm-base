...
import DealsEditAllTable from '@/Components/Deals/DealsEditAllTable.vue';
...
/**
 * Edit-All de Deals. Mantenemos snapshot `original` + `draft` mutable
...
const source = computed(() => props.deals.data);
...
const saveAll = () => {
    ...
    router.post(
        route('crm.deals.edit_all.update'),
...
<template>
    <Head :title="$t('deals.edit_all_title')" />

    <div class="edit-all">
        <SectionHeader
            :back-href="route('crm.deals.index')"
            :title="$t('global.edit_all') + ' — ' + $t('deals.plural')"
            :subtitle="$t('deals.edit_all_subtitle')"
        >
            <template #icon><EditOutlined /></template>
            <template #actions>
                <Button :disabled="dirtyCount === 0 || submitting" @click="discardAll">
                    <UndoOutlined /> {{ $t('deals.edit_all_discard') }}
                </Button>
                <Button
                    type="primary"
                    :loading="submitting"
                    ...
                    @click="saveAll"
                >
                    <SaveOutlined /> {{ $t('deals.edit_all_save_all') }}
                </Button>
            </template>
        </SectionHeader>
        ...
        <Alert v-if="dirtyCount > 0" type="info" show-icon class="status-bar">
            <template #message>
                {{ $t('deals.edit_all_changes', { count: dirtyCount }) }}
            </template>
        </Alert>
        ...
        <div v-if="deals.total > deals.per_page" class="pagination">
            <Pagination
