...
const submit = () => {
    form.delete(route('business_management.invoices.deleteSave', props.invoice.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('invoices.singular')" />

    <div class="delete-page">
        <SectionHeader
            :back-href="route('business_management.invoices.index')"
            :title="$t('global.delete') + ' ' + $t('invoices.record')"
            :subtitle="$t('invoices.delete_hint')"
        ...
        <Card class="delete-card" :bodyStyle="{ padding: '24px 28px' }">
            ...
            >
                <template #message>
                    {{ $t('invoices.delete_about', { name: invoice.name }) }}
                </template>
            ...
            <div class="record-summary">
                <div class="record-summary__row">
                    ...
                    <span class="record-summary__value">{{ invoice.id }}</span>
                </div>
                <div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('invoices.name') }}</span>
                    <span class="record-summary__value">{{ invoice.name }}</span>
                </div>
<div class="record-summary__row">
                    <span class="record-summary__label">{{ $t('invoices.is_active') }}</span>
                    <span class="record-summary__value">
                        <Tag :color="invoice.is_active ? 'success' : 'error'" :bordered="false">
                            {{ invoice.is_active ? $t('global.active') : $t('global.inactive') }}
                        </Tag>
                    </span>
                </div>
            </div>
            ...
            <Form layout="vertical" @submit.prevent="submit">
                ...
                <DeleteFooter
                    :cancel-href="route('business_management.invoices.index')"
