...
import InvoicesEditAllTable from '@/Components/Invoices/InvoicesEditAllTable.vue';
...
/**
 * Edit-All de Invoices. Mantenemos snapshot `original` + `draft` mutable
...
const source = computed(() => props.invoices.data);
...
const saveAll = () => {
    ...
    router.post(
        route('business_management.invoices.edit_all.update'),
...
<template>
    <Head :title="$t('invoices.edit_all_title')" />

    <div class="edit-all">
        <SectionHeader
            :back-href="route('business_management.invoices.index')"
            :title="$t('global.edit_all') + ' — ' + $t('invoices.plural')"
            :subtitle="$t('invoices.edit_all_subtitle')"
        >
            <template #icon><EditOutlined /></template>
            <template #actions>
                <Button :disabled="dirtyCount === 0 || submitting" @click="discardAll">
                    <UndoOutlined /> {{ $t('invoices.edit_all_discard') }}
                </Button>
                <Button
                    type="primary"
                    :loading="submitting"
                    ...
                    @click="saveAll"
                >
                    <SaveOutlined /> {{ $t('invoices.edit_all_save_all') }}
                </Button>
            </template>
        </SectionHeader>
        ...
        <Alert v-if="dirtyCount > 0" type="info" show-icon class="status-bar">
            <template #message>
                {{ $t('invoices.edit_all_changes', { count: dirtyCount }) }}
            </template>
        </Alert>
        ...
        <Alert
            ...
            type="error"
            show-icon
            :message="$t('invoices.name_unique')"
            class="status-bar"
        />

        <Card :bodyStyle="{ padding: 0 }" class="edit-table-card">
            <InvoicesEditAllTable
                v-model:draft="draft"
                :is-dirty="isDirty"
        ...
        <div v-if="invoices.total > invoices.per_page" class="pagination">
            <Pagination
