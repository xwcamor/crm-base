...
import MessagesTrashBulkBar from '@/Components/Messages/MessagesTrashBulkBar.vue';
import MessagesForceDeleteModal from '@/Components/Messages/MessagesForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { messagesTrashColumns } from './config/trashColumns';
...
const searchTerm = ref(props.filters.subject ?? '');
...
watch(searchTerm, (val) => {
    ...
    searchTimer = setTimeout(() => {
        router.reload({
            ...
            data: { subject: val || undefined, page: 1 },
            preserveState: true,
            preserveScroll: true,
            replace: true,
        });
...
const {
    ...
    restoreRouteName:     'communication.messages.restore',
    bulkRestoreRouteName: 'communication.messages.bulk_restore',
});

const forceDeleteOpen       = ref(false);
const forceDeleteTarget     = ref(null);
const forceDeleteForm       = ref({ subject_confirmation: '', reason: '' });
const forceDeleteSubmitting = ref(false);
const forceDeleteErrors     = ref({});

const openForceDelete = (message) => {
    forceDeleteTarget.value = message;
    forceDeleteForm.value = { subject_confirmation: '', reason: '' };
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('communication.messages.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => messagesTrashColumns(t));
...
const onTableChange = (pag) => {
    router.reload({
        only: ['messages', 'filters'],
        data: { page: pag.current, per_page: pag.pageSize, subject: searchTerm.value || undefined },
        preserveState: true,
        preserveScroll: true,
        replace: true,
    });
...
const subtitle = computed(() => {
    const word = props.messages.total === 1 ? t('global.record') : t('global.records');
    return `${props.messages.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('messages.plural')" />

    <div>
        <SectionHeader
            :back-href="route('communication.messages.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('messages.plural')"
            :subtitle="subtitle"
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('messages.restore_hint')"
