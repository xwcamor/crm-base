...
<head>
    ...
    <style>
        /*
         ...
         | 2) font-weight: SOLO `normal` o `bold`.
         | 3) Evitar `position: fixed` con offsets negativos.
         */
                ...
                @foreach ($rates as $rate)
                    <tr>
                        @foreach ($columns as $col)
                            <td>
                                @switch($col)
                                    @case('id')         {{ $rate->id }} @break
                                    @case('base_code')  {{ $rate->base_code }} @break
                                    @case('quote_code') {{ $rate->quote_code }} @break
                                    @case('rate')       {{ $rate->rate }} @break
                                    @case('valid_at')   {{ $rate->valid_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('source')     {{ $rate->source ?? '' }} @break
                                    @case('is_active')
                                        <span class="{{ $rate->is_active ? 'status-active' : 'status-inactive' }}">
                                            {{ $rate->state_text }}
                                        </span>
                                    @break
                                    @case('slug')       {{ $rate->slug }} @break
                                    @case('created_at') {{ $rate->created_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('updated_at') {{ $rate->updated_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('creator')    {{ $rate->creator->name ?? '—' }} @break
                                    @default {{ $rate->{$col} ?? '' }}
                                @endswitch
                            </td>
                        @endforeach
