                ...
                @foreach ($leadSources as $leadSource)
                    <tr>
                        @foreach ($columns as $col)
                            <td>
                                @switch($col)
                                    @case('id')          {{ $leadSource->id }} @break
                                    @case('name')        {{ $leadSource->name }} @break
                                    @case('description') {{ $leadSource->description ?? '' }} @break
                                    @case('category')    {{ $leadSource->category ?? '' }} @break
                                    @case('sort_order')  {{ $leadSource->sort_order }} @break
                                    @case('is_active')
                                        <span class="{{ $leadSource->is_active ? 'status-active' : 'status-inactive' }}">
                                            {{ $leadSource->state_text }}
                                        </span>
                                    @break
                                    @case('slug')       {{ $leadSource->slug }} @break
                                    @case('created_at') {{ $leadSource->created_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('updated_at') {{ $leadSource->updated_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('creator')    {{ $leadSource->creator->name ?? '—' }} @break
