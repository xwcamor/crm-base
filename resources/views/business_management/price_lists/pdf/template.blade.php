...
<head>
    ...
    <style>
        /*
         | Reglas para PDFs en DomPDF (clon del template de Discounts):
         |
                ...
                @foreach ($priceLists as $priceList)
                    <tr>
                        @foreach ($columns as $col)
                            <td>
                                @switch($col)
                                    @case('id')                  {{ $priceList->id }} @break
                                    @case('name')                {{ $priceList->name }} @break
                                    @case('description')         {{ $priceList->description ?? '' }} @break
                                    @case('currency_code')       {{ $priceList->currency_code ?? '' }} @break
                                    @case('global_discount_pct') {{ $priceList->global_discount_pct }} @break
                                    @case('priority')            {{ $priceList->priority }} @break
                                    @case('valid_from')          {{ $priceList->valid_from?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) ?? '' }} @break
                                    @case('valid_until')         {{ $priceList->valid_until?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) ?? '' }} @break
                                    @case('is_default')
                                        <span class="{{ $priceList->is_default ? 'status-active' : '' }}">
                                            {{ $priceList->is_default ? __('global.yes') : __('global.no') }}
                                        </span>
                                    @break
                                    @case('is_active')
                                        <span class="{{ $priceList->is_active ? 'status-active' : 'status-inactive' }}">
                                            {{ $priceList->state_text }}
                                        </span>
                                    @break
                                    @case('slug')       {{ $priceList->slug }} @break
                                    @case('created_at') {{ $priceList->created_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('updated_at') {{ $priceList->updated_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('creator')    {{ $priceList->creator->name ?? '—' }} @break
