                ...
                @foreach ($categories as $category)
                    <tr>
                        @foreach ($columns as $col)
                            <td>
                                @switch($col)
                                    @case('id')          {{ $category->id }} @break
                                    @case('name')        {{ $category->name }} @break
                                    @case('description') {{ $category->description ?? '' }} @break
                                    @case('parent')      {{ $category->parent?->name ?? '' }} @break
                                    @case('sort_order')  {{ $category->sort_order }} @break
                                    @case('is_active')
                                        <span class="{{ $category->is_active ? 'status-active' : 'status-inactive' }}">
                                            {{ $category->state_text }}
                                        </span>
                                    @break
                                    @case('slug')       {{ $category->slug }} @break
                                    @case('created_at') {{ $category->created_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('updated_at') {{ $category->updated_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('creator')    {{ $category->creator->name ?? '—' }} @break
