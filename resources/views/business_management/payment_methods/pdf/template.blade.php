                        ...
                        @foreach ($columns as $col)
                            <td>
                                @switch($col)
                                    ...
                                    @case('code')                 {{ $method->code ?? '' }} @break
                                    @case('description')          {{ $method->description ?? '' }} @break
                                    @case('integration_provider') {{ $method->integration_provider ?? '' }} @break
                                    @case('requires_reference')   {{ $method->requires_reference ? __('global.yes') : __('global.no') }} @break
                                    @case('sort_order')           {{ $method->sort_order }} @break
                                    @case('is_active')
                                        <span class="{{ $method->is_active ? 'status-active' : 'status-inactive' }}">
                                            {{ $method->state_text }}
                                        </span>
                                    @break
                                    @case('slug')       {{ $method->slug }} @break
                                    @case('created_at') {{ $method->created_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('updated_at') {{ $method->updated_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('creator')    {{ $method->creator->name ?? '—' }} @break
                                    @default {{ $method->{$col} ?? '' }}
                                @endswitch
                            </td>
                        @endforeach
