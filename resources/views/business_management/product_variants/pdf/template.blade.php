                ...
                @foreach ($variants as $variant)
                    <tr>
                        @foreach ($columns as $col)
                            <td>
                                @switch($col)
                                    @case('id')         {{ $variant->id }} @break
                                    @case('name')       {{ $variant->name }} @break
                                    @case('sku')        {{ $variant->sku }} @break
                                    @case('barcode')    {{ $variant->barcode ?? '' }} @break
                                    @case('product')    {{ $variant->product?->name ?? '' }} @break
                                    @case('attributes') {{ is_array($variant->attributes) ? json_encode($variant->attributes, JSON_UNESCAPED_UNICODE) : '' }} @break
                                    @case('cost')       {{ $variant->cost }} @break
                                    @case('price')      {{ $variant->price }} @break
                                    @case('sort_order') {{ $variant->sort_order }} @break
                                    @case('is_active')
                                        <span class="{{ $variant->is_active ? 'status-active' : 'status-inactive' }}">
                                            {{ $variant->state_text }}
                                        </span>
                                    @break
                                    @case('slug')       {{ $variant->slug }} @break
                                    @case('created_at') {{ $variant->created_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('updated_at') {{ $variant->updated_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('creator')    {{ $variant->creator->name ?? '—' }} @break
                                    @default {{ $variant->{$col} ?? '' }}
                                @endswitch
                            </td>
                        @endforeach
