                ...
                @foreach ($messages as $message)
                    <tr>
                        @foreach ($columns as $col)
                            <td>
                                @switch($col)
                                    @case('id')             {{ $message->id }} @break
                                    @case('subject')        {{ $message->subject }} @break
                                    @case('audience_type')  {{ $message->audience_type_text }} @break
                                    @case('audience_id')    {{ $message->audience_id ?? '' }} @break
                                    @case('allow_replies')  {{ $message->allow_replies ? '✓' : '—' }} @break
                                    @case('is_active')
                                        <span class="{{ $message->is_active ? 'status-active' : 'status-inactive' }}">
                                            {{ $message->state_text }}
                                        </span>
                                    @break
                                    @case('status')        {{ $message->status_text }} @break
                                    @case('published_at')  {{ $message->published_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) ?? '' }} @break
                                    @case('expires_at')    {{ $message->expires_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) ?? '' }} @break
                                    @case('slug')          {{ $message->slug }} @break
                                    @case('created_at')    {{ $message->created_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('updated_at')    {{ $message->updated_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('creator')       {{ $message->creator->name ?? '—' }} @break
                                    @default {{ $message->{$col} ?? '' }}
                                @endswitch
                            </td>
                        @endforeach
